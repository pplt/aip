...
(
    ...
    -- Sort does not really work
    -- Must make coefficient lists, and then use sort
    crits := sort unique apply( ptsAndCrits, last );
    --- TODO: sort crits
    apply( crits, c -> 
        (
...
-- Note that for the ideals, I need to also use minimal small but not very small pairs.
-- Need a function that gathers all such points (or better: create an ideal generated by the 
