...
\begin{enumerate}
	...
	\item[$\Box$] Address Pedro's comment after the proof of \ref{fundamental-sequences: L}. 
	...
	\item[$\Box$] Update \Cref{sprouting: S} to remove dependence on $p$ to $r$ when we don't require a prime.  Explain this distinction at the beginning of the section.
	\emily[inline]{I did this, but I will check it over with fresh eyes once more.}	
...
\item[$\Box$]  What do you think about Daniel's comment before \Cref{p-sprout: D}?  Maybe we can discuss how to proceed during the Friday meeting.
...
\begin{proof}
   ...
   imply that there exists an integer $\beta = \beta(\denom, \fsr)$ for which the following conditions hold for $r > \beta$.
    ...
    \]
    \Cref{fsr-exist: T} and our choice of $\fsr$ then allow us to choose $\vv{h}_i \in \fsr \cap \opt \ip ( \matA_i, \vv{u}_i, \vv{s}_i, r)$ such that $\vv{u}_{i+1} = \matA_{i+1} \tail{\vv{s}_i}_p - \matA_{i+1} \vv{h}_i$.
...
\begin{proof}
   ...
   By definition, $\matB$ is the collapse of $\matA$ along $\O$, and $\vv{v} \in \ushort(\matA,\vv{u},\rr)$.
   \Cref{collapse of Newton polyhedron: P} tells us that $\collapse{\O}$ is a face of $\collapse{\N}$, the Newton polyhedron of $\matB=\collapse{\matA}$, defined in $\collapse{\N}$ by the collapsed point $\collapse{\vv{c}}$.  It then follows from \Cref{FT descriptions: P}\ref{min} that $ \degree(\matB,\vv{v}) \leq \iprod{\collapse{\vv{c}}}{\vv{v}}$. 
   
   
   On the other hand, since $\vv{v} \in \ushort(\matA, \vv{u}, \rr)$, we may write $ \vv{v} = \matB \tail{\vv{s}}_{\rr} - \matB \vv{h}$ for some $\vv{s} \in \sp_{\QQ}(\matA, \vv{u})$ and $\vv{h}$  optimal for $\ip = \ip(\matA, \vv{u}, \vv{s}, \rr)$.  Then
%
%\begin{equation}
