...
\pedro[inline]{
   ...
   Feel free to rearrange it, if you'r rather start with the linear program.
}
...
It is easy \daniel{This is true because we only look at monomial pairs.  Should we emphasize this?} to see that the feasible set of the integer program $\IP(A,\vv{u},q)$ is bounded, and consequently $\IP(A,\vv{u},q)$ has a well-defined value.
...
\end{equation}
In this section we show that this limit can be realized as the value of a linear program in $\RR^n$, and also has very simple descriptions in terms of the Newton polyhedron of $A$.
...
\begin{proposition}
   ...
   \begin{enumerate}
      \item\label{value} $\val \LP(A, \vv{u})$\textup;
      \item\label{limit} $\lim_{e\to\infty} \frac{\val \IP(A,\vv{u},p^e)}{p^e}$, where $p$ is a positive integer\textup;
      \item\label{lambda} The unique positive real number $\lambda$ with the property that $(1/\lambda)  \cdot \vv{u}$ lies in the boundary of $\N$\textup;
      \item\label{ip} $\iprod{\vv{c}}{\vv{u}}$, where $\vv{c} \in \RR^d$ is any point that defines a face of $\N$ containing $\mf(A, \vv{u})$.
      \item\label{min ip} $\min_{\vv{d}} \, \iprod{\vv{d}}{\vv{u}}$, where the minimum is over all points $\vv{d} \in \RR^d$ that define a standard face of $\N$\textup;
   \end{enumerate}
\end{proposition}

\begin{proof}
   \ \todo[inline]{Show that $(1) = (2)$}
...
The previous proposition and \eqref{eq: limit} suggest the following definition.
...
\begin{example}\label{ex: ft}
   ...
   From yet another perspective, \Cref{fig: splitting polytope}
   \begin{figure}
      \centering
      \includegraphics[width=.45\textwidth]{Pictures/splitting_polytope.pdf}
      \caption{The feasible region of the linear program $\LP(A,\vv{u})$ in \Cref{ex: ft}}
      \label{fig: splitting polytope}
   \end{figure}
   shows the feasible region of the linear program $\LP(A,\vv{u})$, with its optimal set,
...
% \pedro[inline]{
%    We can probably simplify this proof, as it is very believable that if $\defpt$ is nonnegative, then as $\vv{u}$ ranges over positive integral points, $\iprod{\defpt}{\vv{u}}$ takes on only finitely many values less than $\beta$ (when $c_i \ne 0$, this condition allows only finitely many choices for $u_i$; when $c_i = 0$, the value of $u_i$ does not affect the inner product).
% }
...
%    Note that if $\defpt$ defines $\O$, then \Cref{FT descriptions: P} tells us that
%    %
%    \begin{equation}\label{ft inner product identity: e}
%       \ft{A}{\vv{u}} = \iprod{\defpt}{\vv{u}} = \iprod{\defpt}{\collapse{\vv{u}}}.
%    \end{equation}
%    %
%    The inequality $\ft{A}{\vv{u}} < \beta$ then implies that the collapsed point $\collapse{\vv{u}}$ must lie in the set of all $\vv{w} \in \rs(\O)^{\perp}$ with $\vv{w} \ge \vv{0}$ and $\iprod{\defpt}{\vv{w}} < \beta$.
%    However, the positivity of $\defpt$ in $\rs(\O)^{\perp}$ implies that this set is bounded (\eg one may argue as in the proof of \Cref{bounded polytope: P}), and therefore contains only finitely many lattice points.
...
%    Towards this, the assumption on $\vv{w}$ in the expression $\vv{u} = A \vv{s} + \vv{w}$ implies that $\iprod{\defpt}{\vv{w}} = 0$, which allows us to compute that $\lambda = \iprod{\defpt}{\vv{u}} = \iprod{\defpt}{A\vv{s}} = \norm{\vv{s}}$, where the first equality follows from 
% \Cref{FT descriptions: P}, and the last from the assumption on the coordinates of $\vv{s}$ and the fact that the inner product of $\defpt$ with every column of $A$ contained in $\O$ is one.
...
% %
% \[ \lambda = \iprod{\defpt}{\vv{u}} = \iprod{\defpt}{A \vv{s}} + \iprod{\defpt}{\vv{w}} \geq \norm{\vv{s}} + \iprod{\defpt}{\vv{w}} = \lambda + \iprod{\defpt}{\vv{w}}, \]
% %
% which allows us to conclude that $\iprod{\defpt}{A \vv{s}} = \norm{\vv{s}}$ and $\iprod{\defpt}{\vv{w}} = 0$.
% It follows from these identities, and the fact that the standard basis vectors in $\rb(\O)^{\perp}$ correspond to the positive coordinates of $\defpt$, that the point $\vv{s}$ must satisfy the two asserted conditions.
% \end{proof}
...
% \pedro{May add that $\collapse{\defpt} = \defpt$}
% \daniel{I guess this depends on what we decide above.}
% \pedro{I think this is true regardless. Since $\defpt \in \rs(\O)^\perp$, it is fixed by the projection $\RR^d \to \rs(\O)^\perp$.}
\end{remark}
