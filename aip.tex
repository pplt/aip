...
\begin{proposition} 
   \label{face: P}
   If $\vv{a} \in \RR^d$ defines a face $\O$ of a Newton polyhedron $\N$, then $\vv{a}$ is nonnegative, and the $i$-th coordinate of $\vv{a}$ is zero if and only if $\vv{c} + \lambda \vv{e}_i \in \O$  for every $\vv{c} \in \O$ and $\lambda > 0$.
...
\pedro[inline]{
   \begin{example}\label{ex: illustrating FT descriptions}
      If $A$ and $\vv{u}$ are as in \Cref{ex: ft}, then the minimal face of $(A,\vv{u})$, shown in blue in \Cref{fig: newton polyhedron}, is defined by the point $\vv{c} = (3/17, 0, 1/17)$.
      \Cref{FT descriptions: P} then tells us that
      \[\ft{A}{\vv{u}} = \iprod{\vv{c}}{\vv{u}} = 4/17.\]
...
\begin{figure}
   \centering
   \includegraphics[width=.5\textwidth]{Pictures/splitting_polytope.pdf}
...
The inequality $\ft{A}{\vv{u}} < \beta$ then implies that the collapsed point $\collapse{\vv{u}}$ must lie in the set of all $\vv{w} \in \rs(\O)^{\perp}$ with $\vv{w} > \vv{0}$ and $\iprod{\collapse{\vv{a}}}{\vv{w}} < \beta$.  However, the positivity of $\collapse{\vv{a}}$ in $\rs(\O)^{\perp}$ implies that this polyhedron is bounded (e.g., one may argue as in the proof of \Cref{bounded polytope: P}).  
