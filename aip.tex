...
We now our focus on the fundamental sequences described in \Cref{fundamental-sequences: D},  starting with a preliminary result.
\newcommand{\rr}{{r}}
...
\daniel[inline]{Restate in terms of the fundamental sequences?}
...
In particular, for such $p$ we have $\crit(\matA,\vv{u}) = \degree(\matA,\vv{u})$.
\end{proposition}
...
\begin{claim}
   If $\matB$ is a descendant of $\matA$, $(\matB, \vv{v})$ is a very small pair, and $p \equiv 1 \bmod d$, then $\udeficit(\matB, \vv{v}, p) = \degree(\matB,\vv{v})$.
\end{claim}

Let $\vv{s}$ be a special point for $(\matB, \vv{v})$ with denominator $\denom$, so that $\norm{\vv{s}} = \degree(\matB,\vv{v}) \leq 1$, and $\vv{s} = \vv{t}/\denom$, where $\vv{t}$ has nonnegative integer coordinates bounded above by $\denom$.
...
If $\ip = \ip(\matB, \vv{v}, \vv{s}, p)$, we have that $0 \leq \val \ip < \norm{\vv{s}} = \degree(\matB,\vv{v}) \leq 1$  by \Cref{bounded value: P}, forcing the value of the integer program $\ip$ to be $0$.
Hence $\udeficit(\matB, \vv{v}, p) = \deficit \ip = \norm{\vv{s}} - \val\ip = \degree(\matB,\vv{v})$, proving the claim.\\[2mm]
Observe that if $(\matC, \vv{w})$ is a $p$-sprout of a pair $(\matB, \vv{v})$ as above, then $\degree(\matC,\vv{w}) \leq \udeficit(\matB, \vv{v}, p) = \degree(\matB,\vv{v})$ by \Cref{non-leading coefficients: L} and the above claim, so that $(\matC, \vv{w})$ is also very small.
...
Hence $\collapse{\vv{v}} = \collapse{\matB}\vv{s} = \collapse{\matB}\tail{\vv{s}}_p - \collapse{\matB} \, \vv{0} \in \ushort(\matB, \vv{v}, p)$, and $(\collapse{\matB}, \collapse{\vv{v}}) \in \S_{pq}(\matA, \vv{u})$.

The claim implies that $\S^\ast_{pq}(\matA, \vv{u})$ consists of the pairs in $\S_{pq}(\matA, \vv{u})$ with maximum degree.
We know $\degree(\collapse{\matB},\collapse{\vv{v}}) = \degree(\matB,\vv{v})$ by \Cref{collapse of mf and mc: P}, and
that the degree of any $p$-sprout of $(\matB, \vv{v})$ is bounded above by $\degree(\matB,\vv{v})$ by our earlier observation.
Hence $(\collapse{\matB}, \collapse{\vv{v}}) \in \S^\ast_{pq}(\matA, \vv{u})$.
...
Consequently, $\udeficit(\matA_i,\vv{u}_i,p) = \degree(\matA_i,\vv{u}_i) = \degree(\matA,\vv{u})$, for each $1\le i \le e$.
Then for each such $i$, $\epsilon_{p^i}(\matA,\vv{u}) = \delta_{p^i}(\matA,\vv{u}) = \degree(\matA,\vv{u})$, and the desired formula follows from
...
%       \daniel[inline]{All I would add is that in the second case, the critical exponent looks like $\degree(\matA,\vv{u})-r/\Q$ with $r \in \QQ$ is such that this fraction is an integer divided by $\Q$.  Really, I just wanted to explain why all of the critical exponents in our Examples paper looked the way they did.  There aren't any weird intermediate terms there in the case that a critical exponent is not the $F$-threshold.  Is this worth mentioning?}
%       \pedro[inline]{
