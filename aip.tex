...
equals $\frac{3}{2}p^e - \frac{3}{2}$ for even $e$. 
\end{remark}
...
\begin{example}
   ...
   It is interesting to notice that if we were to make this a three dimensional example by adding a constant row to the matrix $A$, we would then have infinitely many ``layers'' looking exactly like \Cref{fig: small vs very small}, and consequently we would have infinitely many small but not very small pairs. 
   Indeed, this behavior can occur for $\numvars \times n$ monomial matrices $A$ for which $m > 2$. For a perhaps simpler (but similar) example, notice that for $t>1$, the monomial pair $(A, \vv{u})$ associated to ideal pair $(\ideala, \ideald)$ is small, but not very small
if $\ideala= \ideal{x^3 z, y^2z}$ and $\ideald = \ideal{x^2, y, z^t}$ are ideals of $\kk[x,y,z]$. 
\end{example}
...
\begin{example}
   ...
   Notice that in this case,  $\O = \mf(A,\vv{u})$ is bounded, so that collapsing is just the identity.
   For any prime $p$, $\val \IP = 2p - 2$, attained by a unique optimal point, namely $(p-1,p-1)$; thus, $\short \IP = \{(1,1)\}$, which is independent of $p$. \daniel{Should we add ``as guaranteed by \Cref{uniform uniform value and image: T}, at least when $p \gg 0$'' here, since that result does apply to the shortfalls of $\IP$?} %\daniel{Should we also say that $\O = \mf(A,\vv{u})$ is bounded in this example, so that collapsing is just the identity?}
...
Set $\Q = \Q_p(A,\vv{u},p)$.  If $e$ is an integer satisfying $1 \leq e \leq \Q$, then $S_{p^e}^{\ast}(A,\vv{u})$ is non-empty, and by construction of $\sprout_p^{\ast}(A,\vv{u})$, there exists a path $(A,\vv{u}) = (A_1, \vv{u}_1) \to \cdots \to (A_e, \vv{u}_e)$ of length $e-1$ in this graph.
...
%
In this case, \Cref{elementary2: R}\eqref{elementary2 periodic in e} implies that the terms $\delta_{p^s}(A,\vv{u})-\epsilon_{p^s}(A,\vv{u})$ are eventually periodic in $s$, which demonstrates that $\crit(A,\vv{u})$ is rational.
...
Thus, if $p \gg 0$ and $(A,\vv{u})$ is associated to some ideal pair $(\ideala, \ideald)$, then 
...
Suppose that $\operatorname{char} \kk = p > 0$, that $\ideala$ is a monomial ideal of $\kk[x_1, \ldots, x_\numvars]$, and that $\idealb$ is an ideal of this same ring with $\ideala \subseteq \sqrt{\idealb}$.
If $\idealc = \mon(\idealb)$, there exists a positive point $\vv{u} \in \NN^\numvars$ such that for all $e \geq 1$,  the following hold.
\begin{enumerate}
\item $\nu(\ideala, \idealb, p^e) = \nu(\ideala, \idealc, p^e) = \nu(\ideala, \ideald, p^e)$, so that 
\item $\ft{\ideala}{\idealb} = \ft{\ideala}{\idealc} = \ft{\ideala}{\ideald}$, and 
\item $\mu(\ideala, \idealb, p^e) = \mu(\ideala, \idealc, p^e) = \mu(\ideala, \ideald, p^e)$, so that
\item $\crit(\ideala,\idealb) = \crit(\ideala, \idealc) = \crit(\ideala, \ideald)$. 
\end{enumerate}
...
\begin{proof}
For the first equality in (1) and (3), first notice that since $\ideala$ is a monomial ideal, the invariants relying on $\idealc$ are well-defined.  Moreover, 
a (Frobenius) power of $\ideala$ sits in $\idealb^{[p^e]}$ if any only if it sits in $\mon(\idealb^{[p^e]})$, which equals
$\idealc^{[p^e]}$ by \Cref{frobenius-powers+mon-commute: L}.

Now, the monomial ideal $\idealc = \mon(\idealb)$ can be written as the intersection of finitely many ideals of the form $I = \langle x_i^{w_i} \mid w_i \geq 1 \rangle$, for some combinatorially-determined $\vv{w} \in \NN^\numvars$ (e.g., see \cite[Lemma 5.18]{miller+sturmfels.combinatorial_CA}).   
Thus, a (Frobenius) power of $\ideala$ is contained in $\idealc^{[p^e]}$ if and only if it is contained in all  $I^{[p^e]}$, so $\mu(\ideala, \idealc, p^e)$ is the maximum among the  $\mu(\ideala, I, p^e)$.  
Fix $I$ achieving this maximum, and let $t$ denote the number of nonzero coordinates of $\vv{w}$.  
After possibly reordering the variables, assume that $w_i > 0$ for $1 \leq i \leq t$, and $w_i = 0$ for $t < i \leq \numvars$.

If $A$ is an $\numvars \times n$ monomial matrix associated to $\ideala$, let $\vv{r}_i$ denotes its $i$-th row, and let $B$ be the $t \times n$ matrix consisting of the first $t$ rows of $A$.
If some column of $B$ were zero, then some monomial of $\ideala$ would have no $x_i$ in its support, for all $1 \leq i \leq t$.  However, this is not possible since  $\ideala \subseteq \idealc \subseteq I = \ideal{x_1^{w_1}, \ldots, x_t^{w_t}}$. Hence $B$ is monomial, so that for all $e \geq 1$, $\feas P(B, \vv{w}, p^e)$ is bounded.  
Therefore, there exists a positive integer $M$ such that for all  $\vv{h} \in \feas\LP(B, \vv{w})$ and $1 \leq i \leq t$, we have that $\vv{r}_i \cdot \vv{h} < M$. 

Let $\vv{u} \in \NN^n$ be the vector whose $i$-coordinate is $w_i$ for $1 \leq i \leq t$, and is $M$ for $t < i \leq \numvars$.
Given $e \geq 1$, notice that the program $\IP(A, \vv{u}, p^e)$  has \emph{at least as many} constraints as $\IP(B, \vv{w},p^e)$, so
$\feas \IP(A, \vv{u}, p^e) \subseteq \feas \IP(B, \vv{w}, p^e)$.
By the same reasoning, $\feas \IP_p(A, \vv{u}, p^e) \subseteq \feas \IP_p(B, \vv{w}, p^e)$.

On the other hand, if $\vv{k} \in \feas \IP(B, \vv{w},p^e)$, then 
$\vv{k}/p^e \in \feas\LP(B,\vv{w})$ and so
...
Hence $\feas \IP(B, \vv{w}, p^e) \subseteq \feas \IP(A, \vv{u}, p^e)$, and equality holds.
In addition, the arithmetic constraint of $\IP_p$ is also satisfied since  $\vv{k} \geq \vv{0}$, and we conclude that the feasible sets of $\IP_p(B, \vv{w}, p^e)$ and $\IP_p(B, \vv{w}, p^e)$ must also agree.
Hence 
\begin{align*}
&\val\IP(B, \vv{w}, p^e) = \val \IP(A, \vv{u}, p^e) = \nu(\ideala, \ideald, p^e) \\
&\val\IP_p(B, \vv{w}, p^e) = \val \IP_p(A, \vv{u}, p^e) = \mu(\ideala, \ideald, p^e)
\end{align*}
where $\ideald$ is the diagonal ideal associated to $\vv{u} \in \NN^\numvars$. 
Finally, (2) and (4) follow from (1) and (3), respectively.
 \end{proof}
