...
Given a monomial pair $(A, \vv{u})$ and positive integer $q$, we define the \emph{universal deficit} and \emph{shortfall} of $(A,\vv{u},q)$ as follows\textup:
...
and
\[  \collapse{ \short \IP(A, \vv{u}, q)} = \ushort (A, \vv{u}, q).\] 
\end{theorem}
...
\begin{definition}
   ...
   \begin{enumerate}
      \item The collapse of $R$ along $\O$ is the polynomial ring $\collapse{R}$ obtained from $R$ by omitting the variable $x_i$ whenever $\canvec_i \in \rb(\O)$.
      \item The collapse of $\ideala$ along $\O$ is the monomial ideal $\collapse{\ideala}$ of $\collapse{R}$ obtained from $\ideala$ by setting $x_i$ equal to $1$ whenever $\canvec_i \in \rb(\O)$.
      \item The collapse of $\ideald$ along $\O$ is the proper diagonal ideal $\collapse{\ideald}$ of $\collapse{R}$ obtained from $\ideald$ by setting $x_i$ equal to $0$ whenever $\canvec_i \in \rb(\O)$.
   \end{enumerate}
\end{definition}
...
\begin{corollary}
   Given a monomial ideal $\ideala$, there exists an integer $\beta = \beta(\ideala)$ with the following property:
...
\item \label{elementary1 periodic in e} Applying \Cref{finitely many deltas for a fixed A: R} to $A$, and each of its descendent monomial matrices, we see that $\cup_{e>1} \graph_{p^e}(A)$ is a finite set, and their recursive nature then implies that the sets $\graph_{p^e}(A)$ are eventually periodic in $e$.  
...
The following theorem, a generalization of \cite[Lemma 6.1]{budur+mustata+saito.roots_bs_polys_monomial}, is the main result of \Cref{monomial-reduction: A}.  This appendix concludes by applying the results herein to prove \Cref{general-mu-theorem: T} by reducing it to the setting where $\idealb$ is a monomial ideal of the form $\ideald = \ideal{x_1^{u_1}, \ldots, x_m^{u_m}}$ for $\vv{u} > \vv{0}$; i.e., reducing it to \Cref{diagonal-mu-theorem: T}.
...
\daniel[inline]{By the way, it looks like we might only use this when $I$ is principal.\emily[inline]{I think you're right.  It still needs to be cited.}}
...
If $c$ is a nonnegative integer such that $(I: f g_i^c) = (I: f g_i^k)$ for every index $1 \leq i \leq s$ and every integer $k \geq c$, then \[ (I: f g_1^c \cdots g_s^c) = (I: f g_1^{k_1} \cdots g_s^{k_s})\] for all integers $k_1, \cdots, k_s \geq c$.
\end{lemma}

\begin{proof}
We induce on $k_1 + \cdots + k_s$.  The base case is when $k_1 = \cdots = k_s = c$, which is trivial, and the induction step, which is left to the reader, involves repeatedly applying the fact that $(I:fab) = ((I:fa):b)$ for all $a,b \in R$.
\end{proof}
...
Now assume that $1 \leq \ell < \numvars$.
For every $\vv{a} \in \mathcal{A}$ and $\ell < i \leq \numvars$, the ideals $(I: x^{\vv{a}} x_i^k)$ are increasing in $k$.  Hence there exists an integer $c \geq 0$ such that
\begin{equation}
\label{colon-simple: e}
(I: x^{\vv{a}}x_i^c) = (I: x^{\vv{a}}x_i^k)
\end{equation}
for every point $\vv{a} \in \mathcal{A}$, every index $\ell < i \leq \numvars$, and every integer $k \geq c$.


Let $\mathcal{V}$ be the set consisting of all points of the form $\vv{v} = \vv{a} + \sum_{\ell <i \leq \numvars} t_i \vv{e}_i$ where $\vv{a} \in \mathcal{A}$, and each integer $t_i$ satisfies $0 \leq t_i \leq c$.  For every such $\vv{v}$, set $\mathcal{W}(\vv{v}) = \emptyset$ if some $t_i$ is not $c$, and set $\mathcal{W}(\vv{v}) = \{ \vv{e}_i : \ell < i \leq \numvars \}$ otherwise.

After unraveling these definitions, it is obvious that \eqref{stab-1: e} is satisfied.  Furthermore, the only case in which \eqref{stab-2: e} is nontrivial is when $\vv{v} = \vv{a} + \sum_{\ell <i \leq \numvars} c \vv{e}_i$ for some $\vv{a} \in \mathcal{A}$,  in which case we must prove that
%
\[( I : x^{\vv{a}} x_{\ell+1}^c \cdots x_\numvars^c ) =   ( I : x^{\vv{a}} x_{\ell+1}^{c+w_{\ell+1}} \cdots x_{\numvars}^{c+w_\numvars}) \]
for all nonnegative integers $w_{\ell+1}, \ldots, w_{\numvars}$.  However, this is an immediate consequence of \eqref{colon-simple: e} and \Cref{colon-product-stabilization: L}.
\end{proof}
...
\emily[inline]{Note that this Appendix proves \Cref{general-mu-theorem: T} by reducing it to the monomial case.  This is cited as \Cref{diagonal-mu-theorem: T} that no longer exists.} 
