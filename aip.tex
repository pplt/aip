...
The (convex) \emph{cone generated by points $\vv{u}_1,\ldots,\vv{u}_k \in \RR^n$}, denoted $\cone(\vv{u}_1,\ldots,\vv{u}_k)$, is the set consisting of all \emph{conical combinations} of $\vv{u}_1, \ldots, \vv{u}_k$, that is, points of the form $\sum_{i=1}^k \lambda_i \vv{u}_i$, where the $\lambda_i$ are nonnegative real numbers.
...
If $(A, \vv{u})$ is a pair with $\O = \mf(A, \vv{u})$, $\vv{s} \in \sp_{\QQ}(A, \vv{u})$ is a point with denominator $D$, and $q>\beta D$, then 
%
...
\begin{corollary}
Given a monomial matrix $A$, there exists an integer $\beta$ with the following property\textup:  If $q > \beta$ and $(A, \vv{u})$ is a pair with $\O = \mf(A, \vv{u})$, then $\val \IP(A, \vv{u}, q) = \val \IP(\collapse{A}, \collapse{\vv{u}}, q)$ and $\ol{ \im \IP(A, \vv{u}, q)} = \im \IP(\collapse{A}, \collapse{\vv{u}}, q)$ where $\collapse{A}$ \textup(respectively, $\collapse{X}$\textup) is the collapse of $A$  \textup(respectively, $X$\textup) along $\O$.
\end{corollary}
...
\begin{proposition}
   ...
   then every pair $(A, \vv{z})$ with $\vv{z} \in \Z$ is small, and if $p \gg 0$ and $e \geq 0$, then
...
\Cref{fig: sierpinski 3-gasket} illustrates the self-similarity of the Sierpi\'nski $3$-gasket in $\RR^2$.  In general, notice that since each can be realized by removing a union of open sets from $\RR^n$, all $\sierp_p$ are closed sets.  
...
\begin{remark}
The points in $\vv{t}\in \sierp_p$ satisfying $A\vv{t} < \vv{u}$ are precisely those in the intersection of 
...
\]
In \Cref{fig: feas fip}, the feasible set for the problem is shown in red, and the optimal set is highlighted in green, for small values of $p$.  
\end{example}
...
\begin{subfigure}{.48\textwidth}
\centering
  \includegraphics[width=1\textwidth]{Pictures/ex4_char2.pdf}\hskip .04\textwidth
\caption{$p=2$}  
\end{subfigure} 
\begin{subfigure}{.48\textwidth}
\centering
\includegraphics[width=1\textwidth]{Pictures/ex4_char3.pdf}
\caption{$p=3$}  
\end{subfigure}

\begin{subfigure}{.48\textwidth}
\centering
  \includegraphics[width=1\textwidth]{Pictures/ex4_char5.pdf}\hskip .04\textwidth
\caption{$p=5$}  
\end{subfigure}  
\begin{subfigure}{.48\textwidth}
\centering
  \includegraphics[width=1\textwidth]{Pictures/ex4_char7.pdf}
\caption{$p=7$}  
\end{subfigure}  
...
% \emily[inline]{Still need to justify that the closure of $\feas\Sigma^\circ_p(A, \vv{u})$ is \fip_p(A, \vv{u})$.}
% \pedro[inline]{
...
%    Then $(\frac12,\frac12)$ is in $\feas \fip_p(A, \vv{u})$, but is not in the closure of $\feas\Sigma^\circ_p(A, \vv{u})$, which consists of points of norm $\le \frac12$.
...
% }
% \emily[inline]{This is a good idea; this change is now implemented.}
...
\val\fip_p(A,\vv{u}) \ge \displaystyle \lim_{q\to \infty}\frac{\val\IP_p(A,\vv{u}, q)}{q} = \crit(A,\vv{u}).
 \]
On the other hand, fix $\vv{t} \in \feas \LP(A, \vv{u}) \cap \sierp_p$, fix $q \geq 1$ a power of $p$, and a sequence $\{ \vv{t}_e \}_{e=1}^\infty$ in $\NN^n$, for which all $\norm{\vv{t}_e} < p$ and 
...
\crit(A,\vv{u}) = \lim_{q\to \infty} \frac{\val \IP_p(A, \vv{u}, q)}{q} \geq \lim_{q \to \infty}   \norm{\vv{t}^\star_q} = \norm{\vv{t}}.
 \]
Finally, given $\vv{s} \in \feas \fip_p(A, \vv{u}, p)$, fix a sequence of points $\{ \vv{s}_e \}_{e=1}^\infty$ in $\feas \LP(A, \vv{u}) \cap \sierp_p$ that limit to $\vv{s}$. Since $\vv{t} \to \norm{\vv{t}}$ defines a continuous function $\RR^n \to \RR$, we have that $\crit(A, \vv{u}) \geq \lim \limits_{e \to \infty} \norm{ \vv{s}_e } =  \norm{ \vv{s} }$.
