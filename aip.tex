...
\pedro[inline]{
   I combined the lemma with part of the old  \Cref{faces under collapsing: P}, that I thought would fit better here.
}
% \begin{lemma}
...
\begin{proposition}[Faces under collapsing]
   ...
   Then $\pi$ induces a one-to-one correspondence between the faces of $\N$ whose recession bases contain $\rb(\O)$ and the faces of $\collapse{\N}$.
   More precisely,
   \begin{enumerate}[$(1)$]
      \item If $\G$ is a face of $\N$ such that $\rb(\O) \subseteq \rb(\G)$, defined in $\N$ by a point $\vv{g}$, then $\collapse{\G} = \pi(\G)$ is a face of $\collapse{\N}$, defined by the point $\collapse{\vv{g}}$.%, and  $\pi^{-1}(\collapse{\L}) = \L$.
      \item If $\H$ is a face of $\collapse{\N}$ defined by a point $\vv{h}$, then $\pi^{-1}(\H)$ is a face of $\N$ defined by the unique point $\vv{c}\in\rs(\O)^\perp$ such that $\collapse{\vv{c}} = \vv{h}$, and the recession basis of $\pi^{-1}(\H)$ contains $\rb(\O)$.% and $\collapse{\pi^{-1}(\L)} = \L$.
      \item If $\G$ and $\H$ are as above, then $\pi^{-1}(\collapse{\G}) = \G$ and $\collapse{\pi^{-1}(\H)} = \H$.
   \end{enumerate}
   In particular, taking $\G = \O$ in $(1)$, we see that $\collapse{\O}$ is a face of $\N$ defined by a positive point, and is therefore bounded.
\end{proposition}

\begin{proof}
   If $\G$ and $\vv{g}$ are as in $(1)$, the point $\collapse{\vv{g}}$ is obtained by deleting zero coordinates from~$\vv{g}$, so $\iprod{\vv{g}}{\vv{u}} = \iprod{\collapse{\vv{g}}}{\collapse{\vv{u}}}$ for every $\vv{u} \in \RR^\numvars$.
   Let $\vv{v} \in \collapse{\N}$, and choose $\vv{u} \in \N$ such that $\collapse{\vv{u}} = \vv{v}$.
   Then
   \begin{equation}
      \iprod{\collapse{\vv{g}}}{\vv{v}} = \iprod{\vv{g}}{\vv{u}} \ge 1.\label{ineq1}
   \end{equation}
   ...
   If $\H$, $\vv{h}$, and $\vv{c}$ are as in $(2)$, then the point $\vv{h} = \collapse{\vv{c}}$ is obtained by deleting zero coordinates from~$\vv{c}$.
   If $\vv{u} \in \N$, then $\collapse{\vv{u}} \in \collapse{\N}$, so
   \begin{equation*}
      \iprod{\vv{c}}{\vv{u}} = \iprod{\vv{h}}{\collapse{\vv{u}}} \ge 1
   \end{equation*}
   with equality attained if and only if $\collapse{\vv{u}} \in \H$, or equivalently, $\vv{u} \in \pi^{-1}(\H)$.
   This shows that $\pi^{-1}(\H)$ is a face of $\N$ defined by the point $\vv{c}$, and the fact that the recession basis of this face contains $\rb(\O)$ follows immediately from our choice of $\vv{c} \in \rb(\O)^\perp$.

   Turning to $(3)$, note that the second identity holds because $\pi$ is surjective, and that $\G \subseteq \pi^{-1}(\collapse{\G})$ is immediate.
   To verify the reverse containment, note that if $\vv{u} \in \pi^{-1}(\collapse{\G})$, then $\collapse{\vv{u}} \in \collapse{\G}$, and in view of $(1)$ we have $\iprod{\vv{g}}{\vv{u}} = \iprod{\collapse{\vv{g}}}{\collapse{\vv{u}}} = 1$, which implies that $\vv{u} \in \G$.
\end{proof}
