...
TO DO LIST:
\begin{enumerate}
 \item[$\CheckedBox$] Put ``titles'' in new statements, maybe remove some?.
 ...
 \item[$\Box$] Use this to explicitly a finite set $\mathsf{W}$ of positive points in $\NN^m$ such that \[ \{\crit(A, \vv{u}) : (A, \vv{u})\text{ is very small}\} = \{ \crit(A,\vv{u}) : \vv{u} \in \mathsf{W}  \}\] for all $p \gg 0$, depending only on $A$.  The point is that we will need to invoke that crit is invariant under collapsing. 
