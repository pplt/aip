...
Now, specialize to the case that $\LL=\kk$ is a field of characteristic $p>0$.  To conclude in this special case, we must show that $\mu(\ideala, \idealc, p^e) = \val \IP_p(A, \vv{u}, p^e)$.  However, it is clear that $\mu(\ideala, \idealc, p^e) = \val \IP_p(B, \widetilde{\vv{v}}, p^e)$, and to justify that the values of $\IP_p(B, \widetilde{\vv{v}}, p^e)$ and $\IP_p(A, \vv{u}, p^e)$ agree, we will show that their feasible sets coincide:   First, note that the arithmetic constraints of these programs are identical, and that each linear constraint of $\IP_p(B, \widetilde{\vv{v}}, p^e)$ is a linear constraint of $\IP_p(A, \vv{u},p^e)$, and so $\feas \IP_p(A, \vv{u}, p^e)$ is at least contained in $\feas \IP_p(B, \widetilde{\vv{v}}, p^e)$.  On the other hand, if $\vv{k} \in \feas \IP_p(B, \widetilde{\vv{v}}, p^e)$, then the normalized point $\vv{k}/p^e$ is feasible for the linear program $\LP(B,\widetilde{\vv{v}})$, and the bounds $u_i \geq \omega$ for $t < i \leq \numvars$ then guarantee $A (\vv{k}/p^e) < \vv{u}$, which in turn demonstrates that $\vv{k} \in \feas \IP_p(A, \vv{u}, p^e)$. 
...
By the special case considered above, there is a positive point $\vv{u}_i \in \ZZ^\numvars$ such that, when $\LL=\kk$ is a field of characteristic $p>0$, then $\mu(\ideala, \idealc_i, p^e) = \val \IP_p(A, \vv{u}_i, p^e)$ for every positive integer $e$, where $\idealc_i = \mathcal{C}_i \LL[x]$.    It follows from this, \eqref{irreducible-decomposition: e}, and the fact that standard Frobenius powers commute with finite intersections in $\kk[x]$,  that 
