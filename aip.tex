...
\item[$\Box$] Replace any remaining instances of $d$ for dimension with {\tt \text{numvars}}. 
...
\begin{proof}
   ...
   If $\lambda > 0$, then $\vv{u} + \lambda \canvec_i \in \N$ lies in $\O$ if and only if $\iprod{\defpt}{\vv{u} + \lambda \canvec_i} = \alpha + \lambda c_i$ equals $\alpha$.
