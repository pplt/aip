...
\author{Daniel J.~Hern\'andez TEST TEST}
...
By definition, both $\mon(I)_p$ and $\mon(I_p)$ are monomial ideals, and so they are equal if and only if they contain the some monomials.  However, if $h$ is a monomial contained in $I$, then it is clear that $h_p$ is a monomial contained in $I_p$ for all $p$.  Thus, to prove \Cref{mon-operation-modulo-p: T} it suffices to prove the folllowing statement:  If $p \gg 0$ and $h$ is a monomial with $h \notin I$, then $h_p \notin I_p$.

Given any $h$ not contained in $I$, then setting $J = I + \langle h \rangle$ in \Cref{noncontainment mod p: L} tells us that $h_p \notin I_p$ for all primes $p$ that are large relative to $h$.  This argument establishes \Cref{noncontainment mod p: L} in the special case that $\mon(\idealb)$ is $\idealm$-primary.

When $\mon(\idealb)$ is not $\idealm$-primary, then there are infintely many monomials not contained in $I$, and so this argument breaks down.  In this case, our strategy will be to prove that, even though there may be infinitely many monomials not in $I$, there are still only finitely many ideals of the form $(I:x^{\vv{u}})$ with $x^{\vv{u}} \notin I$.    This is accomplished in \Cref{monomial-noetherian-decomposition: L} below.  We begin our arguments by recalling a basic and useful fact about colon ideals.
