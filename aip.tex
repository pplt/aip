...
The set $\sierp_{2,2} \cap [0,1]^2$ is the familiar Sierpi\'nski triangle.  Indeed, the points $\vv{t} \in [0,1]^2$ that have \emph{no} expansion $\vv{t} = \sum_{e=0}^{\infty}\frac{\vv{t}_e}{2^e}$ with $\norm{\vv{t}_1} < 2$ are precisely the points in the open triangle  \[ T = \{ (a,b) \in \RR^2 : a,b < 1, a+b > 1 \}.\]
...
\[ \val \fip_p(A, \vv{u}) : = \max \{ \norm{\vv{t}}: \vv{t} \in \feas \fip_p(A, \vv{u}) \} = \sup \{ \norm{\vv{t}} : \vv{t} \in \sierp_{p,n}, A \vv{t} < \vv{u} \} \] is a well-defined real number.  As usual, we define the optimal set of $\fip_p(A, \vv{u})$ to be the set $\opt \fip_p(A, \vv{u})$ of feasible points that obtain this maximum.
...
\Cref{fig: feas fip details} provides a more precise description of these quantities. It is worth mentioning that when $p=5$, the optimal sets, and hence values, of $\fip_p$ and $P$ agree.  On the other hand, if $p=2,3,6$, then $\val \fip_p < \val P$.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%START OF FIGURE DISPLAYING FRACTAL PROGRAM DATA
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
$p$ & $\val \fip_p$ & $\opt \fip_p$  \\ \hline
$2$ & $1/8$ & $\conv((1/20, 3/40),(1/12, 1/24))$ \\ \hline
$3$ & $1/9$ & $\conv((1/36, 1/12),(7/81, 2/81))$ \\ \hline
$5$ & $7/50$ & $\{(2/25, 3/50) \}$  \\ \hline
$7$ & $34/245$ & $\{(19/245, 3/49) \}$ \\ \hline
\end{tabular}
\endgroup
...
$p^{-e}  \feas \IP_p(A, \vv{u}, p^e)$ lies in $\feas \fip_p(A,\vv{u})$.  It then follows from this and \eqref{ft as limit of normalized program values: eq} that
%
\[
