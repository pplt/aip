...
\subsection{Monomial matrices and monomial ideals}
\label{monomial newton preliminaries: ss}
...
\[ \ideala = \langle x^{\vv{a}_1}, \ldots, x^{\vv{a}_n} \rangle \subseteq \kk[x_1, \ldots, x_\numvars] \]
...
is a $\numvars \times n$ matrix over $\ZZ$ with nonnegative, nonzero rows and columns.
The key role of such matrices in this article motivates the following definition.

\begin{definition}
A \emph{monomial matrix} is a matrix over $\ZZ$ with nonnegative, nonzero rows and columns.
If, as in this section, $A$ is a $\numvars \times n$ monomial matrix, then we call $\ZZ^n$ the \emph{domain lattice}, and $\ZZ^\numvars$ the \emph{target lattice}, of $A$.
\end{definition}
...
\begin{definition}
   A \emph{monomial pair} $(A, \vv{u})$ consists of a monomial matrix $A$ and a positive point $\vv{u}$ in the target lattice of $A$.  If, as in this section, $A$ is a $\numvars \times n$ monomial matrix, then $\vv{u}$ is simply a point in $\NN^\numvars$ with $\vv{u} > \vv{0}$.
\end{definition}
...
\[ \ideald \coloneqq \diag(\vv{u}) \coloneqq \langle x_1^{u_1}, \ldots, x_\numvars^{u_\numvars} \rangle \subseteq \kk[x_1, \ldots, x_\numvars].\]
...
It is also immediate that $\vv{t} \in \sierp_{p,n}$ if and only if $p^m  \vv{t} \in \sierp_{p,n}$ for some (equivalently, for every) integer $m \in \ZZ$; this reflects in the self-symmetry that will be observed in the pictures that follow.
...
\pedro{Together with the line segments connecting $(1,0)$ to $(1,1)$ to $(0,1)$; but maybe we don't need to be so precise here.}
...
\begin{remark}
   Note that the closure in the above definition need not be the set $\{ \vv{t} \in \sierp_{p,n} : A \vv{t} \le \vv{u} \}$.
...
In this subsection, we gather a few simple general facts about such arithmetic programs that are valid in an arbitrary positive characteristic.  More specialized arguments, most of which are only valid in sufficiently large characteristics, will appear in subsequent subsections.  
