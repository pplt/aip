...
\begin{remark}[On $p$ being sufficiently large]
   ...
   Moreover, as in this $\idealm$-primary case there are only finitely many very small pairs, $\ft{A}{\vv{u}}$ also takes on only finitely many values.\footnote{For later use, note that this conclusion remains true without the $\idealm$-primary assumption: when restricted to very small pairs, $\ft{A}{\vv{u}}$ is bounded above by $1$, and thus takes on only finitely many values by \Cref{discreteness: P}.}
...
\begin{example}
   ...
   \daniel{This picture suggests that the points get closer to the Newton polyhedron as the level in the graph increases.  Is this always the case?}
   \pedro{Actually, from second to third level, the points get further.}
   \daniel{OK.  What's up with the bars everywhere?}
\end{example}
...
\begin{lemma}
   ...
   \]
   with equality if $A$ is the monomial matrix associated to an $\idealm$-primary monomial ideal that is generated by homogeneous polynomials all of the same degree, with respect to some positive $\ZZ$-grading.
\end{lemma}
