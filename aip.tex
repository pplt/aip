...
\label{single multiplier: L}
  Consider a Noetherian domain $\LL$.  Given an ideal $I$ of $\LL[x]$, there exists a single nonzero element $\tau \in \LL$ with which to test inclusion in $\mon(I)$, i.e., such that for every monomial $x^{\vv{u}} \in \LL[x]$, $x^{\vv{u}} \in \mon(I)$ if and only if $\tau x^{\vv{u}} \in I$.
\end{lemma}
...
\begin{theorem}
   ...
   Consider a radically compatible pair $(A, \mathcal{C})$, where $A$ is a monomial matrix with $m$ rows, and $\mathcal{C}$ is a collection of monomials in the ambient variables $x = x_1, \ldots, x_m$.
   Then, there exists a finite, nonempty set $\mathcal{W}$ of positive points in $\ZZ^m$ with the following property\textup:
   ...
   and there is a point $\vv{u} \in \mathcal{W}$ that achieves this maximum for every positive integer $e$.
\end{theorem}
