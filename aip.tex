...
\details[inline]{
   ...
   \begin{itemize}
      ...
      \item[$\Box$] Use $\vv{d}$ for defining points?  Pro: opens up $\vv{a}$ for columns of $A$.  Con:  maybe $\collapse{\vv{d}}$ looks ugly?  Another con is "$d$" is used for number of variables and $\RR^d$.  Emily suggest using $\vv{c}$.  This is probably a good idea.
      ...
      Daniel now prefers saying ``$\collapse{X}$ denotes the collapse of $X$ along $\O$\ldots'' without specifying what $X$ is (e.g., a matrix, a point, a subset of lattice points).  Pedro had the better idea to simply write ``Let the superscript bar denote collapse along ...''  I think we agreed Pedro's way is better.
      ...
      \item[$\Box$] Label $\canvec_1$, $\canvec_2$, $\canvec_3$ in Figure 1, and other figures.
...
We say that such a point $\vv{a}$ \emph{defines} $\O$ in $\N$.  In this article, we are laregly concerned with faces $\O$ of $\N$ that do not lie in any coordinate subspace of $\RR^d$, which we call \emph{standard}.


\daniel[inline]{I turned this into an ``official" convention.  I also emphasized that $\alpha$ must be positive when $\O$ is standard, which means we may rescale without changing inequalities}

\begin{convention}
\label{alpha=1: convention}
Let $\O$ and $\vv{a} \in \RR^d$ and $\alpha \in \RR$ be as above.  If $\O$ is standard, then $\alpha$ must be positive, which allow us to rescale $\vv{a}$ so as to assume that $\alpha = 1$.   Thus, throughout this article, we always assume that we have normalized in this way when considering defining points of standard faces.
\end{convention}
...
\pedro[inline]{
   I've elaborated it some more. Maybe it should be a proposition.
}

\begin{remark}\label{rmk: inner product with columns of A}
If $\vv{a} \in \RR^d$ defines a standard face $\O$ of $\N$, and $\vv{s} \in \RR^n$ has nonnegative coordinates, then $\iprod{\vv{a}}{A\vv{s}} \geq \norm{\vv{s}}$,  and equality holds if and only if $s_i = 0$ whenever the $i$-th column of $A$ is not in $\O$. \daniel{I added the condition that $\vv{s}$ be nonnegative.}

Indeed, if $\vv{c}_i$ denotes the $i$-th column of $A$, then \Cref{alpha=1: convention} and the nonnegativity of $\vv{s}$ imply that $\iprod{\vv{a}}{\vv{c}_i} \cdot s_i \geq s_i$ for every $1 \leq i \leq n$, with equality if and only if $s_i = 0$ or $\vv{c}_i \in \O$.
Thus,
\[ \iprod{\vv{a}}{A\vv{s}} = \sum_{i=1}^n \iprod{\vv{a}}{\vv{c}_i} \cdot s_i \geq  \sum_{i=1}^n s_i  = \norm{\vv{s}},\]
and equality holds if and only if $s_i = 0$ whenever $\vv{c}_i \notin \O$.
\end{remark}
...
\begin{proposition} 
   \label{face: P}
   If $\vv{a} \in \RR^d$ defines a face $\O$ of a Newton polyhedron $\N$, then $\vv{a}$ is nonnegative, and the $i$-th coordinate of $\vv{a}$ is zero if and only if $\vv{u} + \lambda \canvec_i \in \O$  for every $\vv{u} \in \O$ and $\lambda > 0$.
   In particular, the supporting indices of $\vv{a}$ depend only on $\O$, and $\O$ is bounded if and only if $\vv{a}$ is positive. 
\end{proposition}

\daniel[inline]{I changed $\vv{c} \in \O$ to $\vv{u} \in \O$.  I think that the convention of these notes (at least, this is how it is in my head) is that letters lower in the alphabet (e.g., $\vv{a}, \vv{b}, \vv{c}$) represent points in the dual space to $\RR^d$ (that is, determine the linear function $\iprod{\vv{a}}{\cdot}$) and points higher in the alphabet $(e.g., \vv{u}, \vv{v}, \vv{w})$ represent points in $\RR^d$ itself}
\pedro[inline]{That's a useful convention. I'll keep it in mind.}
\daniel[inline]{We don't always stick to it.  E.g., the columns of $A$, and the standard basis vectors.}
\pedro[inline]{We could use, e.g., $\vv{v}_i$ for the columns of $A$.}

\begin{proof}
   If $\vv{u} \in \O$, then adding to $\vv{u}$ any nonnegative point in $\RR^d$ produces a point in $\N$.
   In particular, if $\iprod{\vv{a}}{\vv{u}} = \alpha$, then $\iprod{\vv{a}}{\vv{u} + \lambda \canvec_i} \geq \alpha$ for every standard basis vector $\canvec_i$ in $\RR^d$ and $\lambda > 0$.
   This observation implies that $a_i = \iprod{\vv{a}}{\canvec_i} \ge 0$ for each $i$, so $\vv{a} \geq \vv{0}$, and that $\vv{u} + \lambda \canvec_i \in \O$ for every $\lambda > 0$ if and only if $\iprod{\vv{a}}{\canvec_i} = 0$.  

Similar logic will show that if $\rb(\O) \coloneqq  \{ \canvec_i \in \RR^d : \iprod{\vv{a}}{\canvec_i} = 0\}$, then 
\begin{equation}
\label{face: e}
...
\begin{definition}
   If $\vv{a} \in \RR^d$ defines $\O$, then the \emph{recession basis} of $\O$ is the set $\rb(\O)$ of all standard basis vectors $\canvec_i$ in $\RR^d$ such that the $i$-th coordinate of $\vv{a}$ is zero, and the \emph{recession subspace} of $\O$ is the subspace $\rs(\O)$ of $\RR^d$ spanned by $\rb(\O)$.  
\end{definition}
...
%  \pedro[inline]{
%     My preference would be to keep zeros, and avoid abuse of notation; work always in the same $\RR^d$, and in the same polynomial ring.
%     I don't really see a conflict between keeping zeros versus drawing things in a smaller space---if we have an object that lives in, say, $\RR^3$, but happens to be contained in some coordinate plane, it's natural to draw that object in the plane, instead of 3-space; we'd just say that we are only showing that subset of $\RR^3$.
...
%     OR, perhaps better, leave $\rs(\O)^\perp$ alone---don't make any identification---but define collapse as the composition of the maps
%     \[\RR^d \stackrel{\text{proj}}{\longrightarrow} \rs(\O)^\perp \stackrel{\approx}{\longrightarrow} \RR^{\#\rb(\O)^\perp}.\]
%     This seems to be how you're thinking of collapses.
%  }
...
%  }
% \emily[inline]{
% OK, I finally got a chance to think carefully about this; thanks for waiting.  Although I see the points both ways, since we are motivated by algebra, I prefer to omit the rows rather than to have zero rows, in analog with moving to a polynomial ring in fewer variables, rather than remaining in the same ring. The geometry also simplifies
% in a very obvious way.   
% I also have been thinking of $\rs(\O)^{\perp}$ as $\operatorname{span}_{\RR} \rb(\O)^{\perp}$. 
% Although we might need to be careful with our definitions, I think it will be worth it.  
% }
% \pedro[inline]{
%    Thanks for your input, Emily.
%    My suggestion, then, it that we leave the definitions of $\rs(\O)$ and $\rs(\O)^\perp$ unchanged (as subsets of $\RR^d$), and define collapse as the composition of the maps
%    \[\RR^d \stackrel{\text{proj}}{\longrightarrow} \rs(\O)^\perp \stackrel{\approx}{\longrightarrow} \RR^{\#\rb(\O)^\perp},\]
%    where the second map is the coordinate isomorphism with respect to the obvious basis, and the collapse of $A$ as the matrix of the composition of $\RR^n \stackrel{A}{\longrightarrow} \RR^d$ and the above.
%    This way collapse will be given by ``deleting rows'', as opposed to replacing them with zeros.
% }
...
\begin{remark}
   ...
   If $\vv{a} \in \RR^d$ defines $\O$ in $\N$, then the standard basis vectors in $\rb(\O)^{\perp}$ correspond to the nonzero coordinates of $\vv{a}$.
   Consequently, $\iprod{\vv{a}}{\vv{u}} = \iprod{\collapse{\vv{a}}}{\collapse{\vv{u}}}$ for every $\vv{u} \in \RR^d$.
...
\section{Linear programs associated with monomial pairs}
...
\begin{definition}
Given a monomial pair $(A,\vv{u})$, where $A$ is a matrix with $n$ columns, the linear program $\LP(A, \vv{u})$ in $\RR^n$ consists of maximizing the function $\vv{k} \mapsto \norm{\vv{k}}$ subject to the constraints $\vv{k} \geq \vv{0}$ and $A \vv{k} \leq \vv{u}$.
\end{definition}


It is easy \daniel{This is true because we only look at monomial pairs.  Should we emphasize this?} to see that the feasible set of the linear program $\LP(A,\vv{u})$ is bounded, and therefore a polytope in $\RR^n$.
Consequently, $\LP(A,\vv{u})$ has a well-defined value.

\subsection{Relations with $F$-thresholds}
\label{opt sets: SS}

\ \comment[inline]{Maybe we should fix $(A, \vv{u})$ and $\N$.}
...
\begin{proposition}
   ...
   If $\N$ is the Newton polyhedron of $A$, then
...
\daniel[inline]{Your proof is way better, and should be the one we give in the paper.  I sometimes get too carried away with collapsing}
\pedro[inline]{I'm giving it a shot below; original proof is commented out.}

\begin{proof}
   It suffices to show that there are only finitely many numbers $\ft{A}{\vv{u}}$ less than $\beta$ with $\mf(A, \vv{u}) = \O$ being fixed.
   Consider such a pair, and suppose $\vv{a}$ defines $\O$ in the Newton polyhedron of $A$.
   Then \Cref{FT descriptions: P} tells us that $\ft{A}{\vv{u}} = \iprod{\vv{a}}{\vv{u}}$, and the nonnegativity of $\vv{a}$ implies that, as $\vv{u}$ ranges over positive integral points, this inner product takes on only finitely many values less than $\beta$.
   Indeed, when $a_i \ne 0$, this upper bound allows only finitely many choices for $u_i$, while when $a_i = 0$, the value of $u_i$ does not affect the inner product.
   \end{proof}
...
% \begin{proof}
%    Set $\LP = \LP(A, \vv{u})$ and $\lambda = \val \LP$, and fix $\vv{a} \in \RR^d$ that defines the face $\O = \mf(A, \vv{u})$ in the Newton polyhedron of $A$.
...
% Next, suppose that $\canvec_i \in \rb(\O)$.
%  If every vertex $\vv{s}$ of the optimal set of $\LP = \LP(A, \vv{u})$ was such that $A \vv{s}$ agreed with $\vv{u}$ in the $i$-th coordinate, then the same would be true for every point in the optimal set.
%  However, the special point constructed above shows that this is impossible.
%  Therefore, for every $\canvec_i \in \rb(\O)$, there exists a vertex $\vv{s}_i$ of $\opt \LP$ such that $A \vv{s}_i$ is less than $\vv{u}$ in the $i$-th coordinate.
...
\end{align*}
so the optimal image of $\IP(A,\vv{u},q)$ characterizes the ``leftovers'' of $\ideala^\nu$ modulo $\ideald^{[q]}$.
...
\begin{proof}   
   Fix a point $\vv{a} \in \RR^d$ that defines $\O  = \mf(A, \vv{u})$, and let an overbar denote collapse along $\O$.
   Thus,
...
%\begin{proof} If $\vv{k}$ is feasible for $\IP_p(A, \vv{u}, p^e)$ and $ p^e \canvec_i  \leq \vv{k}$ for some standard basis vector $\canvec_i$ of $\ZZ^n$, then $p^e A \canvec_i  \leq A \vv{k} < \vv{u}p^e$, and therefore $A \canvec_i < \vv{u}$, which contradicts the smallness of $(A, \vv{u})$.  Thus, every coordinate of $\vv{k}$ is less than $p^e$, and so the base $p$ expansion of $\vv{k}$ is of the form $\vv{k} = \vv{k}_0 + \cdots + \vv{k}_{e-1} p^{e-1}$.  The arithmetic constraint of the program then implies that $\norm{\vv{k}} \leq p^e-1$.
%\end{proof}
...
If $\vv{a} \in \RR^d$ defines the face $\O$ in $\N$, then \Cref{collapse of Newton polyhedron: P}  states that $\collapse{\vv{a}}$ defines the standard face $\collapse{\O}$ of $\collapse{\N}$, the Newton polyhedron of $B$, and \Cref{FT descriptions: P} then implies that \[\ft{B}{\vv{v}} \leq \iprod{\collapse{\vv{a}}}{\vv{v}}.\]
