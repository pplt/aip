...
\Input{ \\
   ...
   $\bullet$ A universal denominator $\denom$ for $A$\\
   $\bullet$ A representative $p_0$ for a congruence class modulo $\denom$
}
\medskip
\Output{ \\ $\bullet$ A generating function $F(\mathsf{p},\mathsf{t}) \in \QQ(\mathsf{p})\llbracket \mathsf{t} \rrbracket$ for $\val\IP_\pp(A,\vv{u},\pp^e)$
}
\medskip
$\S_{p},\  \S^*_{p} \leftarrow \{(A,\vv{u})\}$\;
$e \leftarrow 1$\;
%\texttt{done} $\leftarrow$ \texttt{false}\;
$M_1 \leftarrow \ft{A}{\vv{u}}\mathsf{p} - \udeficit(A,\vv{u},p_0)$\;
\While{true}{
   $e\leftarrow e+1$\;
   $S_{p^e} \leftarrow \big\{(A,\vv{v}): \vv{v} \in \bigcup_{(A,\vv{v}^*) \in \S^*_{p^{e-1}}} \ushort(A,\vv{v}^*, p_0)\big\}$\;
   \If{$\exists k < e: \S_{p^{k}} = \S_{p^e}$}{
      \KwRet{$\frac{\sum_{1\le i<k} M_i\tt^i}{1-\pp\tt}+\frac{\sum_{k\le i<e} M_i\tt^i}{(1-\pp\tt)(1-\tt^{e-k})}$}
   }
   $\max \leftarrow \max \{\ft{A}{\vv{v}}: (A,\vv{v}) \in \S_{p^e}\}$\;
   \If{$\max >1$}{
      \KwRet{$\frac{\sum_{1\le i<e}M_i\tt^i}{1-\pp\tt} + \frac{(\pp - 1)\tt^e}{(1-\pp \tt)(1-\tt)}$}
   }
   $\S^*_{p^e} \leftarrow \{(A,\vv{v})\in \S_{p^e}: \ft{A}{\vv{v}} = \max\}$\;
   $\min \leftarrow \min\{\udeficit(A,\vv{v},p_0): (A,\vv{v})\in \S^*_{p^e}\}$\;
   $\S^*_{p^e} \leftarrow \{(A,\vv{v})\in \S^*_{p^e}: \udeficit(A,\vv{v},p_0) = \min\}$\;
   $M_e \leftarrow p \max - \min$\;
}
\end{algorithm}
