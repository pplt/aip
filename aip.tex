...
\daniel[inline]{Describe the significance of the stability of the optimal image here.  Maybe give an example.}
...
Below, we identify an important case in which we are able to solve the arithmetic integer program $\IP_p(A, \vv{u}, p^e)$ in sufficiently large characteristics.   This result is fundamental, and using it, we will eventually be able to solve every such arithmetic program, at least when $p$ is large relative to $A$.
...
If $(A, \vv{u})$ is very small, and $p > \beta$, then  \[ \val \IP_p(A, \vv{u}, p) = \ft{A}{\vv{u}} \cdot p - \delta(A, \vv{u}, p) \] and 
