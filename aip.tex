...
In this section, we consider a variant of an integer program in which we impose an additional and highly nonlinear constraint.
As this new constraint is arithmetic in nature, we call such an optimization problem an \emph{arithmetic integer program}, and we will focus exclusively on one such family of optimization problems.
...
\begin{align*}
  \ideala^{[\mu]} &= \ideal{x^{A\vv{k}}: \binom{\mu}{\vv{k}} \not\equiv 0 \bmod{p}}\\
  &\equiv \ideal{x^{A\vv{k}}: \binom{\mu}{\vv{k}} \not\equiv 0 \bmod{p}\text{ and } A\vv{k} <\vv{u}q} \bmod \ideald^{[q]}\\
...
\end{equation*}
In the next subsection we show that this limit can be realized as the value of a \emph{fractal linear program} in the domain of $A$.
...
\]
In \Cref{fig: feas fip}, the feasible set for $\fip_p = \fip_p(A,\vv{u})$ is shown in blue, the feasible set for $\LP(A,\vv{u})$ is shown in gray, and the optimal set is highlighted in green, for small values of $p$.  
\begin{figure}
  \centering
\begin{subfigure}{.49\textwidth}
\centering
  \includegraphics[width=.9\textwidth]{Pictures/ex4_char2.pdf}\hskip .04\textwidth
\caption{$\opt \fip_2 = \conv\big(\big(\frac1{20},\frac3{40}\big),\big(\frac1{12},\frac1{24}\big)\big)$}  
\end{subfigure} 
\begin{subfigure}{.49\textwidth}
\centering
\includegraphics[width=.9\textwidth]{Pictures/ex4_char3.pdf}
\caption{$\opt \fip_3 = \conv\big(\big(\frac1{36},\frac1{12}\big),\big(\frac7{81},\frac2{81}\big)\big)$}  
\end{subfigure}
...
\caption{$\opt \fip_5 = \opt \LP(A,\vv{u}) = \big\{\big(\frac2{25}, \frac3{50}\big)\big\}$}  
\end{subfigure}  
\begin{subfigure}{.49\textwidth}
\centering
  \includegraphics[width=.9\textwidth]{Pictures/ex4_char7.pdf}
\caption{$\opt \fip_7 = \big\{\big(\frac{19}{245}, \frac3{49}\big)\big\}$}  
\end{subfigure}  
\caption{The feasible and optimal sets of $\fip_p = \fip_p(A, \vv{u})$ for $A$ and $\vv{u}$ described in \Cref{ex: feas fip}, for various $p$ values.}
\label{fig: feas fip}
\end{figure}
\end{example}
