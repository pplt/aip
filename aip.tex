...
TO DO LIST:
\begin{enumerate}
 \item Write essential statements/proofs.
 \item Decide on name of paper.
 \item Decide on name for minimal coordinate.
 \item Make sprout notation.
 \item Put in examples.
 \item Decide on how to define $\widehat{\witt}$.  (Sprouting graph?)
 \item Fill in/rewrite preliminaries.
 \item Reorganize and motivate the $\Pi$ and $\Theta$, and their connection, in Sections 5 and 6.
 \item ``fractal linear program;'' solve $P$ in ``Sierpinski gasket''
 \item Direct proof that $\delta$/$\Delta$ are independent of $\vv{s}$.
 \item Minimize and/or ``algebrafy'' statements.
 \item Background section on Frobenius powers, $\mu$, $\nu$, etc.
 \item Derive some easy corollaries for very general hypersurfaces.
\end{enumerate}
}
...
\emily[inline]{Track where $\mathbb{O}$ is used:  Maybe it is only used in a few proofs.  Then we could take out the definition and just remark that these finiteness properties exist. }
...
\emily[inline]{The following could just replace the above statement?  Replace integer programming language with algebraic language?}
...
\pedro[inline]{Postpone introduction of medium small points until immediately before definition of $\widehat{\witt}$ graph?}
...
\emily[inline]{We think that $\{ \witt^e : e \geq 1 \}$ is finite, but only care that $\bigcup_{e=1}^\infty \witt^e(A)$ is.}
...
\emily[inline]{This is a terminating path in $\widehat{\witt}(A)$ (Pedro's version! i.e., all pairs are small and the last is medium-small)} 
...
% \[ \val \IP_p(B, \vv{v}p) = \ft{B}{\vv{v}} \cdot p - \delta_p(B, \vv{v}) \] for any vertex $(B, \vv{v})$ of $\widehat{\witt}_p(A, \vv{u})$.    In addition, if $(B, \vv{v})$ and $(D, \vv{z})$ are any two such vertices, then $\val \IP_p(B, \vv{v}) < \val \IP_p(D, \vv{z})$ if and only if $\ft{B}{\vv{v}} < \ft{D}{\vv{z}}$, or these two quantities agree and $\delta_p(B, \vv{v}) > \delta_p(D, \vv{z})$.  
% \end{corollary}
...
\begin{corollary}\label{cor: constant mus}
   If $(A,\vv{u})$ is a small monomial pair and  $\beta = \beta(A)$ is as in \Cref{thm: higher mu formula for paths}, then $\crit$ and $\mu$ are constant on each level of $\widehat{\witt}(A,\vv{u},p)$ for every $p\ge \beta$. 
\end{corollary}
...
%    If $(A_1, \vv{u}_1) \to \cdots \to (A_e, \vv{u}_e)$ is a path in $\widehat{\mathfrak{W}}(A,\vv{u},p)$, and $(B, \vv{v})$ is any vertex of $\widehat{\mathfrak{W}}(A, \vv{u},p)$ whose level is the same as that of $(A_1, \vv{u}_1)$, then
%    \begin{equation*}
%      \mu(B,\vv{v},qp^e)
...
% 
% \details[inline]{
% Choose $\beta$ so that the conclusions of \Cref{arithmetic uniform value and image: T,trivial max value: T,cor: upper bound for higher mus} (and maybe more\ldots) hold for each one of the finitely many pairs in $\widehat{\witt}(A, \vv{u},p)$ whenever $p > \beta$.
...
%     Towards this, first note that if $(C, \vv{z})$ does not lie in $\widehat{\witt}(A,\vv{u},p)$, then \Cref{cor: mu comparison} implies that $\mu(C, \vv{z},qp^{e-1}) < \mu(A_2,\vv{u}_2, qp^{e-1})$.
...
\todo[inline]{Point out that the levels of $\widehat{\witt}(A, \vv{u})$ are eventually periodic. This will give an independent proof the rationality of critical exponents.  The way we present a formula for critical exponents may need this observation.}
