...
TO DO LIST:
\begin{enumerate}
	\item[$\Box$] Have a discussion about using $\degree(\matA,\vv{u})$. In the meantime, use the macro ``degree.''
        \emily[inline]{I believe that I replaced the current instances of ``deg'' with ``degree,'' but need to do the same for ``ft.''}
        \pedro[inline]{I replaced all the ``ft of a monomial pair'' with ``degree''.}
        \emily[inline]{Daniel mentioned he has been using the macro ``deg,'' so we should make sure those are updated at some point when there is no chance for conflicts.}
	\item[$\CheckedBox$] Address Pedro's comment after the proof of \ref{fundamental-sequences: L}. 
	\daniel[inline]{Done, but please check out the updated proof.}
	\emily[inline]{Looks good to me.}
	\item[$\Box$]  Set some notation early on, possibly in the introduction:  A pair of ideals is $(\ideala, \idealb)$ is \emph{compatible} if $\ideala \subseteq \sqrt{\idealb}$, is \emph{small compatible} if $\ideala \subseteq \idealb$, and is \emph{very small} if it is compatible and the $F$-threshold of $\ideala$ with respect to $\idealb$ is less than one.  These things appear throughout so should be updated, especially throughout the appendix and remove the definition after \Cref{computing nu for large q: C}. 
	\item[$\CheckedBox$] Update \Cref{sprouting: S} to remove dependence on $p$ to $r$ when we don't require a prime.  Explain this distinction at the beginning of the section.
 ...
 \item[$\Box$] Describe the critical exponents of monomial ideals in \Cref{frobenius-powers-monomial-ideals: S}.  
 ...
 \details[inline]{
 	TO DO LIST:
 	\begin{enumerate}
 	\item[$\Box$]{Address Pedro's questions:
\begin{itemize}
\item If a positive u lies in the boundary of the Newton polyhedron, and $u+e_i$ also lies in the boundary, for every $i$, is the associated ideal principal?
...
\end{itemize}	
 ...
 \emily[inline]{I think it is well-explained; I made a few very minor changes.}
 ...
 \pedro[inline]{
    I included a homogeneous but not $\idealm$-primary example (\Cref{ex: new homogeneous example}).
    If you guys have any suggestions of examples to be added or to replace this one---maybe something less random; something with some mathematical meaning to you---please let me know.
 }
...
\begin{proof}
   ...
   Since there are only finitely many faces, there must be distinct points $\vv{w}_1 < \vv{w}_2$ in this line segment that lie in the same face $\O$.
   If $\defpt$ is a point defining $\O$ in $\N$, then $\iprod{\defpt}{\vv{w}_2- \vv{w}_1}$ must be zero.
   This inner product is a positive linear combination of the components $c_i$ of $\defpt$ with $i\in I$.
...
\begin{proposition}[Minimal face, degree, special and optimal points under collapsing]
   ...
   \begin{enumerate}[$(1)$]
      \item $\mf(\collapse{\matA}, \collapse{\vv{u}}) = \collapse{\O}$ and $\degree(\matA,\vv{u}) = \degree(\collapse{\matA},\collapse{\vv{u}})$.
...
% \begin{proof}
%    Let $\lambda = \degree(\matA,\vv{u}) = \iprod{\defpt}{\vv{u}}$.
...
\label{integrailty-property-of-universal-deficits: R}  The quanitity
\[ \deg(\matA, \vv{u}) \cdot q - \udeficit(\matA, \vv{u}, q) \]
is a natural number for all $(\matA, \vv{u})$ and $q$.   Indeed, as we have already see, if $\vv{s} \in \sp_{\QQ}(\matA, \vv{u})$, then the associated \hl{canonical feasible point} $\vv{s}q - \tail{\vv{s}}_q$ has nonnegative entries.  Thus, its norm is a a natural integer, and adding to this the value of $\ip = \ip(\matA, \vv{u}, \vv{s}, q)$, which is also a natural number, we see that 
...
Recall that a pair of ideals $(\ideala, \idealb)$ of a polynomial ring over a field are called \hl{\emph{compatible}} if $\idealb$ contains $\sqrt{\ideala}$, that is, there exists a positive integer $t$ such that $\ideala^t \subseteq \idealb$. \daniel{Defined outside of appendix?}
...
This graph's construction relies on a positive integer $r$, and its powers, where $r$ need not be prime.  Indeed, they are defined in terms of shortfalls and universal shortfalls, and deficits and universal deficits, which we remind the reader are defined in terms of an arbitrary positive integer $q$.  We choose ``$r$'' because it will ultimately represent a residue class modulo an integer. 
...
\begin{definition}[$r$-sprout]
   \label{r-sprout: D}
   Consider a monomial pair $(\matA,\vv{u})$, and let an overbar denote collapse along $\mf(\matA,\vv{u})$.
   We say that $(\matB, \vv{v})$ is a \emph{$r$-sprout} of $(\matA, \vv{u})$ whenever the following conditions are satisfied.
   \begin{enumerate}
      \item $\matB = \collapse{\matA}$, the collapse of $\matA$ along $\O = \mf(\matA, \vv{u})$.
      \item $\vv{v}$ is any point in $\ushort(\matA, \vv{u}, r) = \ushort(\matB, \collapse{\vv{u}}, r)$.
   \end{enumerate}
\end{definition}
...
\begin{remark}
   ...
   This and the positivity of points in shortfalls show that the $r$-sprout of a monomial pair is also a monomial pair.  Furthermore,  \Cref{finite image: P} tells us that there are only finitely many $r$-sprouts of a fixed monomial pair, while \Cref{small pairs from shortfalls: R} tells us that each such sprouted pair is small.
 \end{remark}
...
%   \emily{No, I removed the ``$= \ushort(\matA,\vv{u},p)$'' since our intention is to choose a point in the actual shortfall.}
   Recall that $p$-shortfall pairs played an important role in the process for computing $\val \IP_p(\matA,\vv{u},p^e)$ described in \Cref{convenient: SS} when $\matA$ is convenient.

   It follows immediately from \Cref{r-sprout: D} and \Cref{arithmetic uniform value and image: T} that a $p$-sprout of a pair $(\matA,\vv{u})$ is the same as the collapse of a $p$-shortfall pair of $(\matA,\vv{u})$ along the face $\O=\mf(\matA,\vv{u})$.  In particular, when $\O$ is bounded there is no difference between $p$-sprouts and $p$-shortfall pairs of $(\matA,\vv{u})$.
\end{remark}

\begin{definition}[The directed sprouting graph]
   Let $\matA$ be a monomial matrix.
   We define $\graph_r(\matA)$ to be the set consisting of all very small $\matA$-pairs, and once $\graph_q(\matA)$ has been defined for some $q$ a power of $r$, we then define $\graph_{qr}(\matA)$ to be the set consisting of all $r$-sprouts of all pairs in $\graph_q(\matA)$.

   We define the vertices of the graph $\sprout_r(\matA)$ to be the disjoint union
   \[ \bigsqcup_{e=1}^{\infty} \graph_{r^e}(\matA). \]
   A vertex in the $r^e$-th component of the above disjoint union is said to be of \emph{level} $r^e$.
   The directed edges in $\sprout_r(\matA)$ are defined only between points of consecutive levels:
   There is a directed edge from a vertex of level $q$ to one of level $qr$ if the latter is a $r$-sprout of the former.
\end{definition}
...
\item \label{elementary1 small pairs} \Cref{r-sprout: R} tells us that every vertex of $\sprout_r(\matA)$ is a small monomial pair.  Furthermore, if $(\matB,\vv{v})$ is a vertex of $\sprout_r(\matA)$, then $\matB$ must be among the finitely many descendants of $\matA$.
...
\begin{definition}%[Numbers associated to the levels of $\sprout^*_p(\matA,\vv{u})$]
   ...
   \begin{align*}
     \epsilon_{r^e}(\matA,\vv{u}) & = \degree(\matB,\vv{v}) \\
     \delta_{r^e}(\matA,\vv{u}) & = \udeficit(\matB,\vv{v},r)
   \end{align*}
   where $(\matB,\vv{v})$ is any pair in $\S^{\ast}_{r^e}(\matA,\vv{u})$.
   Observe that, by definition of $\S^{\ast}_{r^e}(\matA,\vv{u})$, these numbers are independent of the choice of $(\matB,\vv{v}) \in \S^{\ast}_{r^e}(\matA,\vv{u})$.
\end{definition}
...
\label{depth-of-graph: D}
   If $(\matA,\vv{u})$ is very small, then $\mathcal{Q}_r(\matA,\vv{u})$ denotes the largest level of any vertex in the graph $\sprout_r^{\ast}(\matA,\vv{u})$, and $\mathcal{E}_r(\matA, \vv{u})$ is the exponent $e$ such that $\mathcal{Q}_p(A, \vv{u}) = r^{e}$.  Formally,
   \begin{align*}
      \mathcal{Q}_r(\matA,\vv{u}) & = \inf \{ q :  \S^{\ast}_{qr}(\matA,\vv{u}) = \emptyset \} \\       
      \mathcal{E}_r(\matA,\vv{u}) & = \log_r \mathcal{Q}_r(\matA, \vv{u})
   \end{align*}
   where we adopt the convention that $\log_r (\infty) = \infty$.
\end{definition}
...
Furthermore, \Cref{elementary2: R}\eqref{elementary2 periodic in e} implies that one can determine whether $\mathcal{E}_p(\matA, \vv{u}) = \Q_p(\matA,\vv{u}) = \infty$, as in \Cref{ex: new example}, by computing only finitely many levels of $\sprout_p^*(\matA,\vv{u})$.
Finally, \Cref{elementary2: R}\eqref{elementary2 periodic in r} implies that $\mathcal{E}_p(\matA,\vv{u}) = \log_p \Q_p(\matA,\vv{u})$ depends only on $(\matA,\vv{u})$, and on the class of $p$ modulo any universal denominator for $\matA$, but not on $p$ itself.
...
\begin{proof}
   Choose $p \gg 0$ so that the conclusions of \Cref{ILL: T,small not very small value: T,arithmetic uniform value and image: T,lem: upper bound for higher mu} are valid, and also so that $p$ does not divide $d$.  Set $r = \lpr{p}{d}$, which is a unit in $\ZZ/d\ZZ$, and also set $\Q = \Q_p(\matA,\vv{u})$ and $\mathcal{E} = \mathcal{E}_p(\matA, \vv{u})$.  
...
\]
   ...
   We now use this expression to define our sequences:  As recorded in \Cref{curly-Q-E-basic-facts: R}, the value of $\mathcal{E}$ depends only on $r$, but not $p$, and \Cref{elementary2: R}\eqref{elementary2 periodic in e} implies that the same is true for the boxed coefficients above.  Thus, if we define 
   \begin{align*}
...
%         & = \Big ( \sum_{i=1}^{\mathcal{E}} (\epsilon_{p^i}(\matA,\vv{u})p-\delta_{p^i}(\matA,\vv{u})) p^{\mathcal{E}-i} \Big ) p^e + p^e-1 \\
   ...
   Comparing this with \Cref{general AIP: T}, we see that inequality above must be an equality.  Then, invoking the description of $\val \IP_p(\matA, \vv{u}, p^{\mathcal{E}})$ given in \eqref{val-up-to-curly-e-via-two-sequences: e}, we conclude that if $\mathcal{E} \neq \infty$ and $e \geq 1$, then  $\val \IP_p(\matA, \vv{u}, p^{\mathcal{E}+e})$ coincides with
   \[
   \degree(\matA, \vv{u}) \cdot p^{\mathcal{E}+e} - \sum_{1 \leq i < \mathcal{E}} \lambda_{r^i}(\matA, \vv{u}) \cdot p^{\mathcal{E} + e -i} - \boxed{{\eta_{r^{\mathcal{E}}}(\matA, \vv{u})}-1} \cdot p^e  - \boxed{1}.  
	\]
...
\delta_{r^s}(\matA, \vv{u}) & \hspace{61pt} \text{if }1 \leq s \leq \mathcal{E}_r(\matA, \vv{u}) \\ 
1 & \hspace{61pt} \text{if } s >  \mathcal{E}_r(\matA, \vv{u}) \\ 
\end{cases}
\end{align*}
...
\begin{proof} \daniel[inline]{Update proof}
   Fix $(\matA,\vv{u})$ and $r$ as above, and set $\mathcal{E}=\mathcal{E}_r(\matA,\vv{u})$ and $\Q = \Q_r(\matA,\vv{u})$.  
   
   By definition, every $\delta_{r^s}(\matA, \vv{u})$ and $\delta_{r^s}(\matA, \vv{u})$ is rational, and thus, so is each term $\eta_{r^s}(\matA, \vv{u})$ and $\lambda_{r^s}(\matA, \vv{u})$.  When $\mathcal{E} \neq \infty$,  each such term is eventually constant, and when $\mathcal{E} = \infty$,  the eventually periodicity of the graph $\sprout_r^{\ast}(\matA, \vv{u})$ described by \Cref{elementary2: R}\eqref{elementary2 periodic in e} implies that each such term is eventually periodic in $s$.  
   
Furthermore, each $\eta_{r^s}(\matA, \vv{u})$ is either $1$, or equal to $\delta_{r^s}(\matA, \vv{u})$, which is a universal deficit, and hence, must be positive.  Thus, to conclude, it remains to prove that each $\lambda_{r^s}(\matA, \vv{u})$ is nonnegative whenever $\mathcal{E} \neq \infty$ and $1 \leq s \leq \mathcal{E}$.

We first consider the situation when $\mathcal{E} \neq \infty$ and $1 \leq s < \mathcal{E}$.   In this case, by definition of $\mathcal{E}$, both $\S_{r^{s}}^{\ast}(\matA, \vv{u})$ and $\S_{r^{s+1}}^{\ast}(\matA, \vv{u})$ are non-empty, and hence, there exists a pair $(\matC, \vv{z})$ in the latter set that is a $r$-sprout of a pair $(\matB, \vv{v})$ in the former.  In this case, \Cref{fundamental-sequences: D} then tells us that 
...
Finally, we consider the case that $s = \mathcal{E} \neq \infty$.  In this case,  $\S^{\ast}_{r^{\mathcal{E}+1}}(\matA,\vv{u})$ is empty, and so there is an edge $(\matB,\vv{v}) \to (\matC,\vv{z})$ in $\sprout_p(\matA)$ whose initial pair lies in $\S^{\ast}_{p^{\mathcal{E}}}(\matA,\vv{u})$, and whose terminal pair is not very small. Thus, 
\[ \lambda_{r^{\mathcal{E}}}(\matA, \vv{u}) = \delta_{r^{\mathcal{E}}}(\matA, \vv{u}) - 1 = \udeficit(\matB, \vv{v}, r) - 1 \geq \deg(\matC, \vv{z}) - 1   \] where the last inequality follows from \Cref{fundamental-sequences: L}.   However, as $(\matC, \vv{z})$ is not very small, the right-most term above must be positive.
\end{proof}

We now focus on determining the values of the terms $\lambda_{r^s}(\matA, \vv{u})$ and $ \eta_{r^s}(\matA, \vv{u})$ in some important special cases.  
...
\begin{proposition}  If $\matA$ is \hl{convenient and planar}, then there exists $\beta = \beta(\matA)$ with the following property:  If $(\matA, \vv{u})$ is very small, and $p>\beta$, then the value of $\fip_p(\matA, \vv{u})$ either coincides with $\deg(\matA, \vv{u})$, which occurs if and only if $\mathcal{E}_p(\matA, \vv{u})$ is infinite, or is rational number with denominator $\Q_p(\matA, \vv{u})$, a power of $p$.

\begin{proof}  Let $\beta = \beta(\matA)$ be as in \Cref{main theorem wrt diagonal: T}.  Fix $p$ greater than $\beta$, a very small pair  $(\matA, \vv{u})$, and set $\mathcal{E} = \mathcal{E}_p(\matA, \vv{u})$ and $\Q = \Q_p(\matA, \vv{u})$.  We will prove below that
\begin{equation}
\label{delta-epsilon-for-convenient-planar-pairs: e}
\delta_{p^e}(\matA, \vv{u}) = \epsilon_{p^{e+1}}(\matA, \vv{u}) \text{ for every }1 \leq e < \mathcal{E}.
\end{equation}
...
\begin{proposition}
   ...
   \begin{equation}
   \ideala^{[t]} = \ideal{x^{\vv{u}-\vv{1}} : \vv{u} \in \setW\text{ and }\crit(\ideala, \ideald_\vv{u}) > t}\label{eq: description of frobenius powers in terms of crits}   
\end{equation}
where $\ideald_\vv{u}$ denotes the diagonal ideal associated to the positive point $\vv{u}$.
\end{proposition}

\begin{proof}
   If $\vv{u} \in \setW$ and $\crit(\ideala, \ideald_\vv{u}) > t$, then $\ideala^{[t]}\not\subseteq \ideald_\vv{u}$, so there exists a monomial $x^{\vv{v}} \in \ideala^{[t]}$ with $\vv{v} < \vv{u}$.
   Equivalently, $\vv{v} \leq \vv{u}-\vv{1}$, hence $x^{\vv{u}-\vv{1}}\in \ideal{x^{\vv{v}}}\subseteq \ideala^{[t]}$.
   
   Conversely, let $x^{\vv{v}}$ be a minimal generator of $\ideala^{[t]}$.
   Then $\crit(\ideala,\ideald_{\vv{v}+\vv{1}}) > t$, as $x^{\vv{v}}\notin \ideald_{\vv{v}+\vv{1}}$.
   Moreover, as $\ideala^{[t]}$ contains $\ideala^{[1]} = \ideala$, the point $\vv{v}$ cannot lie in the interior of the Newton polyhedron of $\matA$, so the pair $(\matA,\vv{v})$ must be very small.
   If $(\matA,\vv{v}+\vv{1})$ is small, then $\crit(\ideala, \ideald_{\vv{v}+\vv{1}}) \le 1$, and \Cref{finite-representatives-small-crits: P} shows that there exists $\vv{u} \in \setW$ with $\vv{u} \le \vv{v} +\vv{1}$ with $\crit(\ideala,\ideald_{\vv{u}}) = \crit(\ideala, \ideald_{\vv{v}+\vv{1}}) > t$.
   Since $\vv{v} \ge \vv{u} - \vv{1}$, $x^\vv{v}$ lies in the right hand side of \eqref{eq: description of frobenius powers in terms of crits}.
   \todo[inline]{
      Can $(\matA,\vv{v}+\vv{1})$ fail to be small?
      If so, is there always a small-not-very small pair $(\matA,\vv{u})$ such that $\vv{u}\le \vv{v} + \vv{1}$?
   }
\end{proof}
...
\begin{example}
   ...
   \Cref{table: new homogeneous example} shows the Frobenius powers $\ideala^{[t]}$ for $t \in [0,1)$, when $p$ is sufficiently large and congruent to $3$ modulo $8$.
\afterpage{%
    \clearpage% Flush earlier floats (otherwise order might not be correct)
    \thispagestyle{empty}% empty page style (?)
