...
\begin{definition}[Planar and convenient matrices]
   A monomial matrix is \emph{convenient} if it contains among is columns integer multiples of all standard basis vectors of its target lattice.
   A monomial matrix is \emph{planar} if its Newton polyhedron has only one standard face, and this face is bounded.   
\end{definition}
...
\begin{proof}  Suppose that the ambient ring of $\ideala$ involves $m$ variables.  Let $\vv{b} \in \ZZ^m$ be the positive weight vector that defines the grading on the ambient polynomial ring of $\ideala$.  By assumption, the inner product of $\vv{b}$ with each column of $\matA$ equals $d$, and if we normalize  by dividing $\vv{b}$ by $d$, we obtain a positive vector $\vv{c} \in \QQ^m$ whose inner product with every column of $\matA$ is $1$.  Furthermore, as $\ideala$ is assumed to be $\idealm$-primary, every coordinate axis of $\RR^m$ contains some column of $\matA$.   

All of this shows the Newton polyhedron $\N$ associated to $\matA$ contains a unique standard face, namely, the face $\O$ defined by $\vv{c}$, which is compact, as $\vv{c}$ is positive.  

In particular, the collapse along $\O$ is the identity.  Thus, if $(\matB, \vv{v})$ is an $r$-sprout of $(\matA, \vv{u})$, then $\matB = \matA$, and $\vv{v} \in \ushort(\matA, \vv{u}, \rr)$.  To realize $\vv{v}$,  we fix a special point $\vv{s}$ for $(\matA, \vv{u})$, and write $\vv{v} = \matA \tail{\vv{s}}_{\rr} - \matA \vv{h}$ for some $\vv{h}$ that is optimal for $\ip = \ip(\matA, \vv{u}, \vv{s}, \rr)$.   

As $\O$ is the unique standard face of $\N$,  \Cref{FT descriptions: P}\ref{new ip} that $\deg(\matA, \vv{v}) = \iprod{\vv{c}}{\vv{v}}$.  Therefore, as the inner product of $\vv{c}$ with every column of $\matA$ is $1$, 
