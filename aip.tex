...
\begin{example}
   ...
   For a perhaps simpler (but similar) example, notice that for $t>1$, the monomial pair $(A, \vv{u})$ associated to ideal pair $(\ideala, \ideald)$ is small, but not very small if $\ideala= \ideal{x^3 z, y^2z}$ and $\ideald = \ideal{x^2, y, z^t}$ are ideals of $\kk[x,y,z]$. 
\end{example}
...
\begin{example}
   ...
   Notice that in this case the minimal face $\O = \mf(A,\vv{u})$ is bounded, so that collapsing is simply the identity.
