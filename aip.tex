...
%      \frac{624}{773}p^{e} - \frac{1551}{681013}p^{e-1} - \frac{2794}{22025}p^{e-3} - \frac{\lpr{21+10e}{25}}{25}
%   \]
...
%     \]
%     for each sufficiently large prime $p$ congruent to $11$ modulo $2170560$.
% \end{example}
...
\end{cases}
\]
and also, that these formulas are swapped after replacing $\vv{u}$ with $\vv{v}$, i.e., 
\[
...
Recall, from \hl{the introduction}, that a pair of ideals $(\ideala, \idealb)$ of $\LL[x]$ is called \emph{radically compatible} if $\ideala$ is nonzero, $\idealb$ is proper, and $\ideala$ contained in the radical of $\idealb$.  Our purpose in this appendix is two-fold:  First, we show that if $(\ideala, \idealb)$ is a radically compatible pair of ideals of $\kk[x]$, where $\kk$ is a field of positive characteristic $p$, then when determining the values of $\mu(\ideala, \idealb, p^e)$, one may always assume that $\idealb$ is the diagonal ideal associated to a positive point.   This is, in essence, the content of \Cref{reduce-to-monomial-case: T}.  Second, we show that a similar statement holds in the context of \emph{reduction to large prime characteristic}, which is the content of \Cref{invariants associated to a monomial ideal in large characteristic: C}.   Let us also note that none of the results here rely on any of the in-depth examination of arithmetic integer programs and fractal programs conducted in the main text. 

Our approach is based on framework established in \cite[Appendix]{budur+mustata+saito.roots_bs_polys_monomial}.  For example, the \emph{monomialization operator} defined in \Cref{monomialization: D} plays an important role in our arguments, and though this operator was not formally defined in \emph{loc.\ cit.}, it, and a natural variant, appear in \cite[Proposition 6.1 and Lemma 6.2]{budur+mustata+saito.roots_bs_polys_monomial}
...
\label{single multiplier: L}
  Consider a Noetherian domain $\LL$.  Given an ideal $I$ of $\LL[x]$, there exists a single nonzero element $\tau \in \LL$ with which to test inclusion in $\mon(I)$, i.e., such that $x^{\vv{u}} \in \mon(I)$ if and only if $\tau x^{\vv{u}} \in I$ for every monomial $x^{\vv{u}} \in \LL[x]$.
\end{lemma}
...
We also provide examples illustrating that the uniformity properties established here for the $\mu$-invariants attached to a monomial ideal are strictly stronger than the ones for their $\nu$-counterparts established in \emph{loc.\ cit.}, both over a field of fixed positive characteristic, and when reducing to positive characteristic.  

\subsection{Concerning polynomial rings over fields of positive characteristic}

In what follows, we extend our use of the term \emph{radically compatible}, and say that a pair $(A, \mathcal{C})$ consisting of a monomial matrix $A$ with $m$ rows, and a collection $\mathcal{C}$ of monomials in the ambient variables $x = x_1, \ldots, x_m$, is \emph{radically compatible} if for some (equivalently, for every) field $\LL$, the pair of ideals $(\ideala, \idealc)$ is radically compatible,  where $\ideala$ is the monomial ideal of $\LL[x]$ associated to $A$, and $\idealc = \mathcal{C} \LL[x]$.


\begin{theorem}
\label{reduce to diagonal general: T}
  Consider a radically compatible pair $(A, \mathcal{C})$, where $A$ is a monomial matrix with $m$ rows, and $\mathcal{C}$ is a collection of monomials in the ambient variables $x = x_1, \ldots, x_m$.    Then, there exists a finite, nonempty set $\mathcal{W}$ of positive points in $\ZZ^m$ with the following property:  If $\kk$ is a field of characteristic $p>0$, and $(\ideala, \idealc)$ is the radically compatible pair of ideals of $\kk[x]$ associated to $(A, \mathcal{C})$, then 
\[ \mu(\ideala, \idealc, p^e) = \max \{ \val \IP_p(A, \vv{u}, p^e) : \vv{u} \in \mathcal{W} \}\]  and there is a point $\vv{u} \in \mathcal{W}$ that achieves this maximum for every positive integer $e$.
\end{theorem}

\begin{proof}  We will start by considering the special case that $\mathcal{C}$ is corresponds a nonzero point $\vv{v} \in \NN^\numvars$, in the sense that $\mathcal{C} = \{ x_i^{v_i} : v_i \neq 0 \}$. Without loss of generality, we may assume that exactly the first $1 \leq t \leq m$ coordinates of the point $\vv{v}$ are nonzero, so that $\mathcal{C} = \{x_1^{v_1}, \ldots, x_t^{v_t}\}$, and the point $\widetilde{\vv{v}} = (v_1, \ldots, v_t) \in \NN^t$ is positive.  
...
Therefore, the feasible set of the standard linear program $\LP(B, \widetilde{\vv{v}})$ is compact, and so we may fix a positive integer $\omega$ greater than every inner product of the form $\iprod{\vv{r}}{\vv{f}}$, where $\vv{r}$ ranges over the last $m-t$ rows of $A$, and $\vv{f}$ ranges over all points in $\feas \LP(B,\vv{w})$. Let $\vv{u} \in \NN^{\numvars}$ be any point such that $u_i = v_i$ for $1 \leq i \leq t$, and $u_i \geq \omega$ for $t < i \leq \numvars$, and let $\mathcal{W}$ be the set consisting of the single positive point $\vv{u}$.

Now, specialize to the case that $\LL=\kk$ is a field of characteristic $p>0$.  To conclude in this special case, we must show that $\mu(\ideala, \idealc, p^e) = \val \IP_p(A, \vv{u}, p^e)$.  However, it is clear that $\mu(\ideala, \idealc, p^e) = \val \IP_p(B, \widetilde{\vv{v}}, p^e)$, and to justify that the values of $\IP_p(B, \widetilde{\vv{v}}, p^e)$ and $\IP_p(A, \vv{u}, p^e)$ agree, we will show that their feasible sets coincide:   First, note that the arithmetic constraints of these programs are identical, and that each linear constraint of $\IP_p(B, \vv{w}, p^e)$ is a linear constraint of $\IP_p(A, \vv{u},p^e)$, and so $\feas \IP_p(A, \vv{u}, p^e)$ is at least contained in $\feas \IP_p(B, \vv{w}, p^e)$.  On the other hand, if $\vv{k} \in \feas \IP_p(B, \vv{w}, p^e)$, then the normalized point $\vv{k}/p^e$ is feasible for the linear program $\LP(B,\vv{w})$,   and the bounds $u_i \geq \omega$ for $t < i \leq \numvars$ then guarantee $A (\vv{k}/p^e) < \vv{u}$, which in turn demonstrates that $\vv{k} \in \feas \IP_p(A, \vv{u}, p^e)$.  
...
By the special case considered above, there is a singleton $\mathcal{W}_i = \{ \vv{u}_i \}$ such that, when $\LL=\kk$ is a field of characteristic $p>0$, then $\mu(\ideala, \idealc_i, p^e) = \val \IP_p(A, \vv{u}_i, p^e)$ for every positive integer $e$, where $\idealc_i = \mathcal{C}_i \LL[x]$.    It follows from this, \eqref{irreducible-decomposition: e}, and the fact that standard Frobenius powers commute with intersection in $\kk[x]$,  that 
\[ \mu(\ideala, \mathfrak{c}, p^e) = \max \{ \mu(\ideala, \idealc_i, p^e) : 1 \leq i \leq \ell \} = \max \{ \val \IP_p(A, \vv{u}, p^e) : \vv{u} \in \mathcal{W} \} \] where $\mathcal{W} = \{ \vv{u}_1, \ldots, \vv{u}_{\ell} \}$.  Finally, though the points that achieve the maximum may vary with $e$, \Cref{cor: mu comparison} guarantees that there exists a single $\vv{u} \in \mathcal{W}$ such that
   \[\mu(\ideala, \mathfrak{c}, p^e) =  \val \IP_p(A, \vv{u}_i, p^e)\text{ for every $e\in \NN$} \qedhere \]
\end{proof}
...
\begin{theorem}[Reduction to diagonal ideals]
   \label{reduce-to-monomial-case: T} Given a {radically compatible} pair of ideals $(\ideala, \idealb)$ of $\kk[x]$, with $\kk$ a field of characteristic $p>0$, and $\ideala$ the monomial ideal associated to a monomial matrix $A$, there exists a positive point $\vv{u} \in \ZZ^\numvars$ such that \[ \mu(\ideala, \idealb, p^e) = \mu(\ideala, \mon(\idealb), p^e) = \val \IP_p(A,\vv{u}, p^e)\] for every natural number $e$.
   In particular, $\crit(\ideala,\idealb) = \crit(\ideala, \mon(\idealb)) = \val \fip_p(A,\vv{u})$.
\end{theorem}

\begin{proof}
   Set $\idealc \coloneqq \mon(\idealb)$.  Since $\ideala$ is monomial, a Frobenius power of $\ideala$ sits in~$\idealb^{[p^e]}$ if and only if it sits in $\mon(\idealb^{[p^e]})$, which equals $\idealc^{[p^e]}$ by \Cref{frobenius-powers+mon-commute: L}.  Consequently, $\mu(\ideala, \idealb, p^e) = \mu(\ideala, \idealc, p^e)$ for every natural number $e$.  Finally, if we fix a set of monomials $\mathcal{C}$ in $x = x_1, \ldots, x_\numvars$ such that $\mathcal{C} \kk[x] = \idealc$, then invoking \Cref{reduce to diagonal general: T} for the radically compatible pair $(A, \mathcal{C})$ guarantees that there exists a positive point $\vv{u} \in \ZZ^\numvars$ such that $\mu(\ideala, \idealc, p^e) = \val \IP_p(A, \vv{u}, p^e)$ for every natural number $e$.
\end{proof}

\daniel{\emph{analogous} $\leftrightarrow$ \emph{similar}, and expanded.  \Cref{less uniformity for nu: E} is new.}
A result similar to \Cref{reduce-to-monomial-case: T}, but concerning $\nu(\ideala,\idealb,p^e)$, was established in \cite[Section~4]{budur+mustata+saito.roots_bs_polys_monomial}.  Let us summarize it:  If $(\ideala, \idealb)$ and $A$ are as in \Cref{reduce-to-monomial-case: T}, and $e$ is positive, then there is a positive $\vv{u}$ such that $\nu(\ideala, \idealb, p^e) = \val \IP(A, \vv{u}, p^e)$. 

In contrast with \Cref{reduce-to-monomial-case: T}, this allows for the possibility that $\vv{u}$ may also depend on $e$, and in fact, as we see in \Cref{less uniformity for nu: E}, it necessarily does.  Thus, in this sense, the $\mu$-variants enjoy stronger uniformity properties than the $\nu$-invariants.


\begin{example} 
\label{less uniformity for nu: E}
Let $\kk$ be a field of characteristic $p \equiv 3 \bmod 4$, and consider the ideals $\ideala = \langle x^4, y^4 \rangle$ and $\idealb = \mon(\idealb) = \ideal{x^5, x^3 y, y^3} = \ideal{x^3, y^3} \cap \ideal{x^5, y}$.  Then 
\[ \nu(\ideala, \idealb, p^e) = \max \{ \val \IP(A,\vv{u}, p^e), \val \IP(A, \vv{v}, p^e) \} \] where $A$ is the $2 \times 2$ diagonal matrix with $4$ in each diagonal entry,  $\vv{u}=(3,3)$ and $\vv{v}=(5,1)$, and \Cref{max depends on p and e: E} tells us both that this maximum value equals $(3p^e-1)/2$, and that the integer program achieving this maximum depends on $e$.

Furthermore, it is not hard to see that there is no positive point $\vv{w} \in \NN^2$ such that $\val \IP(A, \vv{w}, p^e) = (3p^e-1)/2 = \nu(\ideala, \idealb, p^e)$ for every $e$, at least when $p \equiv 3 \bmod 4$.  Indeed, \Cref{max depends on p and e: E} implies that an equality $\val \IP(A, \vv{w}, p^e) = (3p^e-1)/2$ for all $e$ would force an equality $\ft{A}{\vv{w}} = 3/2$, and so $\vv{w}$ would have to be one $(1,5), (2,4), (3,3), (2,4)$, or $(1,5)$.  However, as $p \equiv 3 \bmod 4$,  \Cref{max depends on p and e: E} would then tell us that, in each case, $\val \IP(A, \vv{w}, p^e) = (3p^e-3)/2$ for infinitely many $e$.
\end{example}
...
We now briefly recall the process of reduction to prime characteristic, at least in the context of polynomial rings.
...
\begin{remark}[Maximal ideals of domains finitely generated over a copy of $\ZZ$]
...
Consider a radically compatible pair of ideals $(I,J)$ of $R=\kk[x]$, where $\kk$ be a field of characteristic zero, and $I$ is the monomial associated to a monomial matrix $M$.  Then, {there exists a finite, nonempty set $\mathcal{W}$ of positive points in $\ZZ^m$, and a function $(A, I_A, J_A) \mapsto \mathcal{U}$} that associates to every finitely generated $\ZZ$-subalgebra $A$ of $\kk$ supporting models $I_A$ of $I$, and $J_A$ of $J$, a dense open set $\mathcal{U} \subseteq \mspec(A)$, with the following properties \textup:
...
\begin{proof}  Fix a collection of monomials $\mathcal{C}$ in $x=x_1, \ldots, x_m$ such that $\mon(J) = \mathcal{C} \kk[x]$.  As $I$ is the monomial ideal associated to $M$, and $(I,J)$ is radically compatible, the pair $(M, \mathcal{C})$ is also radically compatible.   Thus, we may fix a nonempty set of positive points $\mathcal{W} \subseteq \ZZ^\numvars$ satisfying the conclusions of \Cref{reduce to diagonal general: T}, relative to $(M, \mathcal{C})$.  
...
\label{all of mspec: R}
 It follows from the proof of \Cref{invariants associated to a monomial ideal in large characteristic: C}, if \emph{both} $I$ and $J$ are monomial idals, generated by collections of monomials $\mathcal{G}$ and $\mathcal{H}$ in the ambient variables $x = x_1, \ldots, x_m$, respectively, and if the canonical models $I_A = \mathcal{G} R_A$ and $J_A = \mathcal{H} R_A$ are chosen, then one may take the associated set $\mathcal{U}$ to be all of $\mspec(A)$.
\end{remark}
