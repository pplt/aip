...
% The finiteness of $\fsr$,  and \Cref{positive-polynomial: L} below\emily{Is the lemma after theorem to get to theorem statement faster?}, imply that there exists an integer $\beta = \beta(\denom, \fsr)$ for which \eqref{p-big-1} and \eqref{p-big-2} hold whenever $p > \beta$.  In what follows, we assume that $p$ is chosen so that these conditions are satisfied.
    ...
    Consider a finite path \[ (A_1, \vv{u}_1) \to (A_2, \vv{u}_2) \to \cdots \to (A_e, \vv{u}_e) \] in $\graph_p(A)$.  For every $1 \leq i \leq e$, set $\O_i = \mf(A, \vv{u}_i)$, and fix a special point $\vv{s}_i \in \sp(A_i, \vv{u}_i)$ with denominator $\denom$.  If $1 \leq i < e$, then the sprouting $(A_i, \vv{u}_i) \to (A_{i+1}, \vv{u}_{i+1})$ tells us that $A_{i+1}$ is the collapse of $A_i$ along $\O_i$, and that
...
that
    ...
    We will now induce on $e$ to prove that $\sum_{i=1}^e \vv{k}_i p^{e-i}$ is feasible for $\IP(A_1, \vv{u}_1, p^e)$.
    ...
    To prove the above inequality, we use the technique outlined in \Cref{rmk: verifying inequalities using collapses}.
    ...
    To prove that \eqref{target-inequality: e} holds in $\rs(\O_1)$, we verify that the stronger inequality
