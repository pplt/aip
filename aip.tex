...
\item[$\Box$] Put in examples.
 ...
 \pedro[inline]{I started changing to $\IP(A,\vv{u},q)$. Are we going to abbreviate $\IP(A,\vv{u},1)$ by $\IP(A,\vv{u})$? Or insist on always having a $q$, so, for example, the statement of Lemma~7.3 would become ``[\ldots] the quotient when dividing any optimal point of $\IP_p(A, \vv{u}, qp^e)$ by $p^e$ must be optimal for $\IP_p(A, \vv{u}, q)$''?
...
\emily[inline]{I like it how it is.  Is ``euclidean'' typically capitalized?}
\pedro[inline]{Google Ngram shows that ``Euclidean'' beats ``euclidean'' by far, so I made the change.}
...
The \daniel{I added the adjective \emph{convex} here, since the cone generated by a set could also mean all nonnegative multiplies of elements of that set}\emph{convex cone generated by points $\vv{u}_1,\ldots,\vv{u}_k \in \RR^n$}, denoted $\cone(\vv{u}_1,\ldots,\vv{u}_k)$, is the set consisting of all points of the form $\sum_{i=1}^k \lambda_i \vv{u}_i$, where the $\lambda_i$ are nonnegative real numbers.
...
\pedro{Is there a more explicit/concrete characterization for polyhedra/polytopes?}
...
% \daniel[inline]{I prefer this proof.  Shorter, more direct.  We should replace the current proof with this one.  Another benefit of this is that we would no longer need to mention the Minkowski-Weyl Theorem (I didn't know it had a name!) since we would no longer need to make serious use of the recession cone.}
% }
...
\pedro[inline]{
   The terminology introduced in the above paragraph is not being used.
...
\pedro[inline]{
   Maybe include a subsection on multinomial coefficients, including Dickson's Theorem?
\daniel[inline]{This is OK with me, but we will need to update \Cref{aip: D} accordingly.}
...
\begin{notation}
   ...
   \[ \diag(\vv{u}) = \ideal{x_1^{u_1},\ldots,x_d^{u_d}} = \langle x^{\vv{v}} : \vv{v} \in \NN^d \text{ and } \vv{v} \not < \vv{u} \rangle.\] \daniel{I added another easy description that I think we use often}
...
\begin{proof}
   ...
   The final conclusion follows from the fact that $\ideala^{[t]}$ is a monomial ideal, as noted in \Cref{rmk: Frobenius powers of monomial ideals are monomial ideals}.
\end{proof}
...
\pedro[inline]{
   I think we need discreteness of crits here.
   If it weren't for discreteness, $\bigcap_{t<\lambda} \ideala^{[t]}$ could equal $\ideala^{[\lambda]}$.
}
\daniel[inline]{
   Nice catch.  It looks like we do need this.  I think probably can just state that critical exponents are discrete, and just cite our paper, in the proof.
}
...
%Throughout this article, we will compare linear and integer programs with analogous systems of constraints defined by $A$ and $\collapse{A}$ (e.g., $\LP(A, \vv{u})$ and $\LP(\collapse{A}, \collapse{\vv{u}})$, and $\IP(A, \vv{u}, q)$ and $\IP(\collapse{A}, \collapse{\vv{u}}, q)$).  
%
...
\begin{proof}
   ...
   If $\O = \mf(A, \vv{u})$ is bounded, then $\sp(A, \vv{u}) = \opt \LP(A, \vv{u})$ by \Cref{opt versus mc: P}, and so there is always a vertex in this set with denominator~$\ell_{\circ}$.
...
\pedro[inline]{
   I like that idea.
   Examples where the optimal sets are different are not hard to find (that is, they are not very rare).
   Below are pictures of a couple of examples.
   In the first, $A=\left(\begin{smallmatrix}9&4&2\\ 0&5&0\\ 7&3&7\end{smallmatrix}\right)$; in the second, $A=\left(\begin{smallmatrix}8&4&4\\ 6&4&9\\ 7&9&4\end{smallmatrix}\right)$; $\vv{u} = (1,1,1)$ for both.
   The feasible set for $\LP(A, \vv{u})$ is shown in blue, and is properly contained in the feasible set for $\LP(\collapse{A}, \collapse{\vv{u}})$, shown in yellow.
   The optimal set for $\LP(A, \vv{u})$ is shown in green, and is properly contained in the feasible set for $\LP(\collapse{A}, \collapse{\vv{u}})$, which also includes the red line segment.
   These examples are nice, because we can also show the pictures of the Newton polyhedra.
}

\includegraphics[width=.45\textwidth]{Pictures/opt_for_collapse_may_change.pdf}
\includegraphics[width=.45\textwidth]{Pictures/opt_for_collapse_may_change_2.pdf}

\newpage
\section{An auxiliary integer program}

\begin{definition}
   Given a monomial pair $(A,\vv{u})$ and a nonnegative integer $q$, $\IP(A, \vv{u}, q)$ is the integer program in $\ZZ^n$ defined as follows:
\begin{enumerate}
...
% \item Step 4:  Establish that all of the interesting objects that appear in this process do not depend on the choice of special point we picked in Step 2.  This is kind of technical (I bet there is a cleaner way to see this), and is done in \Cref{useful-invariants: ss}.
% \end{enumerate}
% }
...
\begin{proof}
Let $\beta$ be as in \Cref{uniform value: P}.  Fix a pair $(A, \vv{u})$ with $\O = \mf(A, \vv{u})$, a point $\vv{s} \in \sp_{\QQ}(A, \vv{u})$ with denominator $D$, and an integer $q > \beta D$.  Let $\collapse{A}$ and $\collapse{X}$ be as above.
...
\label{uniform periodicity: R}
 ...
 Indeed,  this follows from the observation that if $\denom$ is as in \Cref{uniform denominators for mc:  T}, then we may compute  $\delta(A, \vv{u}, q)$ and $\Delta(A, \vv{u}, q)$ for all pairs $(A, \vv{u})$ and integers $q>0$ in terms of a point in $\sp_{\QQ}(A, \vv{u})$ with denominator $\denom$.
\end{remark}
...
\begin{proof}  
\Cref{uniform denominators for mc:  T}  tells us that once $A$ has been fixed, there exists a positive integer $D$ such that for every pair $(A, \vv{u})$, there exists a point in $\sp_{\QQ}(A, \vv{u})$ with denominator $D$.  Therefore, if $\beta_{\circ}$  is any integer satisfying the condition stated in \Cref{uniform value and image: C}, then we may take $\beta = D \beta_{\circ}$.  
\end{proof}
...
\begin{definition}
   The \emph{Sierpi\'nski $p$-gasket} in $\RRnn^n$ is the set $\sierp_p$
   ...
   \pedro{This seemed like a clean way to extend the definition to $\RRnn^n$, but if anyone has a better idea, feel free to modify it.}
 \[
\vv{t} = q\cdot\Big(\frac{\vv{t}_1}{p} +\frac{\vv{t}_2}{p^2}+\cdots +\frac{\vv{t}_e}{p^e} + \cdots \Big).  
 \]
\end{definition}

From the definition, we immediately see that a point in $\RRnn^n$ is in $\sierp_p$ if the unique nonterminating base $p$ expansions of its coordinates \emph{add without carrying}.
However, this is not a complete description of $\sierp_p$.
...
For instance, the points of $\sierp_2$ in $[0,1]^2$ form the familiar Sierpi\'nski triangle:   
...
\begin{definition}
Given a monomial pair $(A, \vv{u})$, the optimization problem $\fip_p(A,\vv{u})$ consists of maximizing $\vv{t}\mapsto \norm{\vv{t}}$, subject to the constraints $\vv{t} \geq \vv{0}$, $A\vv{t} < \vv{u}$, and $\vv{t}\in \sierp_p$.  The value of the problem, $\val \fip_p(A,\vv{u})$, is defined as the supremum of $\norm{\vv{t}}$ among all $\vv{t} \in \feas \fip_p(A, \vv{u})$.  
\end{definition}

\begin{remark}
The feasible set of $\Sigma_p(A, \vv{u})$ sits inside the intersection of the feasible set of the linear program $\LP(A, \vv{u})$ with the Sierpi\'nski $p$-gasket.  
Since this intersection is a compact set and objective function is continuous, the value of $\Sigma_p(A, \vv{u})$ is finite.
\end{remark}

\emily[inline]{Still need to justify that the closure of $\feas\Sigma^\circ_p(A, \vv{u})$ is $\feas\Sigma_p(A, \vv{u})$.}
\pedro[inline]{
   Unfortunately, I think this is not true.
   Consider, for example, $A=\big(\begin{smallmatrix}2&0\\ 0&2\end{smallmatrix}\big)$, $\vv{u}=(1,1)$, and $p=2$.
   Then $(\frac12,\frac12)$ is in $\feas\Sigma_p(A, \vv{u})$, but is not in the closure of $\feas\Sigma^\circ_p(A, \vv{u})$, which consists of points of norm $\le \frac12$.
   So maybe the best we can hope for is to work directly with $\Sigma^\circ_p(A,\vv{u})$.
}
\emily[inline]{Thanks, Pedro.  It's like you exactly read our minds--we came up with the \emph{exact same} example yesterday on the bus after I got stuck on the proof!  We also concluded that it would be best to describe everything in terms of $\Sigma^\circ$.  I've started this}
\pedro[inline]{
   ``Great minds think alike''?   :-)

   Maybe what we should do is define the feasible set for $\fip_p(A,\vv{u})$ to be the \emph{closure} of the set of all $\vv{t}\in \sierp_p$ satisfying $A\vv{t} < \vv{u}$.
   This way we can talk about value in the usual way, and can also talk about optimal points.
   We can also include a remark saying that that is not as simple as taking the set of all $\vv{t}\in \sierp_p$ satisfying $A\vv{t} \le \vv{u}$ (though these sets often agree), and present our example.
}
...
\pedro[inline]{For this to be true, I think we need to extend $\sierp_p$ to $\RR^n$, or assume that $(A,\vv{u})$ is small}
\emily[inline]{I see your point. It seems that if we extend it to $\RR^n_{\geq 0}$, to get the reverse inclusion, we need both the integer and fraction parts to have expansions that add without carrying. }
\pedro[inline]{
   Indeed!
   That is different from the picture I had originally in mind---that of $\RRnn^n$ ``tiled'' with Sierpinski triangles, all of the same size.
   \begin{center}
      \includegraphics[width=.4\textwidth]{Pictures/sierp.pdf}
   \end{center}
   Instead, the ``holes'' get arbitrarily large:
   \begin{center}
      \includegraphics[width=.4\textwidth]{Pictures/sierp2.pdf}
   \end{center}
   Makes a lot more sense\ldots
}

\daniel[inline]{This is really cool!  We should include the gasket in the unit cube, and also the more general picture in an example.}
\pedro[inline]{
   We could include something like \Cref{fig: sierpinski 3-gasket}.
}

\begin{figure}[ht]
\begin{subfigure}{.48\textwidth}
  \centering
  \includegraphics[width=\linewidth]{Pictures/sierpinski3_a.pdf}
  \caption{Sierpi\'nski 3-gasket in $[0,1]^2$}
\end{subfigure}
\begin{subfigure}{.48\textwidth}
  \centering
  \includegraphics[width=.987\linewidth]{Pictures/sierpinski3_b.pdf}  
...
\val\fip_p(A,\vv{u}) \ge \displaystyle \lim_{q\to \infty}\frac{\val\IP_p(A,\vv{u}, q)}{q} = \crit(A,\vv{u}).
 \]
On the other hand, given $\vv{t} \in \feas\fip_p(A, \vv{u})$, fix a sequence $\{ \vv{t}_e \}_{e=1}^\infty$ in $\NN^n$ for which all $\norm{\vv{t}_e} < p$ and 
...
\crit(A,\vv{u}) = \lim_{q\to \infty} \frac{\val \Pi_p(A, \vv{u}, q)}{q} \geq \lim_{q \to \infty}   \norm{\vv{t}^\star_q} = \norm{\vv{t}}.
 \]
Finally, since $\vv{t} \in \feas \fip_p(A, \vv{u})$ is arbitrary, $\crit(A, \vv{u}) \geq \val \fip_p(A, \vv{u})$ as well, and equality holds. 
\end{proof}
...
\pedro[inline]{We could include a 2D example, with pictures, such as the following, where
   \[
      ...
      \end{pmatrix}
   \]
   (I changed the example; I think in this new one it is easier to see the optimal points.)
}

\begin{figure}[h]
  \centering
  \includegraphics[width=.48\textwidth]{Pictures/ex4_char2.pdf}\hskip .04\textwidth
  \includegraphics[width=.48\textwidth]{Pictures/ex4_char3.pdf}
  \\[.04\textwidth]
  \includegraphics[width=.48\textwidth]{Pictures/ex4_char5.pdf}\hskip .04\textwidth
  \includegraphics[width=.48\textwidth]{Pictures/ex4_char7.pdf}
\end{figure}
...
%    In the first and second picture, the optimal set is that entire line segment.
...
Moreover, pick $\vv{s} \in \sp(A,\vv{u})
...
\begin{proof}   Throughout this proof, $p$ will always be a prime integer congruent to $r$ modulo $D$.  As such, the point $\vv{s}_c \cdot p - \vv{t}_c$  has nonnegative integer coordinates for every $e \geq 1$, and the eventual periodicity of the realization sequence implies that $\vv{s}_c \cdot p - \vv{t}_c$ has nonnegative integer coordinates for all $p \gg 0$ and $e \geq 1$.   Furthermore, the fact that $\vv{s}_c$ is a special point for $(A_c, \vv{u}_c)$ tells us that the norm of $\vv{s}_c \cdot p - \vv{t}_c$ is $\ft{A_c}{\vv{u}_c} \cdot p - \norm{\vv{t}_c}$.  However, by definition of the graph $\graph_r(A, \vv{u})$, we know that the $\ft{A_c}{\vv{u}_c} \leq 1$ and that $\norm{\vv{t}_c} > 0$, and so the point above has norm less than $p$.  In summary, we have just seen that the point
...
As $\vv{s}_l$ is a special point for $(A, \vv{u}_l)$, we may write 
%
\begin{equation}
 \label{l-th term: e}
...
We begin by verifying \eqref{ineq to end proof: e} in $\rs(\O)^{\perp}$.  Let $\collapse{X}$ denote the image of a subset $X$ of $\RR^d$ under the projection $\RR^d \to \rs(\O)^{\perp}$, so that $\collapse{A \vv{t}} = B \vv{t}$ for every $\vv{t} \in \RR^n$.    By construction, $\vv{s}_1$ is a special point for $(A, \vv{u})$.  If $l > 1$, then $A \vv{s}_1 = \vv{u}$, and if $l =1$, then $A \vv{s}_1$ differs from $\vv{u}$ by an element of $\rs(\O)$.  In any case, $B \vv{s}_1 = \collapse{\vv{u}}$.   The assumption that there is an edge  between $(A, \vv{u}_l)$ and $(B, \vv{v})$ in $\graph_r(A, \vv{u})$ also tells us that $B \vv{t}_l = \vv{v}$, and combining these observations with \eqref{A telescoping: e} shows that
...
%
As $\vv{s}_1$ is a special point for $(A,\vv{u})$, we have that $A \vv{s}_1 \leq \vv{u}$, and so to conclude the proof, it suffices to show that quantity being subtracted above is positive in $\rs(\O)$ for all $p \gg 0$ and $e \geq 1$.  Equivalently, multiplying this expression by $p^{l+e}$, we have reduced to showing that 
