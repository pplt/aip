...
\daniel[inline]{If this is what we stick with, then we should definitely clarify (probably immediately after the definition) that we are omitting rows of $A$, and not replacing them with zero.  OR, we could build this into the cool new examples Pedro has written up.}
...
If $A$ is the exponent matrix of $\ideala$, then $\ideala^k$ is generated by monomials $x^{A\vv{k}}$, with $\vv{k}\in \NN^n$ and $\norm{\vv{k}} = k$.
...
\todo[inline]{
   Give algebraic interpretation of the image.
}
