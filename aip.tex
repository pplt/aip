...
\begin{lemma}
\label{easy but important: L}  Consider a list $(A, \vv{u}, \vv{s}, q)$.  If $\vv{k}$ is feasible for $\IP(A, \vv{u}, q)$, then the difference $\vv{k} - (\vv{s}q-\tail{\vv{s}}_q)$ is feasible for $\ip(A, \vv{u}, \vv{s}, q)$.
\end{lemma}
...
\emily[inline]{The statement below appears to be the ``lifting lemma'' in disguise!.  More precisely, it looks like the base case of ILL}

\daniel[inline]{This has been changed to a ``point-by-point" statement, since in its current placement, we haven't established any finiteness properties of $\ip$}
