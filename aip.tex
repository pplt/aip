...
\label{compatible to small compatible: L}
 \hl{Suppose that $(\ideala, \idealb)$ is a compatible pair of ideals in $\kk[x]$, where $\kk$ is a field of characteristic $p>0$, let $t$ be the natural number such that $\ideala^t \not \subseteq \idealb$ but $\ideala^{t+1} \subseteq \idealb$, and set $\idealc = (\idealb : \ideala^t)$.   If $p > t+1$ and $e$ is a positive integer, then}
\[ \mu(\ideala, \idealb, p^e) = t p^e + \mu(\ideala, \idealc, p^e). \] 
\end{lemma}

\begin{proof}  Set $q=p^e$ and $s=\mu(\ideala, \idealc, p^e)$.  The containment $\ideala \subseteq \idealc$ implies that $0 \leq s < q$, and combining this with the bounds that $0 < t < p-1$, we see that 
%
\begin{align*}
...
\end{align*}
%
though to establish the second identity, the case $s=q-1$ must be considered separately, and requires the bound $p>t+1$.  Thus, $\ideala^{[tq+s]} \not \subseteq \idealb^{[q]}$, for otherwise, \[ \ideala^{[s]} \subseteq (\idealb^{[q]} : (\ideala^t)^{[q]}) = \idealc^{[q]}\]  contradicting our choice of $s$.  The containment $\ideala^{[tq+s+1]} \subseteq \idealb^{[q]}$ follows similarly. 
\end{proof}.
...
\label{lifted-programs: L}
 {Suppose that $B$ is a monomial matrix obtained by deleting some rows a monomial matrix $A$ with $m$ rows.    In fact, for the sake of concreteness, suppose that $B$ consists of the first $\ell$ rows of $A$, and is monomial.   Given a positive point $\vv{v} \in \mathbb{Z}^{\ell}$, there is a positive point $\vv{u} \in \ZZ^{\numvars}$ such that $u_i = v_i$ for all $1 \leq i \leq \ell$, and such that the values of $\IP_p(A, \vv{u}, p^e)$ and $\IP_p(B, \vv{v}, p^e)$ coincide for all $p$ and $e$.}
\end{lemma}

\begin{proof}  As $B$ is monomial, the feasible set of the standard linear program $\LP(B, \vv{v})$ is compact, and so we may fix a positive integer $\omega$ greater than every inner product of the form $\iprod{\vv{r}}{\vv{f}}$, where $\vv{r}$ ranges over the last $m-\ell$ rows of $A$, and $\vv{f}$ ranges over all points in $\feas \LP(B,\vv{v})$. In what follows, let $\vv{u} \in \ZZ^{\numvars}$ be any point with $u_i = v_i$ for $1 \leq i \leq \ell$, and whose remaining $m-\ell$ coordinates are at least $\omega$. 

To conclude, we will show that the feasible sets of $\IP_p(A, \vv{u}, p^e)$ and $\IP_p(B, \vv{v}, p^e)$ coincide for all $p$ and $e$.  First, note that the arithmetic constraints of these programs are identical, and that each linear constraint of $\IP_p(B, \vv{v}, p^e)$ is a linear constraint of $\IP_p(A, \vv{u},p^e)$.  Thus, $\feas \IP_p(A, \vv{u}, p^e)$ is contained in $\feas \IP_p(B, \vv{v}, p^e)$.  On the other hand, if $\vv{k} \in \feas \IP_p(B, \vv{v}, p^e)$, then the normalized point $\vv{k}/p^e$ lies in $\feas \LP(B, \vv{v})$, and so if $\ell < i \leq m$, its inner product with the $i$-th row of $A$ is less than $\omega$, and thus, less than $u_i$, which demonstrates that $\vv{k} \in \feas \IP_p(A, \vv{u}, p^e)$.  
\end{proof}
...
\begin{example}
\label{max depends on p and e: E}  The uniform comparison property described in \Cref{cor: mu comparison} may fail if we replace the standard program $\IP$ with the arithmetic program $\IP_p$.  For instance, suppose that $A$ is the $2 \times 2$ diagonal matrix with $4$ in each diagonal entry, and let $\vv{u}_1=(3,3)$ and $\vv{u}_2=(5,1)$;  note that $\ft{A}{\vv{u}_1} = \ft{A}{\vv{u}_2} = 3/2$.

Then, if $p \equiv 3 \bmod 4$, it follows from \Cref{concrete-solution-to-LP-IP: E}  that
\[
\val\IP(A, \vv{u}_1, p^e) = \begin{cases}
\frac{3 p^e - 1}{2} & \text{if } e \text{ is odd} \\
\frac{3 p^e - 3}{2} & \text{if } e \text{ is even}
\end{cases}
\]
...
\[
\val\IP(A, \vv{u}_2, p^e) = \begin{cases}
...
\begin{remark}[$F$-thresholds of small pairs]
   ...
   {Suppose that $\vv{c} \in \RR^d$ defines $\O = \mf(A, \vv{u})$, and without loss of generality, assume that the columns of $A$ lying in $\O$ are $\vv{a}_1, \ldots, \vv{a}_{\ell}$.  Write  $\vv{u} = \vv{v} + \vv{w}$ with $\vv{v} \in \cone(\vv{a}_1, \ldots, \vv{a}_{\ell})$ and $\vv{w} \in \rs(\O)$.  By Carath\'eodory's Theorem for cones, we may express $\vv{v}$ as a conical combination of $t$ linearly independent columns among $\vv{a}_1, \ldots, \vv{a}_{\ell}$, and the smallness of $(A,\vv{u})$ implies that every coefficient in such a conical combination is at most $1$.  Given that $\iprod{\vv{c}}{\vv{a}_i} = 1$ for every $1 \leq i \leq \ell$, it follows that}
\[ \ft{A}{\vv{u}} = \iprod{\vv{c}}{\vv{u}} = \iprod{\vv{c}}{\vv{v}} \leq t \leq \min \{ d, \ell \} \leq \min \{ d, n \}.\]}
...
Throughout this appendix, we focus on polynomial rings in the string of variables variables $x = x_1, \ldots, x_\numvars$, and in an attempt to simplify notation,  given a ring $\LL$, we will {consistently} use $\LL[x]$ to denote the polynomial ring $\LL[x_1, \ldots, x_\numvars]$.

Recall, from {the introduction}, that a pair of ideals $(\ideala, \idealb)$ of $\LL[x]$ is called \emph{radically compatible}\daniel{I want to switch, and in some places have switched, \emph{radically compatible} $\to$ \emph{compatible} and \emph{compatible} $\to$ \emph{small compatible} to emphasize the connection with small monomial pairs.  I have probably failed to update this in various spots} if $\ideala$ is nonzero, $\idealb$ is proper, and $\ideala$ contained in the radical of $\idealb$.  
Our purpose in this appendix is two-fold:  First, we show that if $(\ideala, \idealb)$ is a radically compatible pair of ideals of $\kk[x]$, where $\kk$ is a field of positive characteristic $p$, then when determining the values of $\mu(\ideala, \idealb, p^e)$, one may always assume that $\idealb$ is the diagonal ideal associated to a positive point.   This is, in essence, the content of \Cref{reduce-to-monomial-case: T}.  Second, we show that a similar statement holds in the context of \emph{reduction to large prime characteristic}, which is the content of \Cref{invariants associated to a monomial ideal in large characteristic: C}.   Let us also note that none of the results here rely on any of the in-depth examination of arithmetic integer programs and fractal programs conducted in the main text. 
...
\label{single multiplier: L}
  Consider a Noetherian domain $\LL$.  Given an ideal $I$ of $\LL[x]$, there exists a nonzero element $\tau \in \LL$ with which to test inclusion in $\mon(I)$, i.e., such that $x^{\vv{u}} \in \mon(I)$ if and only if $\tau x^{\vv{u}} \in I$ for every monomial $x^{\vv{u}} \in \LL[x]$.
\end{lemma}
...
\begin{proof}  Consider an ideal $\idealb$ of $R=\kk[x]$, where $\kk$ is a field of characteristic $p>0$, set $\idealc = \mon(\idealb)$, and let $q=p^e$.  We will now demonstrate that $\idealc^{[q]} = \mon(\idealb^{[q]})$.  One containment is immediate:   As $\idealc^{[q]} \subseteq \idealb^{[q]}$, we have that $\idealc^{[q]} = \mon(\idealc^{[q]}) \subseteq \mon(\idealb^{[q]})$.
   ...
   As $x^{\vv{u}} = x^{\vv{v}q} x^{\vv{w}} \in \idealb^{[q]}$, we have $x^{\vv{w}} \in ( \idealb^{[q]}: x^{\vv{v}q}) = (\idealb:x^{\vv{v}})^{[q]}$, where the equality follows from the fact that $R$ is free over $R^q$.
...
In what follows, we extend our use of the term \emph{(small) compatible pair}, and say that a pair $(A, \mathcal{C})$ consisting of a monomial matrix $A$ with $\numvars$ rows, and a collection $\mathcal{C}$ of monomials in the ambient variables $x = x_1, \ldots, x_{\numvars}$, is \emph{compatible (and small)} if for some (equivalently, for every) field $\LL$, the pair of ideals $(\ideala, \idealc)$ is  compatible (and small),  where $\ideala$ is the monomial ideal of $\LL[x]$ associated to $A$, and $\idealc = \mathcal{C} \LL[x]$ is the monomial ideal generated by $\mathcal{C}$.


\begin{theorem}
\label{reduce to diagonal general: T}
{Consider a compatible pair $(A, \mathcal{C})$, where $A$ is a monomial matrix with $m$ rows, and $\mathcal{C}$ is a collection of monomials in the variables $x = x_1, \ldots, x_\numvars$.    Then, there exist monomial pairs $(A_1, \vv{u}_1), \ldots, (A_{\ell}, \vv{u}_{\ell})$, all of which are small when $(A, \mathcal{C})$ is, with the following properties\textup:  Each $A_i$ either equals $A$, or can be obtained from $A$ by deleting some of its rows, and if $\kk$ is a field of characteristic $p>0$, and $(\ideala, \idealc)$ is the compatible pair of ideals of $\kk[x]$ associated to $(A, \mathcal{C})$, then} 
\begin{align*}
\nu(\ideala, \idealc, p^e) & = \max  \val \IP(A_i, \vv{u}_i, p^e)  \\ 
\mu(\ideala, \idealc, p^e) & = \max  \val \IP_p(A_i, \vv{u}_i, p^e) 
\end{align*}
for every positive integer~$e$.  Finally, there exists a single pair $(A_i, \vv{u}_i)$ that achieves the maximum in the expression for $\mu(\ideala, \idealc, p^e)$ for all such $e$
\end{theorem}

\begin{proof}  
We will start by considering the special case that $\mathcal{C}$ is corresponds a nonzero point $\vv{v} \in \NN^\numvars$, in the sense that $\mathcal{C} = \{ x_i^{v_i} : v_i \neq 0 \}$. Without loss of generality, we may assume that exactly the first $t$ coordinates of the point $\vv{v}$ are nonzero, so that $\mathcal{C} = \{x_1^{v_1}, \ldots, x_t^{v_t}\}$, and the truncated point ${\vv{u}} = (v_1, \ldots, v_t) \in \ZZ^t$ is positive.  

Let $B$ be the matrix consisting of the first $t$ rows of $A$.  If $\LL$ is an arbitrary field, and $(\ideala, \idealc)$ is the radically compatible pair in $\LL[x]$ associated to $(A, \mathcal{C})$, then $\ideala \subseteq \sqrt \idealc = \ideal{x_1, \ldots, x_t}$, which shows that every monomial generator of $\ideala$ is divisible by some $x_i$ with $1 \leq i \leq t$, and hence, that every column of $B$ is nonzero. Thus,  $B$ is a monomial matrix, and $(B, {\vv{u}})$ is a monomial pair.  Similarly, if $(A, \mathcal{C})$ is compatible, then every monomial generator of $\ideala$ is a multiple of some $x_i^{v_i}$ for some $1 \leq i \leq t$, and so $(B, \vv{u})$ is small.  Regardless, when $\LL=\kk$ is a field of characteristic $p>0$ it is obvious that $\nu(\ideala, \idealc, p^e) = \val \IP (B, \vv{u}, p^e)$ and $\mu(\ideala, \idealc, p^e) = \val \IP_p(B, \vv{u}, p^e)$ for all $e$.
...
\label{irreducible-decomposition: e}
 ...
 \end{equation}
%
where each set of monomials $\mathcal{C}_i = \{ x_s^{v_s(i)} : v_s(i) \neq 0 \}$ is associated to the nonzero point $\vv{v}_i = (v_1(i), \ldots, v_\numvars(i)) \in \NN^\numvars$. Furthermore, the points $\vv{v}_1, \ldots, \vv{v}_{\ell}$ arising in this way depend only on $\mathcal{C}$, but are independent of the choice of field $\LL$.  In addition, as the pair $(A, \mathcal{C})$ is radically compatible, then so is each pair $(A, \mathcal{C}_i)$.

By the special case considered above, there is a pair $(A_i, \vv{u}_i)$ such that, when $\LL=\kk$ is a field of characteristic $p>0$, then $\nu(\ideala, \idealc_i, p^e) = \val \IP(A, \vv{u}_i, p^e)$ and $\mu(\ideala, \idealc_i, p^e) = \val \IP_p(A, \vv{u}_i, p^e)$ for every positive integer $e$, where $\idealc_i = \mathcal{C}_i \LL[x]$.    It follows from this, \eqref{irreducible-decomposition: e}, and the fact that standard Frobenius powers commute with intersection in $\kk[x]$ | see, e.g., \cite[Lemma 2.3]{blickle+mustata+smith.discr_rat_FPTs} | \pedro{Maybe include reference?} 
...
that 
%
\[ \mu(\ideala, \mathfrak{c}, p^e) = \max  \mu(\ideala, \idealc_i, p^e)  = \max \val \IP_p(A_i, \vv{u}_i, p^e) \] and likewise for $\nu(\ideala, \mathfrak{c}, p^e)$.  Finally,  \Cref{cor: mu comparison} guarantees that, in the case of $\mu(\ideala, \mathfrak{c}, p^e)$, there is a single such index that achieves this maximum for every $e$.
\end{proof}

\begin{remark}
\label{lifting pairs: R}
{Our proof of} \Cref{reduce to diagonal general: T} is explicit, in the sense that the monomial pairs $(A_1, \vv{u}_1), \ldots, (A_{\ell}, \vv{u}_{\ell})$ that it exhibits correspond to the components of the combinatorial decomposition \eqref{irreducible-decomposition: e}.  Nevertheless, there are other valid choices of such pairs that are of theoretical interest.  For instance,  \Cref{lifted-programs: L} implies that one can always assume that there are such pairs with $A_i=A$ for all $1 \leq i \leq \ell$.
\end{remark}


\begin{theorem}%[Reduction to diagonal ideals]
   \label{reduce-to-monomial-case: T} {Given a radically compatible pair of ideals $(\ideala, \idealb)$ of $\kk[x]$, with $\kk$ a field of characteristic $p>0$, and $\ideala$ the monomial ideal associated to a monomial matrix $A$, there exists positive points $\vv{u}_1, \ldots, \vv{u}_{\ell} \in \ZZ^\numvars$ such that}
\begin{align*} 
\nu(\ideala, \idealb, p^e) = \nu(\ideala, \mon(\idealb), p^e) & = \max \val \IP(A,\vv{u}_i, p^e) \\ 
\mu(\ideala, \idealb, p^e) = \mu(\ideala, \mon(\idealb), p^e) & = \max \val \IP_p(A,\vv{u}_i, p^e)
\end{align*}
%
...
\begin{proof} {
   Set $\idealc \coloneqq \mon(\idealb)$.  Since $\ideala$ is monomial, a (Frobenius) power of $\ideala$ sits in~$\idealb^{[p^e]}$ if and only if it sits in $\mon(\idealb^{[p^e]})$, which equals $\idealc^{[p^e]}$ by} \Cref{frobenius-powers+mon-commute: L}.  Consequently, $\nu(\ideala, \idealb, p^e) = \nu(\ideala, \idealc, p^e)$ and $\mu(\ideala, \idealb, p^e) = \mu(\ideala, \idealc, p^e)$ for every $e$.  Finally, if we fix a set of monomials $\mathcal{C}$ in $x = x_1, \ldots, x_\numvars$ such that $\mathcal{C} \kk[x] = \idealc$, then the theorem follows from applying \Cref{reduce to diagonal general: T} and \Cref{lifting pairs: R} with the compatible pair $(A, \mathcal{C})$.
   \end{proof}
...
Let $\kk$ be a field of characteristic $p \equiv 3 \bmod 4$. Consider the ideals $\ideala = \langle x^4, y^4 \rangle$ and $\idealb = \mon(\idealb) = \ideal{x^5, x^3 y, y^3} = \ideal{x^3, y^3} \cap \ideal{x^5, y}$ of $\kk[x,y]$.  Let $A$ be the $2 \times 2$ diagonal matrix with $4$ in each diagonal entry, $\vv{u}_1=(3,3)$, and $\vv{u}_2=(5,1)$.  

Then 
\[ \nu(\ideala, \idealb, p^e) = \max \{ \val \IP(A,\vv{u}_1, p^e), \val \IP(A, \vv{u}_2, p^e) \} \] and \Cref{max depends on p and e: E} tells us both that this maximum value equals $(3p^e-1)/2$, and that the integer program achieving this maximum alternates with $e$.

Furthermore, it is not hard to see that there is no positive point $\vv{v} \in \NN^2$ such that $\val \IP(A, \vv{v}, p^e) = (3p^e-1)/2 = \nu(\ideala, \idealb, p^e)$ for every $e$, at least when $p \equiv 3 \bmod 4$.  Indeed, \Cref{max depends on p and e: E} implies that an equality $\val \IP(A, \vv{v}, p^e) = (3p^e-1)/2$ for all $e$ would force an equality $\ft{A}{\vv{v}} = 3/2$, and so $\vv{v}$ would have to be one $(1,5), (2,4), (3,3), (2,4)$, or $(1,5)$.  However, as $p \equiv 3 \bmod 4$,  \Cref{concrete-solution-to-LP-IP: E}   would then tell us that, in each case, $\val \IP(A, \vv{v}, p^e) = (3p^e-3)/2$ for infinitely many $e$.
\end{example}
...
\begin{remark}[Maximal ideals of domains finitely generated over $\ZZ$]
   ...
   \pedro{The previous claim follows from \cite[V.3.4, Theorem~3(ii)]{bourbaki.commalg}, and does not depend on $A$ being a domain.} \daniel{OK.  In the proof I know, I thought this was important.}  
...
\begin{definition}[Reduction to prime characteristic]
   ...
   If $A$ is a finitely generated $\ZZ$-subalgebra of $\kk$, then we set $R_A \coloneqq A[x]$, and if $I$ is an ideal of $R$, then $I_A$ denotes any ideal of $R_A$ that extends to $I$ under the inclusion $R_A \hooklongrightarrow R$.   We call such an ideal $I_A$ an $A$-model of $I$, and when such an ideal exists, we say that $A$ \emph{supports} {the model $I_A$ of $I$}.\footnote{Note that every ideal $I$ of $R$ has an $A$-model for a suitable $A$: one may take $A$ to be the $\ZZ$-subalgebra of $\kk$ generated by the coefficients appearing in a finite generating set for $I$.}
...
\begin{proof}
   By generic freeness, we may fix a nonzero element $\ell \in A$ such that each of the tensor products $A[\ell^{-1}] \otimes_A (J/I)$ and $A[\ell^{-1}] \otimes_A A[x]/J$ is free over $A[\ell^{-1}]$.
   Let $0 \leq \rho \leq \infty$ denote the rank of the free $A[\ell^{-1}]$-module $A[\ell^{-1}] \otimes_A (J/I)$, and let $B$ be an $A[\ell^{-1}]$-module.

Applying the functor $M \mapsto A[\ell^{-1}] \otimes_A M$ to the short exact sequence
\[ 0 \to J/I \to A[x] / I \to A[x]/J \to 0 \] 
yields a short exact sequence of $A[\ell^{-1}]$-modules that splits, as the last term is free.
Split exactness and free rank are preserved by base change, so applying the functor $L \mapsto B \otimes_{A[\ell^{-1}]} L$ to this split exact sequence then yields the split exact sequence
\[ 0 \to B \otimes_A (J/I) \to B \otimes_A (A[x]/I) \to B \otimes_A (A[x]/J) \to 0 \] 
of $B$-modules, which starts with a free $B$-module of rank $\rho$.  
To conclude the proof, note that the composition of the injective map $B \otimes_A (J/I)  \to B \otimes_A (A[x]/I)$ with the canonical isomorphism $B \otimes_A (A[x]/I) \cong B[x]/IB[x]$ is the canonical map in the statement, and that the free rank $\rho$ of the first term is independent of $B$.
...
\begin{remark}
   Let $\kk$ be a field of characteristic zero, $I$ an ideal of $R=\kk[x]$, and $A$ a finitely generated $\ZZ$-subalgebra of $\kk$ that supports a model $I_A$ of $I$.  
According to \Cref{comparing expansions:  P}, there exists a nonzero element $\ell \in A$ such that 
\[ B \otimes_A I_A  \cong I_AB[x] \quad \text{and} \quad B \otimes_A (R_A / I_A)  \cong B[x]/I_A B[x] \] 
for every $A[\ell^{-1}]$-algebra $B$.  In particular, if we take $B=\kk$, then
\[ \kk \otimes_A I_A  \cong I_A R = I  \quad \text{and} \quad \kk \otimes_A (R_A / I_A)  \cong R/I. \] 
...
These isomorphisms illustrate that  \Cref{reduction: D} is consistent with other standard notions of reduction to prime characteristic considered in the literature.  For example, the {conditions $I_A R = I$ on $I_A$ and $\kk \otimes_A I_A \cong I$} are equivalent, and though $I_A R_A(\idealm)$  may differ from $(A/\idealm) \otimes_A I_A$ for some $\idealm \in \mspec(A)$, they are isomorphic for all $\idealm$ in some dense open subset of $\mspec(A)$.  In particular, every result below concerning the expansions $I_A(\idealm) =I_A R_A(\idealm)$ for all $\idealm$ in such a subset of $\mspec(A)$ is valid for $(A/\idealm) \otimes_A I_A$ for all $\idealm$ in another such subset of $\mspec(A)$.
\end{remark}
...
\item \label{reduction preserves compatibility} \hl{If $(I,J)$ is a (small) compatible pair, then so are almost all of its reductions.  More precisely, If $I^t \subseteq J$ for some natural number $t$, then $I_A(\idealm)^t \subseteq J_A(\idealm)$ for every point $\idealm$ in some dense open subset of $\mspec(A)$.}
...
\begin{proof}
   ...
   Given that the extension of each of $I_A$, $J_A$, and $L$ to $R$ equals $I$, setting $B_{\circ} = \kk$ in \Cref{comparing expansions:  P} implies that their extensions to $B[x]$ agree for every $A[\ell^{-1}]$-algebra~$B$.
 ...
 In the context of the second assertion, both $I_A$ and $\mathcal{C}R_A$ are $A$-models for $I$, and in the context of the third, if $I^t \subseteq J$, then both $J_A$ and $I_A^t +J_A$ are $A$-models for $J=I^t + J$.
...
\begin{proposition}
   ...
   Given ideals $I$ and $J$ of $A[x]$, there exists a nonzero element $\ell \in A$ such that}
   \[
      (J:_{A[x]} I)B[x] = (JB[x] :_{B[x]} IB[x])
   \]
   for any $A[\ell^{-1}]$-algebra $B$.
\end{proposition}

\begin{proof}  \hl{As this result is well-known, we will only consider the case that $I$ is principally generated by an element $f \in A[x]$.  The general case, which requires an additional, standard application of generic freeness, is left to the reader.}
...
\begin{corollary}[The formation of colon ideals commutes with most reductions to prime characteristic]
   \label{colon mod p: C}
 {Let $\kk$ be a field of characteristic zero, and $I$ and $J$ be ideals of $R=\kk[x]$.
   If $A$ is a finitely generated $\ZZ$-subalgebra of $\kk$ that supports models $I_A$ and $J_A$, then  $(J_A :_{R_A} I_A)$ is an $A$-model of $(J:_R I)$.
   Furthermore,} 
   \[ %((I :_R f))_A(\idealm) \coloneqq 
   (J_A :_{R_A} I_A) R_A(\idealm) = (J_A(\idealm) :_{R_A(\idealm)} I_A(\idealm)) \]
   for every point $\idealm$ in some dense open subset of $\mspec(A)$.
    \end{corollary}

 \begin{proof}
    Suppose that $\ell \in A \setminus \{0\}$ satisfies the conclusion of \Cref{colon mod p: P} relative to the ideals $I_A$ and $J_A$ of $R_A=A[x]$.  Our claim then follows from invoking \Cref{colon mod p: P} with $B=\kk$, and with $B=A/\idealm$ for every point $\idealm$ in the dense open set $\mathcal{U} = \{\idealm \in \mspec(A) : \operatorname{char}(A/\idealm) \nmid \ell\}$ of $\mspec(A)$. 
%     We may then take $B=\kk$ in \Cref{colon mod p: P} to see that $(J_A:_{R_A} I_A)R = (J :_{R} I)$, which shows that $(J_A :_{R_A} I_A)$ is an $A$-model for $(J :_R I)$.
...
In \Cref{all monomial colons: C} {below, we will show that, once $J$ is fixed, the element} $\ell$ in \Cref{colon mod p: P} can be chosen independently of $I$, {as long as $I$ is a principal monomial ideal.   In our proof, we will use a basic property of colon ideals:  If $J$ is an ideal of an arbitrary commutative ring $R$, then}
\begin{equation}
\label{iterated colon: e}
 (J: ab) = ((J: a): b) \text{ for every } a,b \in R.
\end{equation}
...
\begin{proof}
 ...
 \[
       (JB[x]:_{B[x]} x_i^{t}) = (J:_{A[x]}x_i^{t})B[x]
                               ...
                               =(JB[x]:_{B[x]} x_i^{t+1})
   \]
   for each $i$, and as above, we may conclude that $(JB[x]:_{B[x]}x^\vv{u}) = (JB[x]:_{B[x]}x^{\phi(\vv{u})})$.

   After replacing $\ell$ with a nonzero multiple, we may also assume that the conclusion of \Cref{colon mod p: P} holds for the finitely many ideals $(J:_{A[x]} x^{\phi(\vv{u})})$ of $A[x]$, and so
...
\[ \mon(J) B[x] = \mon(JB[x])\]
for every $A[\ell^{-1}]$-algebra $B$ {that happens to be a field}.
\end{theorem}
...
\begin{proof}
   ...
   By \Cref{single multiplier: L}, we may fix $a \in A \setminus \{0\}$ such that $x^{\vv{u}} \in \mon(J)$ if and only if $ax^{\vv{u}} \in J$.
...
%   \pedro{or $A[\ell^{-1}][x]$; I prefer the simpler notation} \daniel{OK.  I exchanged $A[\ell^{-1},x] \leftrightarrow A[\ell^{-1}][x]$}
...
\begin{theorem}[The $\mon$ operation commutes with most reductions to prime characteristic]
   ...
   Furthermore,
   \[ \mon(I_A) R_A(\idealm) = \mon(I_A(\idealm)) \]
   for every point $\idealm$ in some dense open subset of $\mspec(A)$. \qed
\end{theorem}

We are now prepared to describe the behavior of the $\nu / \mu$ invariants associated to a monomial ideal under reduction to prime characteristic.  We begin with a general fact that holds for all compatible pairs in characteristic zero, monomial or not.

\begin{proposition}
\label{general-compatible->smalll: P}
  {Consider a compatible pair of ideals $(I,J)$ of $\kk[x]$, where $\kk$ is a field of characteristic zero. Let $t$ be the natural number such that $I^t \not \subseteq J$ but $I^{t+1} \subseteq J$, and set $L=(J: I^t)$.  Then $(I,L)$ is a small compatible pair, and given a finitely generated $\ZZ$-subalgebra $A$ of $\kk$, and models $I_A, J_A$ and $L_A$ for $I,J$ and $L$, respectively, there exists a dense open set $\mathcal{U} \subseteq \mspec(A)$ such that}
%
\[ \mu(I_A(\idealm), J_A(\idealm), p^e) = t p^e + \mu(I_A(\idealm), L_A(\idealm), p^e) \] 
%
for all $\idealm \in \mathcal{U}$ and $e \geq 1$.
\end{proposition}

\begin{proof}   Let $A, I_A, J_A$, and $L_A$ be as above.  A standard application of \Cref{comparing expansions:  P} shows that $I_A(\idealm)^t \not \subseteq J_A(\idealm)$ for all $\idealm$ in some dense open subset $\mathcal{U}$ of $\mspec(A)$, and \Cref{reduction basics: C}  \eqref{reduction preserves compatibility} tells us that, after possible shrinking $\mathcal{U}$, we may also assume that $I_A(\idealm)^{t+1} \subseteq J_A(\idealm)$ for all $\idealm \in \mathcal{U}$. In addition,  \Cref{reduction basics: C} \eqref{different models of same ideal}
 and \Cref{colon mod p: C} imply that we may further assume that 
 \[ L_A(\idealm) = (J_A(\idealm) : I_A(\idealm)^t) \text{ for every $\idealm \in \mathcal{U}$}. \] 
 
 Replacing $\mathcal{U}$ with a dense open subset, we may even assume that the characteristic of $A/\idealm$ is greater than $t+1$ for every $\idealm \in \mathcal{U}$, and the result then follows from \Cref{compatible to small compatible: L} with $p = \operatorname{char}(A/\idealm)$, and $\ideala = I_A(\idealm), \idealb = J_A(\idealm)$, and $\idealc = L_A(\idealm)$.
 \end{proof}


\begin{theorem}
   \label{invariants associated to a monomial ideal in large characteristic: T}   
   {Consider a compatible pair of ideals $(I,J)$ of $R=\kk[x]$, where $\kk$ is a field of characteristic zero, and $I$ is the monomial ideal associated to a monomial matrix $M$.  Then there exists $t \in \NN$, small monomial pairs $(M, \vv{u}_1),  \ldots, (M, \vv{u}_{\ell}),$ and a function $(A, I_A, J_A) \mapsto \mathcal{U}$  that associates to every finitely generated $\ZZ$-subalgebra $A$ of $\kk$ supporting models $I_A$ of $I$, and $J_A$ of $J$, a dense open set $\mathcal{U} \subseteq \mspec(A)$, with the following properties} \textup:
   If $\idealm \in \mathcal{U}, p = \operatorname{char}(A/\idealm)$, then 
   \[
      \mu(I_A(\idealm), J_A(\idealm), p^e) = tp^e + \max \val \IP_p(M, \vv{u}_i, p^e). 
   \] for every positive integer $e$.
Furthermore, for each such $p$, there is a single pair $(M, \vv{u}_i)$ that achieves this maximum for every such $e$.  
\end{theorem}

\begin{proof}  Let $t \in \NN$ and $L = (J: I^t)$ be as in the statement of \Cref{general-compatible->smalll: P}, and let $A, I_A$, and $J_A$ be as above.  \Cref{colon mod p: C} tells us that $L_A \coloneqq (J_A: I_A^t)$ is an $A$-model for $L$, and \Cref{general-compatible->smalll: P} then implies that
\begin{equation}
\label{step-1-mu-reduction: e}
\mu(I_A(\idealm), J_A(\idealm), p^e) = t p^e + \mu(I_A(\idealm), L_A(\idealm), p^e)
 \end{equation}
%
for all $e \geq 1$, and all points $\idealm$ in some dense open subset of $\mspec(A)$.  In light of this, we now shift our attention to the  term $\mu(I_A(\idealm), L_A(\idealm), p^e)$ in \eqref{step-1-mu-reduction: e}.  

To start, recall that $I$ is the monomial ideal of $\kk[x]$ associated to the monomial matrix $M$, and fix a collection of monomials $\mathcal{C}$ in $x=x_1, \ldots, x_m$ such that $\mon(L) = \mathcal{C} \kk[x]$.   We may then fix monomial pairs $(M_1, \vv{u}_1), \ldots, (M_{\ell}, \vv{u}_{\ell})$ satisfying the conclusions of \Cref{reduce to diagonal general: T}, relative to $(M, \mathcal{C})$.  As $(M, \mathcal{C})$ corresponds to the small compatible pair $(I, L)$, each $(M_i, \vv{u}_i)$ may also be taken to be small, and by \Cref{lifting pairs: R} we may also assume that $M_i = M$ for each $1 \leq i \leq \ell$. 

Let $\idealm \in \mspec(A)$, and consider the following analogous ``mod $\idealm$" statements:
%
\begin{equation}
\label{step-3-mu-reduction: e}
\text{$I_A(\idealm)$ is the monomial ideal of $R_A(\idealm) = (A/\idealm)[x]$ associated to $M$.}
\end{equation}
%
\begin{equation}
\label{step-2-mu-reduction: e}
\mon(L_A(\idealm)) = \mathcal{C} R_A(\idealm).
\end{equation}


We claim that each of these statements hold for all points $\idealm$ in some dense open subset of $\mspec(A)$.  For \eqref{step-3-mu-reduction: e}, this simply follows from  \Cref{reduction basics: C} \eqref{reduction preserves being monomial}.  

For \eqref{step-2-mu-reduction: e},  \Cref{mon-operation-modulo-p: T} implies that $\mon(L_A)$ is an $A$-model for $\mon(L)$, and our choice of $\mathcal{C}$ guarantees that $\mathcal{C} R_A$ is another $A$-model for $\mon(L)$.  In light of this redundancy, \Cref{reduction basics: C} \eqref{different models of same ideal} implies that $\mon(L_A) R_A(\idealm) = \mathcal{C} R_A(\idealm)$ for all $\idealm$ in some dense open subset of $\mspec(A)$.  On the other hand, \Cref{mon-operation-modulo-p: T} also tells us that $\mon(L_A) R_A(\idealm) = \mon(L_A(\idealm))$ for all $\idealm$ in some dense open subset of $\mspec(A)$, and combining this with the preceding observation, we conclude that \eqref{step-2-mu-reduction: e} also holds for all $\idealm$ in some dense open subset of $\mspec(A)$.  

Thus, \eqref{step-1-mu-reduction: e}, \eqref{step-3-mu-reduction: e}, and \eqref{step-2-mu-reduction: e} hold simultaneously for all $\idealm$ in some dense open subset $\mathcal{U}$ of $\mspec(A)$.  Fix a point $\idealm \in \mathcal{U}$, and note that
%
\[ \mu(I_A(\idealm), L_A(\idealm), p^e) = \mu(I_A(\idealm), \mon(L_A(\idealm)), p^e) = \mu(I_A(\idealm), \mathcal{C} R_A(\idealm), p^e).\]
Indeed, the first equality follows from \eqref{step-3-mu-reduction: e} and \Cref{reduce-to-monomial-case: T}, and the second from \eqref{step-2-mu-reduction: e}.  It follows from this,  our choice of the $(M, \vv{u}_i)$, and \Cref{reduce to diagonal general: T}, that 
%
\[ \mu(I_A(\idealm), L_A(\idealm), p^e) = \mu(I_A(\idealm), \mathcal{C} R_A(\idealm), p^e)  =  \max \val \IP_p(M, \vv{u}_i, p^e) \]
%
and that there is a pair $(M, \vv{u}_i)$ that achieves this maximum for every positive integer $e$.  Comparing this with \eqref{step-1-mu-reduction: e}
...
The main difference between  \Cref{invariants associated to a monomial ideal in large characteristic: T} and  \Cref{nu invariants associated to a monomial ideal in large characteristic: T} is that, in the latter, there is no reduction to case of small pairs, and as was the case with  \Cref{reduce-to-monomial-case: T}, the positive point $\vv{u}$ that realizes $\nu(I_A(\idealm), J_A(\idealm), p^e)$ is allowed to depend on $e$, and sometimes must, as illustrated by \Cref{less uniformity for nu: E}.

\begin{theorem}
   \label{nu invariants associated to a monomial ideal in large characteristic: T}   
   {Consider a compatible pair of ideals $(I,J)$ of $R=\kk[x]$, where $\kk$ is a field of characteristic zero, and $I$ is the monomial ideal associated to a monomial matrix $M$.  Then there exist monomial pairs $(M, \vv{u}_1),  \ldots, (M, \vv{u}_{\ell}),$ and a function $(A, I_A, J_A) \mapsto \mathcal{U}$  that associates to every finitely generated $\ZZ$-subalgebra $A$ of $\kk$ supporting models $I_A$ of $I$, and $J_A$ of $J$, a dense open set $\mathcal{U} \subseteq \mspec(A)$, with the following properties} \textup:
   If $\idealm \in \mathcal{U}, p = \operatorname{char}(A/\idealm)$, and $e$ is a positive integer, then 
   \[
      \nu(I_A(\idealm), J_A(\idealm), p^e) = \max \val \IP (M, \vv{u}_i, p^e).  \qed
   \]
\end{theorem}
...
%
%OLD VERSION OF APPENDIX APPEARS BELOW
%
%\newpage
%\appendix
%
