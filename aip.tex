...
Suppose that $\mathcal{X}$ is either $\RR^\numvars$ or $\ZZ^\numvars$.  By a \emph{program $\Omega$ in the ambient space $\mathcal{X}$}, we mean an optimization problem seeking to maximize some given linear \emph{objective function} $\RR^\numvars \to \RR$ on some subset of $\mathcal{X}$ defined by a system of linear inequalities, called the \emph{constraints} of the program $\Omega$. If $\mathcal{X} = \RR^\numvars$, then the points in $\mathcal{X}$ satisfying these constraints define a polyhedron in $\RR^\numvars$, while if $\mathcal{X} = \ZZ^\numvars$, then the points in $\mathcal{X}$ satisfying these constraints instead consist of the lattice points in some polyhedron in $\RR^\numvars$.  In either case, we call this set of points in $\mathcal{X}$ the \emph{feasible set} of the program.  We denote it by $\feas \Omega$, and say that the points in this set are \emph{feasible} for the program $\Omega$.

To distinguish between these cases, we refer to a program in $\mathcal{X} = \RR^\numvars$ as a {real linear program}, or \emph{linear program} for short, and to a program in $\mathcal{X}=\ZZ^\numvars$ as an {integer linear program}, or \emph{integer program} for short.

Every program $\Omega$ we consider in this article has the property that the values taken on by the objective function on $\feas \Omega$ are bounded from above, and in this case, the maximum value achieved by the objective function on this feasible set is called the \emph{value} of the program $\Omega$, and is denoted $\val \Omega$.  The subset of $\feas \Omega$ where the objective function achieves this maximum value is called the \emph{optimal set} of $\Omega$, and is denoted $\opt \Omega$.   When $\Omega$ is a linear program in $\RR^\numvars$, its optimal set is a face of the polyhedron $\feas \Omega$ in $\RR^\numvars$.
...
\begin{definition}[Optimal shortfall]  The \emph{optimal shortfall}, or simply \emph{shortfall}, of the integer program $\IP(A, \vv{u}, q)$ is the set $\short \IP(A, \vv{u}, q)$ defined by
%
...
\begin{remark}[Algebraic interpretation of optimal images]
\label{optim motivation: R}
Let $(A, \vv{u})$ be a monomial pair associated to an ideal pair $(\ideala, \ideald)$. If $q$ is a power of $p$, and $\nu = \nu(\ideala,\ideald,q) = \val \IP(A,\vv{u},q)$, then the optimal image of $\IP(A,\vv{u},q)$ characterizes the ``leftovers'' of the ideal $\ideala^\nu$ modulo $\ideald^{[q]}$.

More precisely, the discussion preceding \Cref{aip: D} tells us that the generators of $\ideala^{\nu}$ not contained in $\ideald^{[q]}$ are precisely the monomials of the form $x^{A \vv{k}}$, with $\vv{k}$ in the domain lattice of $A$, and satisfying $\norm{\vv{k}} = \nu$ and $A\vv{k}<\vv{u}q$.  Keeping in mind that $\nu= \val \IP(A, \vv{u}, q)$, these conditions on $\vv{k}$ are equivalent to $\vv{k}$ being optimal for $\IP(A, \vv{u}, q)$.  It follows from this observation that
...
\!{transition}Thus, the $F$-threshold of $\ideala$ with respect to $\ideald$ can be expressed as the limit
%
\begin{equation}
...
Dividing by $q$ and taking the limit as $q$ tends to infinity, we find that the limits in \eqref{ft as limit of normalized program values: eq} are at least $\val \LP$.  Given our earlier work, we conclude that these limits coincide with the value of the linear program $\LP$.
\end{proof}
...
\begin{equation*}
h = \sum_{e=0}^t h_e \, p^e \quad \text{and} \quad \vv{h}=\sum_{e=0}^t \vv{h}_e \, p^e
\end{equation*}
where $0\le h_e < p$ and $\vv{0}\le\vv{h}_e < p  \vv{1}$ for each $0 \leq e \leq t$, and where $h_t$ and $\vv{h}_t$ are allowed to be zero.
In any case, \cite{dickson.multinomial} then tells us that
\[
    \binom{h}{\vv{h}}\equiv \binom{h_0}{\vv{h}_0}\binom{h_1}{\vv{h}_1}\cdots \binom{h_t}{\vv{h}_t} \mod{p}.
\]

In particular, the multinomial coefficient $\binom{h}{\vv{h}}$ is nonzero mod $p$ if and only if $\norm{\vv{h}_e}=h_e$ for each $e$, a condition that is sometimes described by saying that the entries of the vector $\vv{h} \in \NN$ sum to $h$ \emph{without carrying} in base $p$.

In light of this, the arithmetic constraint in \Cref{aip: D} is equivalent to the following condition:  If
 $\vv{k} = \vv{k}_0 + \vv{k}_1 \, p\cdots + \vv{k}_t \, p^t$ is the base $p$ expansion of the nonnegative lattice point $\vv{k}$, then $\norm{\vv{k}_e} < p$ for all $0 \leq e \leq t$.
\end{remark}

Clearly, the feasible set of $\IP_p(A, \vv{u}, q)$ lies in the feasible set of $\IP(A, \vv{u},q)$, and hence, is a finite set.  
...
The set $\sierp_{2,2} \cap [0,1]^2$ is the familiar Sierpi\'nski triangle.
\pedro{Together with the line segments connecting $(1,0)$ to $(1,1)$ to $(0,1)$; but maybe we don't need to be so precise here.} \daniel{Isn't this actually an important point, given the remark following \Cref{fractal program: D}?}
...
\begin{remark}[On the values and optimal sets of fractal programs]  By the discussion preceding \Cref{fractal program: D}, the feasible set of $\fip_p(A, \vv{u})$ is contained in the feasible set of $\LP(A, \vv{u})$, and hence, is bounded.  Thus, $\feas \fip_p(A, \vv{u})$ is compact, and so $\val \fip_p(A, \vv{u})$, the value of $\fip_p(A, \vv{u})$, can be defined as 
\[ \max \{ \norm{\vv{t}}: \vv{t} \in \feas \fip_p(A, \vv{u}) \} = \sup \{ \norm{\vv{t}} : \vv{t} \in \sierp_{p,n}, A \vv{t} < \vv{u} \} \] which is a well-defined real number.  As usual, we define the optimal set of $\fip_p(A, \vv{u})$ to be the set $\opt \fip_p(A, \vv{u})$ of feasible points attaining this maximum.
...
\end{center}
\medskip
\caption{The values and optimal sets of $\fip_p = \fip_p(A, \vv{u})$ for $A$ and $\vv{u}$ described in \Cref{ex: feas fip}, for small values of $p$}
...
\pedro[inline]{Define recession basis here?}
...
Therefore, to conclude our proof,  it remains to show that $\val \LP = \lambda$, where $\LP = \LP(A, \vv{u})$.  Towards this, note that if $\vv{s} \in \feas \LP$, then $\vv{s}\ge \vv{0}$ and $A \vv{s} \leq \vv{u}$, and consequently
  $\norm{\vv{s}} \le \iprod{\vv{c}}{A \vv{s}} \leq \iprod{\vv{c}}{\vv{u}} = \lambda$,
   where the first inequality follows from \Cref{prop: inner product with columns of A}, and the second from the nonnegativity of $\vv{c}$,  a fact that is intuitively clear, but nevertheless established later in this article, in \Cref{face: P}.
   In any case, the value of $\LP$ is at most. 
...
Let $(A, \vv{u})$ be a monomial pair associated to an ideal pair $(\ideala, \ideald)$. If $\beta = \beta(A)$ is as in \Cref{uniform uniform value and image: T}, then this same theorem tells us that if $q > \beta$, then\daniel{If we like this description, I guess we'll have to define $F$-thresholds for ideals, which I am not sure we have done} \[ \nu(\ideala, \ideald, q) = \ft{\ideala}{\ideald} \cdot q - \delta(A, \vv{u},q). \] 
...
In this case,  the collapse along $\O$ is the identity map, and so ${ \im \IP(A, \vv{u}, q)}$ equals ${\vv{u}}q - \Delta(A, \vv{u},q)$.  In light of this, it follows from \Cref{optim motivation: R} that 
