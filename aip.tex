...
We introduce here the notion of a \emph{sprout} of a monomial pair, a concept that was implicitly introduced, though not explicitly defined, in \Cref{small pairs from shortfalls: L}.  Generally speaking, a sprout of a monomial pair is another monomial pair constructed from the original one, in a natural way, using universal shortfalls. 
...
\begin{proposition} \label{sprout: P}  Let $(\matA, \vv{u})$ be a monomial pair,  let an overbar denote collase along $\mf(\matA, \vv{u})$, and let $d$ be a universal denominator for $\matA$.  
...
In \Cref{1-sprouts: P}, we see that for very small pairs, and when $q=1$, the notion of a sprout extends that of collapsing along the associated minimal face, and the universal deficit coincides with the  Newton degree.  In \Cref{nonnegativity-fundamenta-sequence: P}, we relate the Newton degree of a sprout with the universal deficit of the original pair, and in \Cref{sprouts-in-convenient-planar-case: P}, we consider the convenient and planar case.
...
\begin{proof}
   ...
   By definition, $\matB=\collapse{\matA}$ and $\vv{v} \in \ushort_{\q}(\matA,\vv{u})$.
