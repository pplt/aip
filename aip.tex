...
Indeed, if $\vv{c}_i$ denotes the $i$-th column of $A$, then \Cref{alpha=1: convention} and the nonnegativity of $\vv{s}$ imply that $\iprod{\vv{a}}{\vv{c}_i} \cdot s_i \geq s_i$ for every $1 \leq i \leq n$, so that
...
\daniel{We don't always stick to it.  E.g., the columns of $A$, and the standard basis vectors.}
\begin{proof}
