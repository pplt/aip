...
\Input{ \\
   ...
   $\bullet$ a representative $p_0$ for a congruence class modulo a universal denominator $\denom$
}
\medskip
\Output{ \\ $\bullet$ a generating function $F(\pp,\tt) \in \QQ(\pp,\tt)$ for $\val\IP_\pp(A,\vv{u},\pp^e)$
}
\medskip
\Begin{
   \medskip
...
$M_1 \leftarrow \ft{A}{\vv{u}}\pp - \udeficit(A,\vv{u},p_0)$\;
\medskip
\While{\texttt{\em true}}{
   $e\leftarrow e+1$\;
