...
\begin{example} \
% \emily[inline]{If $(A, \vv{u})$ is small but not very small, 
% 
% though 
% \Cref{arithmetic uniform value and image: T} tells us that the shortfall of $\Pi_p(A, \vv{u},q)$ agrees with that of $\Pi(A, \vv{u},q)$
% 
% On the other hand, 
% 
% then though $\val \Pi_p(A,u,q) = p-1$ \Cref{small not very small value: T}
% 
% 
% then $\mu(A,\vv{u},p) = p-1$, so it is less than $\nu(A,\vv{u},p)$.
% In this case, although our description of $\ideala^{\nu(A,\vv{u},p)}$ does not depend on $p$, the generators of $\ideala^{\mu(A,\vv{u},p)} 
% = \ideala^{p-1}$  \emph{can}  depend on $p$:  For instance, if $\vv{u} = (1,1)$ so $\ideala = \langle x, y \rangle$, then $\nu(A,\vv{u},p) = 2p-2$  and $\mu(A,\vv{u},p) = p-1$.  Moreover, $x^{p-(p+1)/2}y^{p-(p+1)/2} \in \ideala^{p-1}$.
% (Actually, all minimal generators of $\ideala^{p-1}$ depend on $p$.)
% }


\emily[inline]{If $(A, \vv{u})$ is small but not very small, then $\mu(A,\vv{u},p) = p-1$, so it is less than $\nu(A,\vv{u},p)$.
In this case, although our description of $\ideala^{\nu(A,\vv{u},p)}$ does not depend on $p$, the generators of $\ideala^{\mu(A,\vv{u},p)} 
= \ideala^{p-1}$  \emph{can}  depend on $p$:  For instance, if $\vv{u} = (1,1)$ so $\ideala = \langle x, y \rangle$, then $\nu(A,\vv{u},p) = 2p-2$  and $\mu(A,\vv{u},p) = p-1$.  Moreover, $x^{p-(p+1)/2}y^{p-(p+1)/2} \in \ideala^{p-1}$.
(Actually, all minimal generators of $\ideala^{p-1}$ depend on $p$.)
}
