...
\end{remark}
  ...
  Much of what remains of this article is dedicated towards studying the behavior of integers $\nu(\ideala, \ideald, p^e)$ and $\mu(\ideala, \ideald, p^e)$ and rational nmbers $\ft{\ideala}{\ideald}$ and $\crit(\ideala, \ideald)$ defined in the introduction in the special case that \[ \ideald := \diag(\vv{u}) := \langle x_1^{u_1}, \ldots, x_d^{u_d} \rangle \subseteq \kk[x_1, \cdots, x_d]\] is the diagonal ideal  associated to a point $\vv{u} \in \NN^d$ with positive coordinates.

Though this context might seem restrictive, given that our ultimate goal is to understand the nature of these numerical invariants when the diagonal ideal $\ideald$ is replaced with an arbitrary ideal $\idealb$, we explain in \Cref{monomial-reduction: A} how understanding this situation allows us to obtain the \Cref{general-nu-theorem: T,general-mu-theorem: T}.
