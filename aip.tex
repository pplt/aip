...
Let $\kk$ be a field of prime characteristic $p>0$ with $[\kk:\kk^p] < \infty$, and consider the polynomial ring $R = \kk[x_1, \cdots, x_d]$.  Given ideals $\ideala$ and $\idealb$ of $R$ with ${\ideala}$ contained in $\sqrt \idealb$, and a nonnegative integer $e$, we define
%
...
\begin{problem}
Find conditions on the ideal $\ideala$ such that the following holds.  Given an ideal $\idealb$ with $\ideala \subseteq \sqrt \idealb$ and a nonnegative integer $e$ $\ideala$ and $\idealb$, there exists a positive intege $N$ and polynomials $P_i(t) \in \QQ[t]$ of degree $e$ for every $i$ that is a unit modulo $N$ such that the following holds.  If $p$ is sufficiently large and $p \equiv i \bmod N$, then $\nu(\ideala_p, \idealb_p, p^e) = P_i(p)$.  When could $N$ be chosen independently of $J$ and $e$?
\end{problem}
...
\label{general-nu-theorem: T}
  Given a monomial ideal $\ideala$ of $\QQ[x_1, \cdots, x_d]$, and an arbitrary ideal $\idealb$ of this polynomial ring with $\ideala \subseteq \sqrt{\idealb}$, there exists positive integers $\ell = \ell(\ideala)$ and  $\beta = \beta(\ideala, \idealb)$, and for every integer $1 \leq r < \ell$ relatively prime to $\ell$ a rational number $\delta(\ideala, \idealb, r)$ satisfying the following conditions.  If $p^e > \beta(\ideala, \idealb)$ and $p^e \equiv r \bmod \ell$, then $ \nu(\ideala_p, \idealb_p, p^e) = \ft{\ideala}{\idealb} \cdot p^e - \delta(\ideala, \idealb, r)$.
\end{theorem}

As above, let $\kk$ be a field of prime characteristic $p>0$ with $[\kk: \kk^p]$ finite.  If $\ideala$ is an ideal of $\kk[x_1, \cdots, x_n]$, then the Frobenius powers of $\ideala$ are a family of ideals $\ideala^{[t]}$ indexed by a nonnegative real parameter $t$.  When the exponent is an integer, then the corresponding Frobenius power can be described concretely in terms of generators as follows:  If $k$ is a natural number, and $\ideala$ is generated by polynomials $f_1, \cdots, f_n$, then $\ideala^{[k]}$ is the ideal generated by the products $f^{\vv{u}} = f_1^{u_1}\cdots f_n^{u_n}$, ranging over all points $\vv{u} \in \NN^n$ for which the multinomial coefficient $\binom{k}{\vv{u}}$ is nonzero modulo $p$  \cite[Proposition~3.5]{hernandez+etal.frobenius_powers}.

Now, if $\idealb$ is an ideal of $\kk[x_1, \cdots, x_n]$ with $\ideala \subseteq \sqrt{\idealb}$, then in analogy with the above definitions, given a nonnegative integer $e$, we define \[\mu(\ideala,\idealb,p^e) \coloneqq \max\big\{k\in \NN : \ideala^{[k]} \not\subseteq \idealb^{[p^e]}\big\}.\]
...
Given a monomial ideal $\ideala$ of $\QQ[x_1, \cdots, x_d]$, there exists a positive integer $\ell = \ell(\ideala)$, and for every ideal $\idealb$ with $\ideala \subseteq {\idealb}$ and for every integer $1 \leq r < \ell$ relatively prime to $\ell$, there exist a positive integer $\beta = \beta(\ideala, \idealb)$ and sequence $(\epsilon(\ideala, \idealb, r, e))_{e=1}^{\infty}$ of nonnegative rational numbers satisfying the following conditions.  If $p > \beta$ and $p \equiv r \bmod \ell$, then \[ \mu(\ideala_p, \idealb_p, p^e) = \ft{\ideala}{\idealb} \cdot p^e - \sum_{s=1}^{e} \epsilon(\ideala, \idealb, r, s) \cdot p^{e-s}.\]
\end{theorem}

\newpage
\section{Frobenius and optimization}
\label{sec: LPs}

Consider a proper monomial ideal
\[ \ideala = \langle x^{\vv{a}_1}, \ldots, x^{\vv{a}_n} \rangle \subseteq \kk[x_1, \ldots, x_d] \]
where $\kk$ is a field of prime characteristic $p>0$.  Without loss of generality, we will assume that each ambient variable is necessary to define $\ideala$, so that \[ A = \begin{bmatrix} \, \vv{a}_1 & \cdots & \vv{a}_n \, \end{bmatrix} \] is a $d \times n$ matrix over $\ZZ$ with non-negative, nonzero rows and columns.  The key role of such matrices in this article motivates the following definition.
...
The generators of a powers of $\ideala$ can be described compactly in terms of the matrix $A$.   Indeed, this follows from the fact that the product $(x^{\vv{a}_1})^{k_1} \cdots (x^{\vv{a}_n})^{k_n}$ can be written as $x^{A \vv{k}} = x^{k_1 \vv{a}_1 + \cdots + k_n \vv{a}_n}$.  That is, $\ideala^n$ is generated by all monomials  of the form $x^{A \vv{k}}$ with $\vv{k} \in \NN^n$ satisfying $\norm{\vv{k}} = n$.
%
% \begin{equation}
...
Let $(A, \vv{u})$ be a monomial pair.  In what follows, we investigate the  behavior of the integers $\nu(\ideala, \ideald, p^e)$ and $\mu(\ideala, \ideald, p^e)$ and rational nmbers $\ft{\ideala}{\ideald}$ and $\crit(\ideala, \ideald)$ defined in the introduction in the special case that \[ \ideald := \diag(\vv{u}) := \langle x_1^{u_1}, \ldots, x_d^{u_d} \rangle \subseteq \kk[x_1, \cdots, x_d].\]
...
\pedro[inline]{If and when we include an analogue to \Cref{cor: every crit is crit wrt diagonal ideal} for $F$-thresholds, we should mention that here, to justify the fact that we're only looking at $F$-thresholds with respect to diagonal ideals. \daniel[inline]{This is what I was going for with the above.}}


As will soon be apparent, it is natural to describe the the integers $\nu(\ideala, \ideald, p^e)$ and $\mu(\ideala, \ideald, p^e)$ and rational nmbers $\ft{\ideala}{\ideald}$ and $\crit(\ideala, \ideald)$ in terms of certain optimization problems.  To describe these optimization problems, we now recall the basic framework of integer and linear programming.
...
\subsection{Relations with integer and linear programs}  We return to the problem at hand.  Recall that for each $q$ a power of $p$ we define the integer
...
\begin{definition}
   Given a monomial pair $(A, \vv{u})$ and positive integer $q$, the integer program $\IP(A, \vv{u}, q)$ in the domain lattice of $A$ consists of maximizing the function $\vv{k} \mapsto \norm{\vv{k}}$ subject to the constraints $\vv{k} \geq \vv{0}$ and $A \vv{k} \leq \vv{u}q - \vv{1}$.
\end{definition}

It is easy to deduce from the fact that the matrix $A$ is a monomial matrix (i.e., has nonnegative, nonzero rows and columns) that the feasible set of the integer program $\feas \IP(A,\vv{u},q)$ is finite.  Consequently, this program has a well-defined value.
...
\begin{definition}  Given a monomial pair $(A, \vv{u})$, the linear program $\LP(A, \vv{u})$ in the domain of $A$ consists of maximizing the function $\vv{k} \mapsto \norm{\vv{k}}$ subject to the constraints $\vv{k} \geq \vv{0}$ and $A \vv{k} \leq \vv{u}$.
\end{definition}
...
\begin{proposition}  The value of $\LP(A, \vv{u})$ equals the $F$-threshold $\ft{\ideala}{\ideald}$.
\end{proposition}

\begin{proof}
   As usual, suppose that $A$ is a $d \times n$ monomial matrix.
   If $\vv{k} \in \ZZ^n$ optimal for the inter progam $\IP(A,\vv{u},q)$, then the scaled point $\vv{k}/q$ is feasible for the linear program $\LP = \LP(A, \vv{u})$, so $\val \LP \geq \val \IP(A, \vv{u}, q)/q$ for all $q$.
   ...
   By design, and the fact that $A$ is monomial, the $i$-th entry of $A \vv{t}_q$ is either less than the $i$-th entry of $A (q \vv{t})$ | which itself is less than $\vv{u} q$, by the feasibility of $\vv{t}$ for $\LP$ | or equals $0$,  in which case it is also less than $u_i q$ since $\vv{u}$ is positive. This observation demonstrates that $\vv{t}_q$ is feasible for $\IP(A, \vv{u}, q)$, and so
...
\subsection{Relations with arithmetic and fractal programs}  We now turn our attention to the numerical invariants that arise when replacing regular powers of $\ideala$ above with Frobenius powers.  Let $(A, \vv{u})$ and $\ideald$ be as above.
...
Consider $\ell \in \NN$ and $\vv{k} \in \NN^n$.  As every natural number has a unique base $p$ expansion, we may uniquely express these quantities as
\begin{equation*}
\ell = \sum_{e=0}^r \ell_e \, p^e \quad \text{and} \quad \vv{k}=\sum_{e=0}^r \vv{k}_e \, p^e ,
\end{equation*}
where $0\le \ell_e < p$ and $\vv{0}\le\vv{k}_e < p  \vv{1}$ for each $0 \leq e \leq r$, and where $\ell_r$ and $\vv{k}_r$ are allowed to be zero.
In any case, \cite{dickson.multinomial} then tells us that
\[
    \binom{\ell}{\vv{k}}\equiv \binom{\ell_0}{\vv{k}_0}\binom{\ell_1}{\vv{k}_1}\cdots \binom{\ell_r}{\vv{k}_r} \mod{p}.
\]

In particular, $\binom{\ell}{\vv{k}}$ is nonzero modulo $p$ if and only if $\norm{\vv{k}_e}=\ell_e$ for each $e$, a condition that is commonly described by saying that the components of the vector $\vv{k}$ add up to $\ell$ \emph{without carrying} in base $p$.

In light of this, the arithmetic constraint on $\vv{k}$ appearing in is equivalent to the following condition:  If
 $\vv{k} = \vv{k}_0 + \vv{k}_1 \, p\cdots + \vv{k}_r \, p^r$ is the unique base $p$ expansion of the nonnegative lattice point $\vv{k}$, then $\norm{\vv{k}_e} < p$ for all $0 \leq e \leq r$.
\end{remark}
...
\end{equation}
%
In what follows, we relate these quantities to a variant of a linear program we call a \emph{fractal linear program}.  To do so, we require the following concept.


\daniel[inline]{To minimize notation, I changed ``in $\RRnn^n$'' to ``of dimension $n$" .  This won't come up outside of this section anymore, so I hope this is OK}

\begin{definition}
\label{sierpinski: D}
   The \emph{Sierpi\'nski $p$-gasket of dimension $n$} is the set $\sierp_{p,n}$ consisting of all points $\vv{t}\in \RR^n$ for which there exist a natural number $s$, and sequence of points $\{ \vv{t}_e \}_{e=1}^\infty$ in $\NN^n$ and such that $\norm{\vv{t}_e} < p$ for all $e \geq 1$, and
 \[
\vv{t} = p^s \cdot\Big(\frac{\vv{t}_1}{p} +\frac{\vv{t}_2}{p^2}+\cdots +\frac{\vv{t}_e}{p^e} + \cdots \Big).
 \]
\end{definition}

We start with some preliminary observations.  If $\widehat{\sierp}_{p,n} := \sierp_{p,n} \cap [0,1]^n$, then an immediate consequence of the definition that\[ \sierp_{p,n} = \bigcup_{s=0}^{\infty} \ p^s \, \widehat{\sierp}_{p,n}. \]

Thus, when describing the basic properties of the Sierpinski $p$-gasket,  we may focus on its restriction to the unit cube.

Recall for every $t \in [0,1]$ such that $p^s t \notin \NN$ for every $s \in \NN$, there exists a unique sequence $\{ t_e \}_{e=1}^{\infty}$ of integers such that $0 < t_e < p$ for all $e \geq 1$, the integer $t_e \neq 0$ is nonzero for infinitely many $e$, and $t = \sum_{e=1}^{\infty} \frac{t_e}{p^e}$. Otherwise, if $p^s t \in \NN$ for some $s \in \NN$, then $t$ has both a non-terminating base $p$ expansion, as described above, but also a terminating one.

Thus, if $\vv{t} \in [0,1]^n$ is such that no component of $p^s \vv{t}$ is an integer for each $s \in \NN$, then there is a unique sequence $\{ \vv{t}_e \}_{e=1}^{\infty}$ in $ \NN^n$ such that $\vv{t} = \sum_{e=1}^{\infty} \frac{\vv{t}_e}{p^e}$, and such point lies in $\widehat{\sierp}_{p,n}$ if and only if $\norm{\vv{t}_e}$ is less than $p$ for all $e \geq 1$.  However, things are more subtle when some component of $p^s \vv{t}$ is an integer.

For instance, if $p=2$, then $(1/4, 1/4) \in \sierp_{2,2}$.  Indeed, for the first component, take the binary expansion $\frac{1}{4} = \frac{1}{2^3} + \frac{1}{2^4} + \frac{1}{2^5} + \cdots$, and for the second component, simply take the binary expansion $\frac{1}{4} = \frac{1}{2^2}$.  Note that if one instead considered the non-terminating expansion for both components, or the terminating expansion for both components, then the resulting expansion of $(1/4, 1/4)$ would fail to satisfy the condition in \Cref{sierpinski: D}.
...
\daniel[inline]{With the new organization, we should think about where to talk about images.  I think it might be best to only mention them closer to when they are used.}
...
\begin{equation}\label{ft as limit of normalized program values: eq}
\ft{\ideala}{\ideald} = \lim_{e\to\infty} \frac{\nu(\ideala,\ideald,p^e)}{p^e} = \lim_{e\to\infty} \frac{\val \IP(A,\vv{u},p^e)}{p^e}.
\end{equation}
...
\emily{Added to eliminate linear programming from preliminaries.}
...
This proposition and \eqref{ft as limit of normalized program values: eq} suggest the following definition.
...
By definition, both $\mon(I)_p$ and $\mon(I_p)$ are monomial ideals, and so they are equal if and only if they contain the same monomials.  However, if $h$ is a monomial contained in $I$, then it is clear that $h_p$ is a monomial contained in $I_p$ for all $p$.  Thus, to prove \Cref{mon-operation-modulo-p: T} it suffices to prove the folllowing statement:  If $p \gg 0$ and $h$ is a monomial with $h \notin I$, then $h_p \notin I_p$.
