...
\daniel[inline]{Sounds good.  Can you remember the example?  Was it $\langle x^2, y^2 \rangle$ and $\vv{u} = (1,1)$ in characteristic $2$?}
}
...
Here, as in \Cref{sec: LPs},  we use $A$ to denote a fixed $d \times n$ monomial.  In fact, throughout this section, we fix a monomial pair $(A, \vv{u})$, so that $\vv{u}$ is a point in $\NN^d$ with positive entries.  Our goal is to study the linear program $\LP(A, \vv{u})$ in terms of the \emph{Newton polyhedron} associated to the matrix $A$.
