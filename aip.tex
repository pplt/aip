...
We work in the polynomial ring $\kk[x_1, \cdots, x_d]$ over a field $\kk$, and adopt standard notation for describing monomials in this ring:  If $\vv{u} \in \NN^d$, then \[ x^{\vv{u}} = x_1^{u_1} \cdots x_d^{u_d}.\]
...
\begin{proposition}
   ...
   \begin{enumerate}
      ...
      \item\label{limit} The limit of $q^{-1}\val \IP(A,\vv{u},q)$, as $q$ tends to infinity. \daniel{I replaced this with a more general statement, since I am thinking we out to emphasize that this isn't necessarily a prime characteristic result}
...
%    \end{quote}   
%    \daniel[inline]{I could have sworn that we needed the more general versions somewhere, but it looks like I am wrong.  Let's just go with the simpler statement, and revert back later if we realize we need to.}
%    \pedro[inline]{I went ahead and made the change.}
%   }
...
\begin{proof}[Proof of \Cref{FT descriptions: P}]
   %\ \todo[inline]{Show that $(1) = (2)$\emily[inline]{I'm working on this.}}
      Given $\vv{s} \in \ZZ^n$ optimal for $\IP(A,\vv{u},q)$, $q^{-1} \vv{s}$ is feasible for $\LP = \LP(A, \vv{u})$, so $\val \LP \geq q^{-1} \val \IP(A, \vv{u}, q)$ for all $q$. After taking the limit at $q$ grows, we see that the $\val \LP$ is at least the limit appearing in $(2)$.
   Alternately, given an optimal point $\vv{t} \in \RR^n$ for $\LP$,
   define $\vv{t}_q \in \ZZ^n$ as the point whose $i^\text{th}$ coordinate is $0$ if $t_i=0$, and otherwise equals $\lceil q t_i \rceil - 1$, the greatest integer less than $q t_i$.  
   By design, the $j^\text{th}$ entry of $A \vv{t}_q$ is either less than the $j^\text{th}$ entry of $A (q \vv{t}) < \vv{u} q$, or equals $0$,  in which case it is also less than $u_j q$ since $\vv{u}$ is positive. This means that $\vv{t}_q$ is feasible for $\IP(A, \vv{u}, q)$, and 
   \[\val \IP(A, \vv{u}, q) \geq \norm{\vv{t}_q} \geq \sum_{i=1}^n \lceil q t_i \rceil - 1 \geq q \norm{\vv{t}} -n = q \cdot \val \LP - n.\]
   Multiplying by $q^{-1}$ and taking the limit as $q$ grows, we find that the limit appearing in $(2)$ is at least $\val \LP$, and hence these two numbers coincide.
