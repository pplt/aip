...
\begin{example}
   ...
   \begin{figure}
      \centering
      \includegraphics[width=\textwidth]{pictures/sprouting_graph.pdf}
   ...
   \begin{figure}
      \centering
      \includegraphics[width=.5\textwidth]{pictures/graph_on_newton.pdf}
      \caption{A geometric representation of the sprouting subgraph}
      \label{fig: graph on newton}
   \end{figure}
   Note that $\sprout^*_p(A,\vv{u})$ is essentially the same as $\sprout^*_p(\collapse{A},\collapse{\vv{u}})$, which is pictured geometrically in \Cref{fig: graph on newton}, together with the Newton polyhedron for~$\collapse{A}$.
