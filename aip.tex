...
where the overbar denotes collapse along the minimal face $\mf(A, \vv{u})$.
\end{definition}
...
\begin{example}
   ...
   As $q$ ranges from $1$ to $17$, $\collapse{A}\tail{\vv{s}}_q$ ranges over seventeen different positive lattice points.
   ...
   This shows that the optimal set for $\ip$ may be infinite.
   In \Cref{finite image: P} we will show, however, that $\collapse{A}(\opt \ip)$ is always finite. 
\end{example}
...
\begin{proof}
   ...
   Indeed, if $s_i$ is zero, then so is $\tail{s_i}_q$, while the feasibility of  $\vv{h}$ for $\ip = \ip(A, \vv{u}, \vv{s}, q)$ implies that $h_i$ is nonnegative.
   ...
   To proceed, write $\vv{u} = A \vv{s} + \vv{w}$ for some $\vv{w}$ that is positive in $\rs(\O)$, as in \Cref{mc: D}.
...
\begin{proof}
   ...
   To see that this vector is positive, note that $\collapse{\vv{u}} = \collapse{A} \vv{s}$ and $\collapse{A} \tail{\vv{s}}_q$ are both linear combinations with positive coefficients of the same set of columns of $\collapse{A}$.
...
\begin{proof}
   ...
   Whenever the $i$-th column of $A$ does not lie in $\O$, $s_i$ must be zero, and consequently $k_i$ is nonnegative. 
   Consequently, \Cref{prop: inner product with columns of A} tells us that
\begin{equation}
\label{bound in inner product: e}
\norm{\vv{k}} \leq \iprod{\defpt}{A \vv{k}} = \iprod{\collapse{\defpt}}{\collapse{A} \vv{k}},
\end{equation}
where the equality follows from \Cref{rmk: collapse of a matrix,collapse of a defining vector: R}.
That same proposition applied to $\tail{\vv{s}}_q$ gives us
\begin{equation}
...
We now explore some finiteness properties, and our results are of two types:  \Cref{finite image: P} concerns the integer program $\ip$ associated with some fixed monomial list,  while \Cref{finitely many secondary programs: T} and \Cref{finitely many coord sums: C} concern the nature of these programs as the list varies.
...
\begin{proof}  This follows from \eqref{norm of tail: e} and the proof of \Cref{finitely many secondary programs: T}.
\end{proof}
...
\begin{proof}  \Cref{finite image: P} implies that for every monomial list $(A, \vv{u}, \vv{s}, q)$,  there exists a \emph{finite} subset $\fsr(A, \vv{u}, \vv{s}, q)$ of $\opt (A, \vv{u}, \vv{s}, q)$ such that
\[  \im \ip (A, \vv{u}, \vv{s}, q) = \ol{ A(\fsr(A, \vv{u}, \vv{s}, q)) } =  \collapse{A}(\fsr(A, \vv{u}, \vv{s}, q)) \]
and \Cref{finitely many secondary programs: T} then implies that these sets may be chosen in such a way so that $\fsr(A) = \cup \, \fsr(A, \vv{u}, \vv{s}, q)$ is finite, where the union is over all $A$-lists.
\end{proof}
