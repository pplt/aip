...
We say that such a point $\defpt$ \emph{defines} $\O$ in $\N$.  In this article, we are laregly concerned with faces $\O$ of $\N$ that do not lie in any coordinate subspace of $\RR^d$, which we call \emph{standard}.
...
Indeed, if $\vv{a}_i$ denotes the $i$-th column of $A$, then \Cref{alpha=1: convention} and the nonnegativity of $\vv{s}$ imply that $\iprod{\defpt}{\vv{a}_i} \cdot s_i \geq s_i$ for every $1 \leq i \leq n$, with equality if and only if $s_i = 0$ or $\vv{a}_i \in \O$.
Thus,
...
\begin{example}\label{ex: illustrating FT descriptions}
   If $A$ and $\vv{u}$ are as in \Cref{ex: ft}, then the minimal face of $(A,\vv{u})$, shown in blue in \Cref{fig: newton polyhedron}, is defined by the point $\defpt = (3/17, 0, 1/17)$.
   \Cref{FT descriptions: P} then tells us that
   \[\ft{A}{\vv{u}} = \iprod{\defpt}{\vv{u}} = \tfrac4{17}.\]
   From another perspective, \Cref{fig: splitting polytope}
   \begin{figure}
      \centering
