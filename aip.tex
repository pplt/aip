...
The following theorem plays a key role in our algorithm for computing the value of $\IP_p(A, \vv{u}, p^e)$ for $p \gg 0$ when $\ft{A}{\vv{u}} \le 1$ and the matrix $A$ is associated to an $\idealm$-primary ideal, where $\idealm$ is the ideal generated by the variables.
...
\pedro{``that achieve $M$'': is this clear enough? Maybe replace with ``with $\val \IP_p(A, \vv{v}, p^e) = M$ (here and on the proof).}
\end{theorem}
