...
\comment[inline]{Going with $\sprout(A,\vv{u},p)$}
 ...
 % \pedro[inline]{How about simply ``$\operatorname{sprout}(A,\vv{u},p)$''? ``$\operatorname{sprout}_p(A,\vv{u})$''? ``$p\text{-sprout}(A,\vv{u})$"? I think the first is probably the most obvious and most consistent with other notation in the paper.
 % \emily[inline]{I like the first option.} \daniel[inline]{OK.  Let's adopt this then.}
 % }
\item[$\Box$] Put in examples.
 ...
 \pedro[inline]{Let's change to $\IP(A,\vv{u},q)$, then.}
\end{enumerate}
}
...
\begin{definition} 
\label{aip: D}
If $p>0$ is a prime integer, then $\IP_p(A, \vv{u})$ is the arithmetic integer program in $\ZZ^n$ defined as follows:
\begin{enumerate}
