...
\emily{New transition.}
Now that we have a grasp on the value of the arithmetic integer program $\Pi_p(A, \vv{u}, p^e)$ when the matrix $A$ is associated to an $\idealm$-primary monomial ideal, we broaden our scope to address the case that $A$ is an arbitrary monomial matrix.  In order to do so, we first build a technical framework in which to carry out our investigation, starting with the \emph{directed sprouting graph}.  
...
\emily{New transition.}
Consider the following subgraphs of the directed sprouting graph.  Their definitions are motivated by the sets with the same names used in the $\idealm$-primary setting, which appear in the proof of \Cref{m-primary AIP: T}, and from which \Cref{m-primary algorithm: R} is built from. 
...
\begin{proof}
   ...
   The existence of $\ell$ satisfying (1) will follow from \Cref{comparing expansions:  L} applied to the containments $(J:_{A[x]}x^\vv{u}) \subseteq A[x]$, once we verify that there are only finitely many ideals $(J:_{A[x]} x^\vv{u})$.
   ...
   Indeed, if $\vv{v} = (v_1,\ldots,v_m)$ with $v_i = \min\{u_i,N\}$, then repeated applications of \eqref{iterated colon: e} and our choice of $N$ show that $(J:_{A[x]}x^\vv{u}) = (J:_{A[x]}x^\vv{v})$.

   Suppose $\ell$ satisfies (1).
   By possibly replacing $\ell$ with a nonzero $A$-multiple, we may assume that $\ell$ also satisfies the conclusion of \Cref{colon mod p: L} for the finitely many ideals $(J:_{A[x]} x^\vv{v})$ with $\vv{v} \le (N+1)\vv{1}$.
...
\pedro[inline]{
   I agree.
   We should just include a note like ``In the same way that \Cref{colon mod p: L} implies \Cref{colon mod p: C}, \Cref{mon-operation-modulo-p: L} implies the following result.''
}
...
\begin{corollary}
   ...
   If $\maxi \in \mathcal{U}$ and $p = \operatorname{char}(A/\maxi)$, then there exists a diagonal ideal $\ideald$
   \daniel{
   ...
   in $R_A(\maxi)$ associated to a positive point in $\NN^m$ such that
   \[
      \mu(I_A(\maxi), J_A(\maxi), p^e) = \mu(I_A(\maxi), L_A(\maxi), p^e)  = \mu(I_A(\maxi), \ideald, p^e)
   \]
   for every $e \geq 1$.
   In particular,
   \[
...
\Cref{expansion of monomial mod most p is monomial: C,containment in radical mod p: C} tell us that $I_A(\maxi)$ is a monomial ideal contained in the radical of $J_A(\maxi)$, while if we set $L=\mon(J)$, then \Cref{mon-operation-modulo-p: T} tells us that $L_A = \mon(J_A)$ is a model for $L$, and that $L_A(\maxi) = \mon(J_A(\maxi))$.    Our claims then follow from \Cref{reduce-to-monomial-case: C}.
\end{proof}
