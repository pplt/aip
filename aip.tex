...
\title[Fractal and arithmetic programs]{Fractal and arithmetic programs, and the Frobenius powers of monomialideals}
...
\begin{proof}[Proof of \Cref{FT descriptions: P}]
      Given $\vv{s} \in \ZZ^n$ optimal for $\IP(A,\vv{u},q)$, $q^{-1} \vv{s}$ is feasible for $\LP = \LP(A, \vv{u})$, so $\val \LP \geq q^{-1} \val \IP(A, \vv{u}, q)$ for all $q$. After taking the limit at $q$ grows, we see that the $\val \LP$ is at least the limit appearing in \eqref{limit}.
   ...
   Multiplying by $q^{-1}$ and taking the limit as $q$ grows, we find that the limit appearing in \eqref{limit} is at least $\val \LP$, and hence these two numbers coincide.
