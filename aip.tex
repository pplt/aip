...
\pedro{
   Is there a more concrete characterization for polyhedra?
   E.g., points not in any proper face?
}
...
\pedro[inline]{
   ...
   By \Cref{opt set: C}, $\opt \LP$ is defined by $A\vv{s} \le \vv{u}$ and $\vv{s}\ge \vv{0}$, with equality in some specific coordinates, and thus defined by a system of inequalities $B \vv{x} \le \vv{b}$, where $\vv{b}$ is an integral vector and $B$ is a submatrix of the matrix $M$ obtained by stacking $A$, $-A$, the identity matrix $I_n$, and $-I_n$.
...
If $\O$ is bounded, then $\rb(\O) = \emptyset$, and so $\cone( \rb(\O)) = \{\vv{0} \}$ is equal to its relative interior.  Next, set $\lambda = \ft{A}{\vv{u}}$ and assume that $\O$ is unbounded.
 ...
 \pedro{I'm confused by this argument. What characterization of relative interior is being used here?}
\end{proof}
...
\pedro[inline]{
   ...
   Or bring back \Cref{ex: ft} again, where $\opt \LP(A,\vv{u})$ is the line segment connecting $(1/17, 0, 3/17)$ and $(2/17, 1/17, 1/17)$, and only the second endpoint is special for $(A,\vv{u})$.
