...
\todo[inline]{Add conventions for $p,q, e, \kk,\mathfrak{m}$, etc}
...
We start with the following theorem, which underlies some useful finiteness results that we later establish. 
...
\label{small pairs from shortfalls: E}  
...
\begin{proof}
If we fix a universal denominator $\denom$ for $A$,   then \Cref{finitely many coord sums: C} implies that there exists an integer $\beta=\beta(A,\denom)$ such that $p/\denom > \norm{\tail{\vv{s}}_p}$ for every $A$-list $(A,\vv{u},\vv{s}, p)$ with $p > \beta$.  In what follows, we will assume that $p > \beta$.

Next, suppose that $(A,\vv{u})$ is small, but not very small, and let $\vv{s}$ be a special point for this pair with denominator $\denom$.   \Cref{canonical-feasible: T} then states that 
\[ \vv{k} \coloneqq \vv{s}p-\tail{\vv{s}}_p \in \feas \IP(A,\vv{u},p). \] 

Since $(A,\vv{u})$ is not very small, $\vv{s} = \ft{A}{\vv{u}} > 1$, and our choice of $\denom$ then implies that $\norm{\vv{s}} \geq 1 + 1/\denom$, so that \[\norm{\vv{k}} \geq p + p/\denom - \norm{\tail{\vv{s}}_p} > p\] where the strict inequality follows from our choice of $p \gg 0$.  

This bound implies that there exists a point $\vv{h} \leq \vv{k}$ with $\norm{\vv{h}} = p-1$.  Without loss of generality, assume $h_1 < k_1$ , and define $\vv{g} = \vv{h}q + \vv{e}_1(q-1)$.  By construction, $\vv{g} \leq \vv{k}q, \norm{\vv{g}} = qp-1$, and $\binom{\norm{\vv{g}}}{\vv{g}} \not\equiv 0 \bmod p$.  Furthermore, the fact that $\vv{k} \in \feas \IP(A,\vv{u},p)$ implies that $A\vv{g} \leq A{\vv{k}}q < \vv{u}qp$, and so $\vv{g} \in \feas \IP_p(A,\vv{u},qp)$.  It follows that $\val \IP_p(A,\vv{u},qp) \geq \norm{\vv{g}} = qp-1$, and \Cref{trivial small value bound: P} implies that this bound must be an equality.
\end{proof} 
...
%
\[\deficit \IP_p(A,\vv{u},p)=\deficit \IP(A,\vv{u},p) = \udeficit(A,\vv{u},p) \] and 
\daniel{Weird observation:  If I replace the command ``ol" with ``collapse" below, the $p$ becomes bold too.  Check later to see if this introduced other typesetting errors in the paper.}
\emily{It works OK if we put curly brackets around the entire vector, including the {\tt vv}.}
%
...
\begin{proof}  If $\beta = \beta(A)$ is as in \Cref{uniform uniform value and image: T}, then \[ \val \IP(A, \vv{u}, p) = \ft{A}{\vv{u}} \cdot p - \udeficit(A, \vv{u}, p) \] for every monomial pair $(A, \vv{u})$ and $p > \beta$.  Given that $\ft{A}{\vv{u}} \leq 1$, the positivity of $\udeficit(A, \vv{u},p)$ then implies that this quantity is less than $p$.  Consequently, every $\vv{k}$ feasible for $\IP(A, \vv{u}, p)$ satisfies $\norm{\vv{k}} \leq p-1$, and therefore automatically satisfies the arithmetic constraint of $\IP_p(A, \vv{u}, p)$.  We conclude that $\IP(A, \vv{u}, p) = \IP_p(A, \vv{u}, p)$ whenever $\ft{A}{\vv{u}}$ is at most $1$ and $p > \beta$, and our assertion follows from applying \Cref{uniform uniform value and image: T} once again.
\end{proof}

\begin{remark} \label{program behavior small: R}
For $(A, \vv{u})$ is very small, \Cref{arithmetic uniform value and image: T} tells us that for $p \gg 0$, the programs $\Pi = \Pi(A, \vv{u},p)$ and $\Pi_p = \Pi_p(A, \vv{u},p)$ coincide, and consequently, the collapse of the shortfall along $\O = \mf(A,\vv{u})$ of $\Pi_p$ equals that of $\Pi$.  
Hence, since the collapse of $\short \Pi$ along $\O = \mf(A, \vv{u})$ does not depend on $p$,  but rather on the class of $p$ modulo a universal denominator (see \Cref{pair periodicity: R} and \Cref{uniform uniform value and image: T}), the collapse of $\short \Pi_p$ is also independent of $p$.  

However, if $(A, \vv{u})$ is small but not very small, $\Pi$ and $\Pi_p$ need not agree.  What's more, the collapses of their shortfalls along the minimal face can also differ.  
For instance, let $A$ be the $2 \times 2$ identity matrix, and $\vv{u} = (1,1)$.  Then for $p \gg 0$, $\val \Pi_p = p-1$ by \Cref{small not very small value: T}, while $\val \Pi = 2p-2$. 
In this case, $\O = \mf(A,\vv{u})$ is bounded, so we know $\short \Pi$ does not depend on depend on $p$ for $p \gg 0$.  On the other hand, $(\frac{p-1}{2}, \frac{p-1}{2}) \in \opt \Pi_p$ for all $p$, so that $(\frac{p+1}{2}, \frac{p+1}{2})$, which depends strongly on $p$, is always an element of $\short \Pi_p$.

This example illustrates that the nature of the programs $\Pi$ and $\Pi_p$ can differ drastically when $(A,\vv{u})$ is very small, 
and when it is just small. This motivates our technique of reducing to the \emph{very small} setting.  
\end{remark}
...
Observe that this set that depends on the class of $p$ modulo $\denom$, but not on $p$ itself.  Furthermore, \Cref{small pairs from shortfalls: E} tells us that every pair in $\S_{p^2}$ is small.
...
Let $\S_{p^2}^{\ast} = \S_{p^2}^{\ast}(A,\vv{u})$ be the subset of $\S_{p^2}$ consisting of all $(A,\vv{v})$ that first maximize $\ft{A}{\vv{v}}$, and only then minimize $\udeficit(A,\vv{v},p)$.  By definition, $\S_{p^2}^{\ast}$ depends on $p$ modulo $\denom$, and our choice of $p \gg 0$ guarantees that $\S_{p^2}^{\ast}$ corresponds to the points in $\S_{p^2}$ that achieve $M_1$.  Therefore, we may write \[ M_1 = \epsilon_{p^2} p - \delta_{p^2}\]  where $\epsilon_{p^2},\delta_{p^2}$ are positive rational numbers that depend on $p$ modulo $\denom$.
...
\S_{p^3} \coloneqq \S_{p^3}(A,\vv{u}) & = \{ (A,\vv{w}):  \vv{w} \in \short_p(A,\vv{u},p^2) \} \\
 ...
 \end{align*}
%
Observe that $\S_{p^3}$ depends on the class of $p$ modulo $\denom$, but not on $p$ itself.  Furthermore, \Cref{small pairs from shortfalls: E} tells us that every pair in $\S_{p^3}$ is small.
...
\item \label{elementary1 periodic in p} Furthermore, \Cref{pair periodicity: R} tells us that if $\denom$ is a universal denominator for $A$, then the sprouting graph  $\sprout_p(A)$ (meaning, all of its vertices \emph{and} arrows) depends on the class of $p$ modulo $\denom$.
...
\begin{proof}
Fix a monomial matrix $A$, and let $p \gg 0$ so that the conclusions of \Cref{ILL: T,small not very small value: T,arithmetic uniform value and image: T,lem: upper bound for higher mu} are valid.
...
To conclude our proof, it remains to show that the non-leading coefficients in $\val \IP_p(A,\vv{u},p^e)$ are nonpositive, but this is done in  \Cref{non-leading coefficients: C}.
\end{proof}
...
\item[$\Box$]  Describe an analog of \Cref{leftover p large: R}:  Explain how our proof demonstrates that $\mu(A,\vv{u},p^e) = \mu(\ol{A}, \ol{\vv{u}}, p^e)$ for every $p \gg 0$ and $e \geq 1$.  This follows from the fact that $\sprout_p^{\ast}(A,\vv{u}) = \sprout_p(\ol{A}, \ol{\vv{u}})$. 
\item[$\Box$] Should we explain how the eventual periodicity of $\sprout_p^{\ast}(A,\vv{u})$ implies that $\crit(\ideala, \ideald)$ is always rational?  I can see that it might be better to state this with $\ideald$ replaced by an arbitrary ideal $\idealb$, but this will require us to have the contents of the appendix settled.
...
\item[$\Box$]  It seems like if $A$ comes from a positively homogeneous $\idealm$-primary ideal, then most non-leading coefficients vanish, and it should be true that either $\crit(A,\vv{u}) = \ft{A}{\vv{u}}$ or $\crit(A,\vv{u})$ is rational with denominator a power of $p$.  Is this worth pointing out?
\end{enumerate}
}
...
If $(B, \vv{v})$ is a $p$-sprout of  $(A, \vv{u})$, then \[ \udeficit(A, \vv{u}, p) \geq \ft{B}{\vv{v}}\]
with equality if $A$ is the monomial matrix associated with an $\idealm$-primary monomial ideal that is homogeneous with respect to some positive $\ZZ$-grading.
\end{lemma}
...
By definition, $B$ is the collapse of $A$ along $\O$, and $\vv{v} \in \ushort(A,\vv{u},p)$.  With this notation, \Cref{collapse of Newton polyhedron: P} tells us that $\ol{\O}$ is a face of $\ol{\N}$, the Newton polyhedron of $B=\ol{A}$, and that $\ol{\O}$ is defined in $\ol{\N}$ by the collapsed point $\ol{\vv{c}}$.




\Cref{FT descriptions: P}\eqref{lambda} implies that there exists a point $\vv{n} \in \ol{\N}$ such that $\vv{v} = \ft{B}{\vv{v}} \cdot \vv{n}$.  As noted earlier, the point $\ol{\vv{c}}$ defines the face $\ol{\O}$ in $\ol{\N}$, and by \Cref{alpha=1: convention}, it follows that $\iprod{\ol{\vv{c}}}{\vv{n}}$ is at least $1$.  Thus, 
%
...
However, the containment $\vv{v} \in \ushort(A, \vv{u}, p)$ implies that we may write $ \vv{v} = B \tail{\vv{s}}_p - B \vv{h}$ for some $\vv{s} \in \sp_{\QQ}(A, \vv{u})$ and $\vv{h}$  optimal for $\ip = \ip(A, \vv{u}, \vv{s}, q)$.  As $\ol{\vv{c}}$ defines $\ol{\O}$,  the inner product of $\collapse{\defpt}$ with each column of $B$ is at least one, with equality whenever that column lies on $\collapse{\O}$.  Arguing as in the proof of \Cref{bounded value: P}, one may show that $\iprod{\collapse{\defpt}}{B \tail{\vv{s}}_p} = \norm{\tail{\vv{s}}_p}$ and also that $\iprod{\collapse{\defpt}}{ B \vv{h}} \geq \norm{\vv{h}} = \val \ip$,
...
The non-leading coefficients under consideration are as follows.
\begin{itemize}
\item $-(\delta_{q}(A,\vv{u})-\epsilon_{qp}(A,\vv{u}))$ for some $q=p^e$ less than $\Q$.
\item $-(\delta_{\Q}(A,\vv{u}) - 1)$ when $\Q$ is finite.
\item $-1$ and $-\delta_q(A,\vv{u})$ for some $q=p^e$ at most $\Q$.
\end{itemize}

Rationality is clear, and as the arguments are similar, we will only show that the coefficient $-(\delta_{\Q}(A,\vv{u})-1)$ is nonpositive whenever $\Q$ is finite.  In this case, $\S^{\ast}_{\Q p}(A,\vv{u})$ is empty, and so there must exist a pair in $\S_{\Q p}(A,\vv{u})$ that is not very small.  Thus, there is a path $(B,\vv{v}) \to (C,\vv{z})$ in $\sprout_p(A)$, where the first pair lies in $\S^{\ast}_{\Q}(A,\vv{u})$, and the second is not very small. \Cref{non-leading coefficients: L} then tells us that 
$\delta_{\Q}(A,\vv{u}) = \udeficit(B,\vv{v},p) \geq \ft{C}{\vv{z}} > 1$.
\end{proof}
...
\emily[inline]{Note that this Appendix proves \Cref{general-mu-theorem: T} by reducing it to the monomial case.  This is cited throughout this appendix as \Cref{diagonal-mu-theorem: T} that no longer exists.} 
