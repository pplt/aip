...
\emily[inline]{New: Add what we can say about critical exponents.}
\emily[inline]{New: Explain that our method replaces an arbitrary $\idealb$ with a diagonal ideal.}
...
\subsection{Relations with arithmetic and fractal programs}
 ...
 We now turn our attention to the integers $\mu(\ideala, \ideald, p^e)$ and rational numbers $\crit(\ideala,\ideald)$.  First, recall that for each $q$ a power of $p$, the integer $\mu(\ideala, \ideald, q)$ is defined as 
...
It is also immediate that $\vv{t} \in \sierp_{p,n}$ if and only if $p^s  \vv{t} \in \sierp_{p,n}$ for some (equivalently, for every) integer $s \in \ZZ$; this reflects the self-symmetry observed in \Cref{fig: sierpinski 3-gasket}.
...
Recall that every point $t \in [0,1]$ either has a unique base $p$ expansion, which is necessarily non-terminating, or else $p^s t \in \NN$ for some $s \in \ZZ$, in which case $t$ has both a terminating and a non-terminating expansion.
Thus, if $\vv{t} \in [0,1]^n$ is such that no component of $p^s \vv{t}$ is an integer for each $s \in \NN$, then there is a unique sequence $\{ \vv{t}_e \}_{e=1}^{\infty}$ in $ \NN^n$ such that $\vv{t} = \sum_{e=1}^{\infty} \frac{\vv{t}_e}{p^e}$, and such point lies in ${\sierp}_{p,n}$ if and only if all $\norm{\vv{t}_e}$ are less than $p$.
However, things can be more subtle when some $p^s \vv{t}$ has an integer component.
...
\begin{proposition}\label{prop: inner product with columns of A}
   If $\defpt \in \RR^\numvars$ defines a standard face $\O$ of the Newton polyhedron $\N$ associated to the monomial matrix $A$, and $\vv{s} \in \RR^n$ has nonnegative entries, then the inner product $\iprod{\defpt}{A\vv{s}}$ is at least $\norm{\vv{s}}$, and equality holds if and only if $s_i = 0$ whenever the $i$-th column of $A$ is not in $\O$.
\end{proposition}
...
For that, we shall need the following well-known characterization of vertices of a polyhedron, whose proof can be found, for instance, in \cite[Theorem~3.34]{conforti+etal.integer_programming}.
...
\begin{definition}[Least positive residue] If $a, b \in \ZZ$ are positive, then $\lpr{a}{b}$ is the \emph{least positive residue} of $a$ modulo $b$, that is, $a \equiv \lpr{a}{b} \bmod b$ and $1 \leq \lpr{a}{b} \leq b$.
\end{definition}
...
This expression is clearly independent of the choice of  integers $a$ and $b$.
...
Furthermore,
%
\[ \lambda q - \tail{\lambda}_q = \frac{aq-\lpr{aq}{b}}{b} \] is an integer, and in fact, is the \emph{greatest integer less than $\lambda q$}, that is, $\up{\lambda q} - 1$.
This number agrees with $\down{ \lambda q}$ if and only if $\lambda q$ is not an integer.
%\daniel[inline]{Elaborate, describe in terms of rounding up, compare with rounding down}
\end{remark}
...
\begin{proof}
   Set $\O = \mf(A, \vv{u})$.
   The fact that $\vv{s} \in \sp(A, \vv{u})$ implies that \[ \vv{u} = A \vv{s} + \vv{w}\] for some point $\vv{w}$ that is positive in $\rs(\O)$.
...
\begin{remark}
   ...
   In the above proof, we use for the first time what will turn out to be a recurring technique in the remainder of this article:
   In order to prove an inequality between points in $\RR^\numvars$, we prove that the inequality holds in the recession subspace $\rs(\O)$ and that it holds for the collapses of the points along $\O$.
   The latter shows that the inequality also holds in the complementary subspace $\rs(\O)^\perp$, and thus holds in $\RR^\numvars$.
\end{remark}
...
% \daniel{We should remind the reader that the collapsed inequalities are a subset of the ones determined by $A$} 
where the overbar denotes collapse along the face $\O = \mf(A, \vv{u})$.
\end{definition}

Recall that since each row of $\collapse{A}$ is a row of $A$, the inequalities determined by $\collapse{A} \vv{h}  < \collapse{A} \tail{\vv{s}}_q$ above are a subset of those in $A \vv{h}  < A \tail{\vv{s}}_q$.
...
\begin{example}
   ...
   As $q$ ranges from $1$ to $17$, $\collapse{A}\tail{\vv{s}}_q$ ranges over seventeen different positive integral points; this positivity and integrality will be shown in general in \Cref{tail projection: P} below.
   For each of those, the value of $\ip$ is either $0$ or $1$, and the point $(k + \val\ip, k, -2k)$ is feasible, and therefore optimal for $\ip$, for each $k \in \NN$.
   This shows that the optimal set for $\ip$ may be, and typically is, infinite.
\end{example}

Since the feasible set for $\ip(A,\vv{u},\vv{s},q)$ is infinite,
\pedro{Is it ever finite?}
it is not immediately clear that this integer program has a well-defined value.
We now move towards proving that it does, and establishing some useful bounds on that value.


\begin{proposition}
\label{tail projection: P}
If $(A, \vv{u}, \vv{s}, q)$ is a monomial list and $\O = \mf(A, \vv{u})$, then $\collapse{A}\tail{\vv{s}}_q$ is a positive lattice point, where the overbar denotes collapse along $\O$.
\end{proposition}
...
If $(A, \vv{u}, \vv{s}, q)$ is a monomial list, then $\ip(A, \vv{u}, \vv{s}, q)$ has a well-defined value, and
...
which demonstrates that $\val \ip < \norm{\tail{\vv{s}}_q}$.  Finally, the positivity of $\collapse{A}\tail{\vv{s}}_q$ described in \Cref{tail projection: P} implies that $\vv{0}$ is feasible for $\ip$.
\end{proof}
...
\begin{definition}[Shortfall and deficit]
   \label{deficit ip: D}
   The \emph{shortfall} of the integer program $\ip(A, \vv{u}, \vv{s}, q)$ is the set \[ \short \ip(A, \vv{u}, \vv{s}, q) = \collapse{A} \tail{\vv{s}}_q - \collapse{A} (\opt \ip(A, \vv{u}, \vv{s}, q))\]
   where the overbar denotes collapse along $\O = \mf(A, \vv{u})$.
...
\Cref{tail projection: P} and the constraints of $\ip$ imply that the shortfall of $\ip(A, \vv{u}, \vv{s}, q)$ consists of positive lattice points, while \Cref{bounded value: P} shows that the deficit of $\ip(A, \vv{u}, \vv{s}, q)$ is indeed positive.
...
\begin{proof}  Adopt the notation from the proof of \Cref{bounded value: P}.
If $\vv{k}$ is optimal for $\ip$, then \eqref{bound in inner product: e} implies that \[ \val \ip = \norm{\vv{k}} \leq \iprod{\collapse{\defpt}}{\collapse{A} \vv{k}}\] and so $\collapse{A} \vv{k}$ is a lattice point in the polyhedron of all points $\vv{v}$  in $\rs(\O)^{\perp}$ with $\vv{v} < \collapse{A} \tail{\vv{s}_q}$  and $\iprod{\collapse{\defpt}}{\vv{v}} \geq \val \ip$.  The positivity of $\collapse{\defpt}$ in $\rs(\O)^{\perp}$ and \Cref{bounded polytope: L} then tell us  that this polyhedron is bounded.
\end{proof}
...
Next, let $\collapse{A}$ be the collapse of $A$ along the face $\O$.  If $\vv{s} \in \sp_{\QQ}(A, \vv{u})$, then $\vv{0} \leq \tail{\vv{s}}_q \leq \vv{1}$ for every integer $q > 0$, where $\vv{1}$ is the vector in the domain lattice of $A$ consisting of all ones.  Consequently, $\vv{0} \leq \collapse{A} \tail{\vv{s}}_q \leq \collapse{A}\, \vv{1}$, and as \Cref{tail projection: P} tells us that $\collapse{A} \tail{\vv{s}}_q$ has integer coordinates, it follows that there are only finitely many possibilities for this point.
\end{proof}
...
We start our process by defining $\mathcal{S}_1$ to be the first in the sequence of shortfalls associated to the programs $\IP_p(A,\vv{u},p^e)$.  In other words, set \daniel{Replace $\mathcal{S}_e \mapsto \mathcal{S}_{p^e}$?}
\[ \mathcal{S}_1 \coloneqq \short \IP_p(A,\vv{u},p) = \ushort(A,\vv{u},p)\]   
a set that depends on the class of $p$ modulo $\ell$, but not on $p$ itself.

If we define $M_e \coloneqq \max \{ \val \IP_p(A, \vv{v}, p^e) :  \vv{v} \in \mathcal{S}_1 \}$, then  \Cref{small pairs from shortfalls: E} tells us that the pairs $(A, \vv{v})$ appearing in the definition of $M_e$ are all small, and \Cref{trivial small value bound: P}  then tells us that $\val \IP_p(A, \vv{v}, p^e)$ is at most $p^e-1$ for each such pair.  Thus, $M_e \leq p^e-1$, and setting $q=p$ in \Cref{general AIP: T} tells us that
%
\begin{equation}
...
\end{equation}
%
where the union is over all $\vv{v} \in \mathcal{S}_1$ that achieve $M_e$.

As $\mathcal{S}_1$ depends on $p$ modulo the universal denominator $\denom$, the issue of whether there exists a point $\vv{v} \in \mathcal{S}_1$ with $(A,\vv{v})$ not very small also depends on $p$ modulo $\ell$.  When this occurs, \Cref{small not very small value: T} tells us that $M_e = p^e-1$, and \eqref{m-primary value: e} then allows us to conclude that 
%
...
On the other hand, if  $(A,\vv{v})$ is very small for each $\vv{v} \in \mathcal{S}_1$, then \Cref{arithmetic uniform value and image: T} instead allows us to compute $M_1$ as 
\[ M_1 = \max \{ \ft{A}{\vv{v}}p-\udeficit(A,\vv{v},p) : \vv{v} \in \mathcal{S}_1\} \] 

Let $\mathcal{S}_1^{\ast}$ be the subset of $\mathcal{S}_1$ consisting of all $\vv{v}$ that first maximize $\ft{A}{\vv{v}}$, and only then minimize $\udeficit(A,\vv{v},p)$.  By definition, the set $\mathcal{S}_1^{\ast}$ depends on $p$ modulo $\ell$, and our choice of $p \gg 0$ guarantees that $\mathcal{S}_1^{\ast}$ corresponds to the points in $\mathcal{S}_1$ that achieve $M_1$.  Therefore, we may write $M_1 = \epsilon_1 p - \delta_1$ where $\epsilon_1, \delta_1$ are positive rational numbers that depend on $p \bmod \ell$.  

Setting $e=1$ in \eqref{m-primary value: e} then shows that 
%
\[ \val \IP_p(A,\vv{u},p^2) = \ft{A}{\vv{u}}p^2 - (\udeficit(A,\vv{u},p)-\epsilon_1)p - \delta_1  \] 
%
while setting $e=1$ in \eqref{m-primary shortfall: e} describes the second set of shortfalls as
%
\[ \mathcal{S}_2 \coloneqq  \short \IP_p(A,\vv{u},p^2) = \cup_{\vv{v} \in \mathcal{S}_1^{\ast}} \short \IP_p(A,\vv{v},p) = \cup_{\vv{v} \in \mathcal{S}_1^{\ast}} \ushort(A,\vv{v},p) \]  
%
a set that depends on $p$ modulo $\ell$, but not on $p$ itself.


We continue by setting $M'_e = \max \{ \val \IP_p(A, \vv{w}, p^e) :  \vv{w} \in \mathcal{S}_2\}$.  Arguing as above, we see that all pairs $(A,\vv{w})$ appearing in the definition of $M'_e$ are small, and this allows us to invoke \Cref{general AIP: T} with $q=p^2$ to see that
%
\begin{equation}
\label{m-primary value 2: e}
 \val \IP_p(A,\vv{u},p^{e+2}) = \val \IP_p(A,\vv{u},p^2)p^e+ M'_e
 \end{equation}
% \ft{A}{\vv{u}}p^{e+2} - (\udeficit(A,\vv{u},p)-\epsilon_1)p^{e+1} - \delta_1p^e
and also that 
%
\begin{equation}
...
\end{equation}
where the union is over all $\vv{w} \in \mathcal{S}_2$ that achieve $M'_e$.

Further arguing as above, we see that if there exists $(A,\vv{w})$ with $\vv{w} \in \mathcal{S}_2$ that is not very small, then $M'_e = p^e-1$.  In this case,  \eqref{m-primary value 2: e} and our earlier determination of $\val \IP_p(A,\vv{u},p^2)$ tell us that $\val \IP_p(A,\vv{u},p^{e+2})$ equals
%
\[ \ft{A}{\vv{u}}p^{e+2} - (\udeficit(A,\vv{u},p)-\epsilon_1)p^{e+1} - (\delta_1-1)p^e -1 \] 
%
As this formula is valid for all $e \geq 1$, our process terminates in this case.

However, if  $(A,\vv{w})$ is very small for each $\vv{w} \in \mathcal{S}_2$, then \Cref{arithmetic uniform value and image: T} instead implies that $M'_1 = \max \{ \ft{A}{\vv{w}}p-\udeficit(A,\vv{w},p) : \vv{v} \in \mathcal{S}_2\}$.  As above, the subset $\mathcal{S}^{\ast}_2$ of points that achieve $M'_1$ depends on $p$ modulo $\ell$, and can be explicitly described in a concrete way.  Thus, there exist positive rational numbers $\epsilon_2, \delta_2$, depending on $p \bmod \ell$, such that $M'_1 = \epsilon_2 p -\delta_2$, and setting $e=1$ in \eqref{m-primary value 2: e} then demonstrates that $\val \IP_p(A,\vv{u},p^3)$ equals
%
\[ \ft{A}{\vv{u}}p^{3} - (\udeficit(A,\vv{u},p)-\epsilon_1)p^{2} - (\delta_1- \epsilon_2)p -\delta_2. \] 
%

In this situation, may also set $e=1$ in \eqref{m-primary shortfall 2: e} to see that the third collection of shortfalls may be described as 
%
\[ \mathcal{S}_3 \coloneqq  \short \IP_p(A,\vv{u},p^3) = \cup_{\vv{v} \in \mathcal{S}_2^{\ast}} \short \IP_p(A,\vv{w},p) = \cup_{\vv{v} \in \mathcal{S}_2^{\ast}} \ushort(A,\vv{w},p) \]  
%
a set that, like $\mathcal{S}_1$ and $\mathcal{S}_2$, depends on $p$ modulo $\ell$, but not on $p$ itself.

If $\mathcal{S}_3$ contains a point that defines a pair that is not very small, then repeating this process, we can compute the value of $\IP_p(A,\vv{u},p^{e+3})$ for all $e$.  Otherwise, we can only compute the value of the next program $\IP_p(A,\vv{u},p^{4})$, but  will also be able to explicitly describe the next set of shortfalls $\mathcal{S}_4$.

We may continue iterating these steps.  Note that, when it is necessary to define it, the $e$-th shortfall $\mathcal{S}_e$ will be a union of universal shortfalls associated to monomial pairs involving $A$.  It then follows from \Cref{finitely many deltas for a fixed A: P} that all defined $\mathcal{S}_e$ are subsets of some finite set determined by $A$.  Thus, there is an upper bound for the number of possible distinct  $\mathcal{S}_e$, and as soon as such a shortfall repeats, the process must cycle from that point onward.
...
\]
Observe that \eqref{p-big-1} and \eqref{p-big-2} imply that for every $1 \leq i \leq e$, the quantity $p/\denom$ is greater than every coordinate of $\vv{1}-\vv{h}_i$, and every coordinate of $A_i \vv{h}_i$.  It then follows from \Cref{uniform value: P} (or rather, its proof) that
\begin{equation}
...
Thus, it suffices to show that $\iprod{\collapse{\defpt}}{\vv{v}} \leq \delta(A, \vv{u}, p)$.  However, by definition of $p$-sprout, $\vv{v} \in \Delta(A, \vv{u}, p)$, and so fixing a point $\vv{s} \in \sp_{\QQ}(A, \vv{u})$, we may write $ \vv{v} = B \tail{\vv{s}}_p - B \vv{h}$ for some $\vv{h}$  optimal for $\ip = \ip(A, \vv{u}, \vv{s}, q)$.  Our choice of $\defpt$ guarantees that the inner product of $\collapse{\defpt}$ with each column of $B$ is at least one, with equality whenever that column lies on $\collapse{\O}$.  Arguing as in the proof of \Cref{bounded value: P}, one may show that $\iprod{\collapse{\defpt}}{B \tail{\vv{s}}_p} = \norm{\tail{\vv{s}}_p}$ and \[ \iprod{\collapse{\defpt}}{ B \vv{h}} \geq \norm{\vv{h}} = \val \ip \]
...
\begin{theorem}
   \label{diagonal-mu-theorem: T}
   Given a monomial ideal $\ideala$ of $\QQ[x_1, \ldots, x_\numvars]$, there exists a positive integer $\ell = \ell(\ideala)$, and for every positive $\vv{u} \in \NN^\numvars$ with $\ideala \subseteq \ideald = \diag(\vv{u})$, and for every integer $1 \leq r < \ell$ relatively prime to $\ell$, there exist a positive integer $\beta = \beta(\ideala, \vv{u})$ and a sequence $\big(\epsilon(\ideala, \vv{u}, r, e)\big)_{e=1}^{\infty}$ of nonnegative rational numbers satisfying the following conditions.
   If $p > \beta$ and $p \equiv r \bmod \ell$, then
   \[ \mu(\ideala_p, \ideald_p, p^e) = \ft{\ideala_p}{\ideald_p} \cdot p^e - \sum_{s=1}^{e} \epsilon(\ideala, \vv{u}, r, s) \cdot p^{e-s}.\]
\end{theorem}

\begin{proof} \textcolor{red}{prove}
\end{proof}

Finally, as mentioned earlier, our main theorem,\Cref{general-mu-theorem: T}, can be reduced to \Cref{diagonal-mu-theorem: T}, as proven at the end of  
\Cref{monomial-reduction: A}.
}
