...
Given a very small pair $(\matA, \vv{u})$, and positive integer $r$, the fundamental sequences $\{ \lambda_{r^s}(\matA, \vv{u})\}_{s=1}^{\infty}$  and $\{ \eta_{r^s}(\matA, \vv{u}) \}_{s=1}^{\infty}$ satisfy the conditions described in \Cref{main theorem wrt diagonal: T}.  That is, each is an eventually periodic sequence of rational numbers, with each term $\lambda_{r^s}(\matA, \vv{u})$  nonnegative, and each term $\eta_{r^s}(\matA, \vv{u})$  positive.
\daniel[inline]{Add that these sequences only depend on $r$ modulo a universal denominator for $\matA$, and all of its descendants?  Or be much more explicit in \Cref{main theorem wrt diagonal: T}, and less so here?  Should we include information on invariance of fundamental sequences under collapsing?}
\end{proposition}
...
\begin{lemma}
Suppose that $\ideala$ is an $\idealm$-primary ideal generated by monomials that are all of the same degree $d$, with respect to some positive $\ZZ$-grading.  If $\matA$ is the monomial matrix associated to these generators, then every $r$-sprout of a pair $(\matA, \vv{u})$ is of the form $(\matA, \vv{v})$, and satisfies  $\degree(\matA,\vv{v}) = \udeficit(\matA, \vv{u}, \rr)$. 
\end{lemma}
...
In particular, the collapse along $\O$ is the identity.  Thus, if $(\matB, \vv{v})$ is an $r$-sprout of $(\matA, \vv{u})$, then $\matB = \matA$, and $\vv{v} \in \ushort(\matA, \vv{u}, r)$.  To realize $\vv{v}$,  we fix a special point $\vv{s}$ for $(\matA, \vv{u})$, and write $\vv{v} = \matA \tail{\vv{s}}_r - A \vv{h}$ for some $\vv{h}$ that is optimal for $\ip = \ip(\matA, \vv{u}, \vv{s}, r)$.   
...
\[ \deg(\matA, \vv{v}) = \iprod{\vv{c}}{\vv{v}} = \iprod{\vv{c}}{\matA \tail{\vv{s}}_r} - \iprod{\vv{c}}{\matA \vv{h}} = \norm{\tail{\vv{s}}_r} - \tail{\vv{h}} = \norm{\tail{\vv{s}_r}} - \val \ip.\]
However, by definition, the right-most term above equals $\udeficit(\matA, \vv{u}, r)$. 
\end{proof}
...
\begin{proposition}  Suppose that $\ideala$ is an $\idealm$-primary ideal generated by monomials that are all of the same degree, with respect to some positive $\ZZ$-grading.  If $\matA$ is the monomial matrix associated to these generators, then there exists $\beta = \beta(\matA)$ with the following property:  If $(\matA, \vv{u})$ is very small, and $p>\beta$, then either \[ \fip_p(\matA, \vv{u}) = \deg(\matA, \vv{u}) \] which occurs if and only if $\Q_p = \Q_p(\matA, \vv{u})$ is infinite, or 
\[ \fip_p(\matA, \vv{u}) = \deg(\matA, \vv{u}) - \frac{\lambda_{\Q_p}(\matA, \vv{u})}{\Q_p} \] 
in which case the numerator $\lambda_{\Q_p}(\matA, \vv{u})$ above is positive. 

\daniel[inline]{This still needs some work.  The original statement appears below.}
...
\begin{proof}
   ...
   \todo[inline]{
      What to do when $\crit(\ideala, \ideald_{\vv{v}+\vv{1}}) \le 1$?
