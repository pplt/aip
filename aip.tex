...
\pedro{Is the universal denominator inherited by the descendants? Or do we pick the lcm of universal denominators of all descendants?}\emily{I think we need to fix one for all descendants.}
\end{enumerate}
\end{remark}
...
\begin{proof}
   ...
   %below, \pedro{Maybe place the lemma before this theorem?}
...
\begin{remark}[Elementary observations]
   ...
   \begin{enumerate}
      ...
      \pedro{$A$ and descendants?}\emily{Yes, I think we need this.}
   \end{enumerate}
\end{remark}
...
\begin{proof}
   ...
   In this case, $\S^{\ast}_{\Q p}(A,\vv{u})$ is empty, and so there is an edge $(B,\vv{v}) \to (C,\vv{w})$ in $\sprout_p(A)$ whose initial pair lies in $\S^{\ast}_{\Q}(A,\vv{u})$, and whose terminal pair is not very small. \Cref{non-leading coefficients: L} then tells us that $\delta_{\Q}(A,\vv{u}) = \udeficit(B,\vv{v},p)$ is at least $\ft{C}{\vv{w}}$, which is greater than $1$ as $(C,\vv{w})$ is not very small.
...
\pedro[inline]{
   Maybe we can just add this to the statement of the big theorem?
   Or as a remark immediately after the statement, where we can say that that comes up when studying Bernstein--Sato roots?
}
%\newpage
%
...
\begin{corollary}
Given a monomial matrix $A$, fix a universal denominator $\denom$ for all the descendants of $A$ (e.g., take the product of universal denominators of each of its finitely many descendants).  Then there exists $\beta = \beta(A)$ such that whenever $p>\beta$ and $p \equiv 1 \bmod \denom$, 
\[
...
\emily[inline]{In process of writing the proof.}

\begin{proof}
Let $\vv{s}$ be a special point for a very small $A$-pair $(A,\vv{u})$ with denominator $\denom$, so that $\norm{\vv{s}} = \lambda \leq 1$. 
Then $\vv{s} = \vv{t}/d$ for some $\vv{0} \leq \vv{t} \leq \vv{1} d$.
Since $p \equiv 1 \bmod \denom$, this means that $\tail{s_i}_p$ equals $\lpr{t_i p}{d}/d = t_i/d$ if $s_i \neq 0$, and equals $0$ otherwise.  Hence 
$\tail{\vv{s}}_p = \vv{s}$.

Let $\Theta = \Theta(A, \vv{u}, \vv{s}, q)$.  Then by \Cref{bounded value: P}, $0 \leq \val \Theta < \norm{\vv{s}} = \lambda \leq 1$, and the value of the integer program $\Theta$ must equal $0$.  Hence $\udeficit(A, \vv{u}, p) = \deficit \Theta = \norm{\vv{s}} - \val\Theta = \lambda$.

Let an overbar denote collapse along $\O = \mf(A, \vv{u})$.  
As pointed out in the proof of \Cref{bounded value: P}, $\vv{0}$ is feasible for $\Theta$, and so $\overline{\vv{u}} = \overline{A}\vv{s} = \overline{A}\tail{\vv{s}}_p - \overline{A} \vv{0} \in \ushort(A, \vv{u}, p)$.
Hence $(\overline{A}, \overline{\vv{u}})$ is a $p$-sprout of $(A, \vv{u})$.
Noticing \Cref{comparing deltas: R}, observe that $\udeficit(\overline{A}, \overline{\vv{u}}, p) = \udeficit(A, \vv{u}, p) = \norm{\vv{s}} = \lambda$.



Now take any $p$-sprout $(\overline{A}, \vv{v})$ of $(A, \vv{u})$. 
...
with these statements to \cite[Chapter 3]{hernandez.thesis} for more details\daniel{Is it OK to just cite my thesis?}.
