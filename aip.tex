...
The assumptions on~$\ideala$ and~$\idealb$ ensure that each $\nu(\ideala, \idealb, p^e)$ is a well-defined integer.
...
\label{single multiplier: P}
  Consider a Noetherian domain $\LL$.  Given an ideal $I$ of $\LL[x]$, there exists a single nonzero element $\tau \in \LL$ with which to test inclusion in $\mon(I)$, \ie such that for every monomial $x^{\vv{u}} \in \LL[x]$, $x^{\vv{u}} \in \mon(I)$ if and only if $\tau x^{\vv{u}} \in I$.
\end{proposition}
