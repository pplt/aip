...
\pedro[inline]{
   The following was in its own super-short subsection, at the end of \Cref{s: newton}.
   I moved it here, as it depends on nothing but the definition of degree.
}
...
\label{collapse: D}
Suppose that $\O$ is a \hl{proper}
\pedro{Our faces are proper by definition. I suggest we require the face to be standard---or else the collapse of a monomial matrix need not be monomial.}
face of the Newton polyhedron $\N$ of a monomial matrix $\matA$ with $\numvars$ rows.
...
\begin{proposition}[Faces under collapsing]
   ...
   \begin{enumerate}[$(1)$]
      ...
      \item If $\H$ is a face of $\collapse{\N}$ defined by a point $\vv{h}$, then $\pi^{-1}(\H)$ is a face of $\N$ defined by the unique point $\vv{c}\in\rs(\O)^\perp$ such that $\collapse{\vv{c}} = \vv{h}$.
...
\begin{proof}
   ...
   The point $\collapse{\vv{g}}$ is obtained by deleting zero coordinates from~$\vv{g}$, so $\iprod{\vv{g}}{\vv{u}} = \iprod{\collapse{\vv{g}}}{\collapse{\vv{u}}}$ for every $\vv{u} \in \RR^\numvars$.
   ...
   The fact that $\G$ is standard if and only if $\collapse{\G}$ is standard follows from our using the same scalar $\alpha$ in the definition of these faces.
   ...
   Now let $\H$, $\vv{h}$, and $\vv{c}$ be as in $(2)$, and suppose $\iprod{\vv{h}}{\vv{v}} \ge \beta$ whenever $\vv{v} \in \collapse{\N}$, with equality if and only if $\vv{v} \in \H$.
   Note that the point $\vv{h} = \collapse{\vv{c}}$ is obtained by deleting zero coordinates from~$\vv{c}$.
   If $\vv{u} \in \N$, then $\collapse{\vv{u}} \in \collapse{\N}$, so
   \begin{equation*}
      \iprod{\vv{c}}{\vv{u}} = \iprod{\vv{h}}{\collapse{\vv{u}}} \ge \beta
   \end{equation*}
...
\pedro[inline]{
   I separated the following from the previous proposition, as it was too buried in a very long statement.
}

Taking $\G = \O$ in \Cref{faces under collapsing: P}(1), we see that $\collapse{\O}$ is a face of $\N$ defined by a positive point, which gives us the following corollary.

\begin{corollary}
   \label{cor: O bar is bounded}
   If $\O$ is a standard face of a Newton polyhedron $\N$, then $\collapse{\O}$ is a bounded standard face of $\collapse{\N}$, where the overbar denotes collapse along $\O$.
   \qed
\end{corollary}
...
\begin{proof}
   ...
   Using the notation of \Cref{faces under collapsing: P}, it is easy to see that the composition $\rho\circ \pi$ deletes the $i$-th coordinate of a point if and only if the $i$-th coordinate of $\vv{c}$ is zero.
   Since $\vv{c}$ is the point defining the standard face $\pi^{-1}(\H) \cap \N$ of $\N$, $\rho\circ \pi$ is the collapsing along that face.
\end{proof}
...
\begin{proof}
   ...
   It is clear that a projection preserves relative interiors, and so $\lambda^{-1}\collapse{\vv{u}}$ must lie in the relative interior of $\collapse{\O}$, a  face of $\collapse{\N}$ by \Cref{cor: O bar is bounded}.
...
We end this section with a couple of technical results concerning the lifting of collapsed monomial pairs.
In their statements, $\matA$ is a monomial matrix, $\O$ is an unbounded standard face of the Newton polyhedron $\N$ of $\matA$, and overbars denote collapsing along~$\O$.
...
%   We will prove that if $\rb(\O) \subseteq \rb(\L)$, then $\O =\L$.
   ...
   Since $\collapse{\L}$ is a face of $\collapse{\N}$ by \Cref{faces under collapsing: P},
%   and $\lambda = \degree(\collapse{\matA},\vv{w})$ by \Cref{collapse of mf and mc: P},
   the minimality of $\collapse{\O} = \mf(\collapse{\matA}, \vv{w})$ implies $\collapse{\O} \subseteq \collapse{\L}$, and invoking \Cref{faces under collapsing: P} again we conclude that $\O \subseteq \L$.
...
\begin{proof}
   ...
   Denoting collapsing along $\O$ by an overbar, $\collapse{\O}$ is a bounded standard face of $\collapse{\N}$ by \Cref{cor: O bar is bounded}, and we construct a finite set $\setW_{\collapse{\O}}$ of positive points in the target lattice of $\collapse{\matA}$ as in the previous paragraph.
