...
\begin{example}
   ...
   Let $\idealm \subseteq \kk[x_1,\ldots,x_\numvars]$ be the maximal ideal generated by the variables, and let $\ideala \subseteq \kk[x_1,\ldots,x_\numvars]$ be an $\idealm$-primary ideal generated by monomials of the same degree $d$ under a positive $\ZZ$-grading.
...
\begin{example}
   ...
   \pedro[inline]{
      ...
      I was thinking of including this as an example after the big theorem.
      I was also planning to include an example that has all of the features missing in this one:
      \begin{itemize}
         \item intermediate powers
         \item levels with more than one pair
         \item it cycles, as opposed to reaching a small but not very small pair
   \end{itemize}
      I have a simple example with all of these features, but I'm looking for a better one.
   }
   % \pedro[inline]{
   %    BTW, two things I have yet to see in examples:
   %    \begin{itemize}
   %       \item Pairs on the same level of $\sprout^*_p(A,\vv{u})$ that have different universal shortfalls.
   %       \item $\udeficit$ deciding anything when computing $\S^*_{p^e}$; it has always been the case that if $F$-thresholds are equal, then so are the $\udeficit$s.
   %    \end{itemize}
   % }
...
\begin{example}
   ...
   \begin{figure}
      \centering
      \includegraphics[width=.4\textwidth]{pictures/sprouting_graph_2.pdf}
      \caption{The sprouting subgraph for \Cref{ex: new example}}
      \label{fig: sprouting graph 2}
   \end{figure}
   \Cref{fig: sprouting graph 2} shows the sprouting subgraph associated to the pair $(A,\vv{u})$, where
   \[
      A = \begin{bmatrix} 10&3&5 \\ 3&9&8 \\ 3&8&5 \end{bmatrix}
      \quad\text{and} \quad
      \vv{u} =\begin{bmatrix} 6 \\ 6 \\ 5 \end{bmatrix}
   \]
   when $p$ is congruent to $5$ modulo a universal denominator for $A$.
   The matrices $B$ and $C$ in the figure are the first and second rows of $A$, respectively.
   Unlike the previous example, here $\sprout^*_p(A,\vv{u})$ is infinite---levels $3$ and $4$ alternate \emph{ad infinitum}.
\end{example}
...
\begin{example}
   ...
   \[
    ...
    \]
    Finally, if $(A,\vv{u})$ is associated to an ideal pair $(\ideala,\ideald)$, then
    \[\crit(\ideala,\ideald) = \frac{4}{17} - \frac{3}{17p^3}\]
    for sufficiently large $p$ congruent to $11$ modulo a universal denominator for~$A$.
    \pedro[inline]{This needs to be checked more carefully; same for next example.}
\end{example}

\begin{example}
   \label{ex: new example.2}
   \Cref{table: new example data} shows the data associated to the pair $(A,\vv{u})$ in \Cref{ex: new example}, whose sprouting subgraph is shown in \Cref{fig: sprouting graph 2}.
   \begin{table}
      \begin{center}
         \begingroup
         \setlength{\tabcolsep}{8pt} % Default value: 6pt
         ...
         \begin{tabular}{cccc}
           ...
           $e$ & $\epsilon_{p^e}(A,\vv{u})$ & $\delta_{p^e}(A,\vv{u})$ & $\val\IP_p(A,\vv{u},p^e)$ \\
           \midrule
           1 & $\frac{169}{178}$ & $\frac{133}{178}$ & $\frac{169}{178}p - \frac{133}{178}$\\
           2 & $\frac{2}{3}$ & $\frac{1}{3}$ & $\frac{169}{178}p^2 - \frac{43}{534}p -\frac13$ \\
           3 & $\frac{1}{3}$ & $\frac{2}{3}$ & $\frac{169}{178}p^3 - \frac{43}{534}p^2 - \frac23$ \\
           4 & $\frac{2}{3}$ & $\frac{1}{3}$ & $\frac{169}{178}p^4 - \frac{43}{534}p^3 - \frac13$ \\
           \bottomrule
         \end{tabular}
         \endgroup
         % The \begingroup ... \endgroup pair ensures the separation
   ...
   \end{table}
   For any $e\ge 2$ we have
   \[
      \val\IP_p(A,\vv{u},p^e) = 
      \begin{cases}
         \frac{169}{178}p^e - \frac{43}{534}p^{e-1} - \frac13 & \text{if $e$ is even}\\
         \frac{169}{178}p^e - \frac{43}{534}p^{e-1} - \frac23 & \text{if $e$ is odd}\\
      \end{cases}
   \]
   and, if $(A,\vv{u})$ is associated to an ideal pair $(\ideala,\ideald)$, then
   \[\crit(\ideala,\ideald) = \frac{169}{178} - \frac{43}{534p}\]
   for any sufficiently large prime $p$ congruent to $5$ modulo a universal denominator for $A$.
   \pedro[inline]{
      I'm looking for an example to replace this one, where $\crit$ has three terms.
   }
\end{example}
...
\pedro[inline]{
   ...
   Observing that the monomials $x^{\vv{k}}$ with  $\vv{k} <\vv{1}q$ form a basis for $R$ over $R^q$, for each $i$ let $a_i^q\in R^q$ be the $x^\vv{w}$-coordinate of $f_i$.
...
\begin{remark}[Reduction to prime characteristic]
   ...
   If $\maxi \in \mspec(A)$, then $I_A(\maxi)$ is the extension of $I_A$ to $R_{A}(\idealm) \coloneqq A/\maxi[x_1, \ldots, x_\numvars]$ under the quotient map $R_A \longrightarrow R_A(\idealm)$.
   As noted in \Cref{maximal ideals: R}, the finite generation of $A$ over $\ZZ$ implies that $R_A(\maxi)$ is a polynomial ring over the finite field $A/\maxi$, and for this reason, we may regard $I_A(\idealm)$ as a prime characteristic model of~$I$.
   Furthermore, as the point $\maxi \in \mspec(A)$ varies, we obtain models $I_A(\idealm)$ of $I$ over a finite field of characteristic $p>0$ for all but finitely many~$p$.
\end{remark}

This process greatly simplifies in the special case that $\kk=\QQ$.  Indeed, every ideal $I$ of $R=\QQ[x_1, \ldots, x_\numvars]$ is generated by a set $G$ of polynomials with integer coefficients.  Given such a generating set, we may then take $A=\ZZ$, and let $I_A$ be the ideal generated by $G$ in $R_A=\ZZ[x_1, \ldots, x_\numvars]$.  If  $\maxi$ is the maximal ideal of $A=\ZZ$ generated by a prime integer $p$, then $I_A(\maxi)$ is the ideal of $R_A(\maxi) = \FF_p[x_1, \ldots, x_\numvars]$  generated by the image of $G$.

It is also easy to talk about reduction to prime characteristic for monomial ideals:
If $\kk$ is a field of characteristic zero, and $I$ is a monomial ideal in $R=\kk[x_1, \ldots, x_\numvars]$ generated by a collection of monomials $G$, then $I_A = GR_A = GA[x_1, \ldots, x_\numvars]$ is a \hl{canonical $A$-model} \pedro{Is this a definition?} for every finitely generated $\ZZ$-algebra $A \subseteq \kk$, and $I_A(\maxi) = GR_A(\maxi)$ for every $\maxi \in \mspec(A)$.  
...
%\begin{corollary}
%\label{noncontainment mod p: C}  Let $\kk$ be a field of characteristic zero, and consider an ideal $I$ of $R=\kk[x_1, \ldots, x_\numvars]$.  If $A$ is a finitely generated $\ZZ$-subalgebra of $\kk$ such that $I_A$ is defined, then there exists a dense open set $\mathcal{U} \subseteq \mspec(A)$ such that $(A/\maxi) \otimes_A I_A \cong I_A(\maxi)$ for every $\maxi \in \mathcal{U}$.  Furthermore, $I$ is a proper ideal of $R$ if and only if $I_A(\maxi)$ is a proper of $R_A(\maxi)$ for every $\maxi \in \mathcal{U}$.
%\end{corollary}
%
%
%\begin{proof}   Suppose that $0 \neq \ell \in A$ satisfies the conclusion of \Cref{noncontainment mod p: C} relative to the ideal $J = I_A$ of $R_A = A[x_1, \ldots, x_\numvars]=A[x]$. %
%
...
Let $\kk$ be a field of characteristic zero, and suppose that $I$ and $J$ are ideals of $R=\kk[x_1, \ldots, x_\numvars]$ with $I \subseteq \sqrt{J}$.  If $A$ is any finitely generated $\ZZ$-subalgebra of $\kk$ that supports models $I_A$ and $J_A$, then there exists a dense open set $\mathcal{U} \subseteq \mspec(A)$ such that $I_A(\maxi) \subseteq \sqrt{J_A(\maxi)}$ for every $\maxi \in \mathcal{U}$.
\end{corollary}

\begin{proof}
   The assumption that $I \subseteq \sqrt{J}$ implies that $I^t \subseteq J$ for some natural number $t$.  Fix such a $t$, and suppose that $0 \neq \ell \in A$ satisfies the conclusion of \Cref{comparing expansions:  L} relative to the ideals $J_A \subseteq I_A^t + J_A$ of $R_A$.    
...
Next, let $\mathcal{U}$ denote the set of all points $\maxi \in \mspec(A)$ such that the characteristic of $A/\idealm$ does not divide $\ell$.  Note that $\mathcal{U}$ is an open dense (i.e., open and nonempty) subset of $\mspec(A)$.  If $\maxi \in \mathcal{U}$, then $A/\maxi$ is an $A[\ell^{-1}]$-algebra, and setting $B=A/\maxi$ in \eqref{step in radical containment: e} tells us that
%
...
Let $\kk$ be a field of characteristic zero, and suppose that $I$ is a monomial ideal of $R=\kk[x_1, \ldots, x_\numvars]$.  If $A$ is any finitely generated $\ZZ$-subalgebra of $\kk$ that supports a model $I_A$, then there exists a dense open set $\mathcal{U} \subseteq \mspec(A)$ such that $I_A(\maxi)$ is a monomial ideal for every $\maxi \in \mathcal{U}$.
\end{corollary}
...
\begin{proof}
   ...
   Suppose that $0 \neq \ell \in A$ is such that the conclusions of \Cref{comparing expansions:  L} hold for the containment of ideals $I_A \subseteq L$ and $GR_A \subseteq L$.  By definition,  $I_A \kk[x] = I_A R = I$ and $(GR_A)\kk[x] = G \kk[x] = I$, and so $L\kk[x] = I$ as well.
   In this case, \Cref{comparing expansions:  L} implies that if $B$ is an arbitrary $A[\ell^{-1}]$-algebra, then
   \[
      I_A B[x] = L_A B[x] = G B[x].
   \]
   Thus, if $\mathcal{U} = \{ \maxi \in \mspec(A) : \operatorname{char}(A/\maxi) \nmid \ell \}$,  then for each $\maxi \in \mathcal{U}$, we may set $B=A/\maxi$ above to see that $I_A(\maxi) = G R_A(\maxi)$. 
\end{proof}
...
\begin{lemma}
 ...
 \begin{proof}
    ...
    Furthermore, as in the proof of \Cref{comparing expansions:  L}, the isomorphism $B \otimes_A A[x] \cong B[x_1, \ldots, x_\numvars]=B[x]$ induces an isomorphism between the above exact sequence and the exact sequence
    \[
...
%  Suppose that $f,g_1, \ldots, g_s$ are elements of a ring $T$, and that $I$ is an ideal of $R$.
...
%
%
%Let $\mathcal{V}$ be the set consisting of all points of the form $\vv{v} = \vv{a} + \sum_{s <i \leq \numvars} t_i \canvec_i$ where $\vv{a} \in \mathcal{A}$, and each integer $t_i$ satisfies $0 \leq t_i \leq \ell$.  For every such $\vv{v}$, set $\mathcal{W}(\vv{v}) = \emptyset$ if some $t_i$ is not $\ell$, and set $\mathcal{W}(\vv{v}) = \{ \canvec_i : s < i \leq \numvars \}$ otherwise.
%
%After unraveling these definitions, it is obvious that \eqref{stab-1: e} is satisfied.  Furthermore, the only case in which \eqref{stab-2: e} is nontrivial is when $\vv{v} = \vv{a} + \sum_{s <i \leq \numvars} \ell \canvec_i$ for some $\vv{a} \in \mathcal{A}$,  in which case we must prove that
%%
...
\begin{lemma}
   ...
   \[ \NN^m = \bigcup_{\vv{v}\in \mathcal{V}} (\vv{v} + \NN \mathcal{W}(\vv{v})) \]
...
\pedro{Do we need to be this precise? Can't we just take a single $N\gg 0$ so that all chains $\{(J:x_i^n)\}_n$ are stable for $n\ge N$?}
   ...
   Then for all integers $\ell \geq 0$, $((J : x_i^{u_i}) : x_i^{\ell} ) = ((J : x_i^{u_i+1}) : x_i^{\ell} )$, i.e., 
   \begin{equation*}
...
\[ \mon(J) B[x] = \mon(JB[x]).\]
 ...
 \begin{proof}
    Let $\mathcal{V}$ and $\mathcal{W}(\vv{v})$ be the finite subsets of $\NN^m$ described in \Cref{monomial-noetherian-decomposition: L}, relative to the ideal $J$ of $A[x]$, so that $\NN^m = \bigcup_{\vv{v} \in \mathcal{V}} (\vv{v}+\NN \mathcal{W}(\vv{v}))$, and
    \[
...
% \begin{lemma}
%    \label{colon-product-stabilization: L}
%    Suppose that $f,g_1, \ldots, g_s$ are elements of a ring $R$, and that $I$ is an ideal of $R$.
