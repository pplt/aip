...
\begin{remark}[An algorithm]
   ...
   Somewhat surprisingly, a preliminary implementation in \emph{Macaulay2} by the authors fares well on small examples.
\end{remark}
...
\[ I_A B[x] = L_A B[x] = G B[x] \] Thus, if $\mathcal{U} = \{ \maxi \in \mspec(A) : \operatorname{char}(A/\maxi) \nmid \ell \}$,  the for each $\maxi \in \mathcal{U}$, we may set $B=A/\maxi$ above to see that $I_A(\maxi) = G R_A(\maxi)$. 
\end{proof}
...
\[ \NN^m = \bigcup{\vv{v} \in \mathcal{V}} (\vv{v} + \NN \mathcal{W}(\vv{v})) \] 
