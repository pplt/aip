...
\begin{proof}
   ...
   Before proceeding, recall that $\vv{t}^{\ast}$ is a vertex of $\Q$ if and only if the only expression of $\vv{t}^{\ast}$ as a convex combination of points $\vv{r}$ and $\vv{s}$ in $\Q$ is when $\vv{r}=\vv{s}=\vv{t}^{\ast}$.
