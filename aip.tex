...
They ask for conditions on $\ideala$ under which this special behavior, captured in the definition below, manifests for every $\idealb$ and $e$ \cite[Problem~3.8]{mustata+takagi+watanabe.F-thresholds}.
...
% \pedro{This may be omitted, so we can get to the point quickly}
% Now, if $\idealb$ is an ideal of $\kk[x_1, \ldots, x_\numvars]$ with $\ideala \subseteq \sqrt{\idealb}$, then in analogy with \eqref{eq: nu definition}, given a nonnegative integer $e$, we define
...
The main result of this article is the proof that every nonzero proper monomial ideal is \muCool.
Along the way we recover the aforementioned theorem of Budur, \mustata, and Saito, interpreting the coefficients in \eqref{eq: nu-uniformity for monomial ideals} in the context of certain optimization problems, whose study constitute the core of this work. 
...
\begin{example}
   ...
   \[
    ...
    %congruent to $11$ modulo $2170560$.
\end{example}
...
\begin{example}
   ...
   %congruent to $5$ modulo $464409919955520$.
\end{example}

To convey the significance of our main result, we shall trace back some of our motivations to introduce and study generalized Frobenius powers.

\todo[inline]{
   From here, discuss multiplier ideals, test ideals; motivate Frobenius powers via test ideals of general hypersurfaces; discuss F-jumping exponents/F-thresholds and critical exponents; state consequences for generic hypersurfaces.
}
