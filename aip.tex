...
\daniel[inline]{I've always thought of $\rs(\O)^{\perp}$ as $\operatorname{span}_{\RR} \rb(\O)^{\perp}$, and not an abstract $\RR^{\# \rb(\O)^{\perp}}$, and so unless I am crazy, I think that $\collapse{A \vv{k}} = \collapse{A} \vv{k}$ with the original definition.  Both sides are in $\RR \rb(\O)^{\perp}$.  Maybe it is better to keep the notation $\RR \rb(\O)$ instead of $\rs(\O)$, and similar with $\perp$ analogs.

\medskip

Are our different viewpoints an internal/external direct sum conflict?  

\medskip

Keeping a row of zeros in the collapse is odd to me geometrically;  even when we are drawing collapsed Newton polyhedra, we aren't drawing them in the larger Euclidean space.  Similarly, when we collapse a monomial ideal (which basically, involves forgetting some variables), we are thinking about it as a monomial ideal in a polynomial ring in fewer variables, all of which appear as generators for the collapsed ideal.}
...
\daniel[inline]{I think it is a valid point to note that maybe we aren't so consistent.  When I say something is positive in a coordinate subspace, I mean its projection to that space is positive (intrinsically, in that space).

\medskip

Let $V$ be an $n$-dimensional vector space over $\RR$.  If we fix a basis for $V$, then we can make sense of inequalities like $\vv{u} \leq \vv{v}$ for elements $\vv{u}, \vv{v} \in V$.  Just look at the representations in the basis, and ask for the inequality coefficient by coefficient.  Then positive, nonnegative, etc makes sense.  This all depends on the basis, but once we fix our starting point $\RR^d$, then every other vector space we get inherits a natural basis consisting of some subset of the standard basis for $\RR^d$.  I've always interpreted all inequalities in this way.}
...
\pedro{I think this is true regardless. Since $\vv{a} \in \rs(\O)^\perp$, it is fixed by the projection $\RR^n \to \rs(\O)^\perp$.}
\end{remark}
