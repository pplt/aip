...
\While{\texttt{\em true}}{
   ...
   $\text{\texttt{aux}} \leftarrow \{(A,\vv{v})\in \S_{p^e}: \ft{A}{\vv{v}} = \text{\texttt{max}}\}$\;
   $\text{\texttt{min}} \leftarrow \min\{\udeficit(A,\vv{v},p_0): (A,\vv{v})\in \text{\texttt{aux}}\}$\;
   $\S^*_{p^e} \leftarrow \{(A,\vv{v})\in \text{\texttt{aux}}: \udeficit(A,\vv{v},p_0) = \text{\texttt{min}}\}$\;
...
As in the proof of \Cref{noncontainment mod p: L}, $B \otimes_A R_A \cong B[x_1, \ldots, x_m]=B[x]$, and the above sequence is isomorphic to the exact sequence
