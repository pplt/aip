...
\item[$\Box$] Put in examples.
 ...
 \pedro[inline]{I started changing to $\IP(A,\vv{u},q)$. Are we going to abbreviate $\IP(A,\vv{u},1)$ by $\IP(A,\vv{u})$? Or insist on always having a $q$, so, for example, the statement of Lemma~7.3 would become ``[\ldots] the quotient when dividing any optimal point of $\IP_p(A, \vv{u}, qp^e)$ by $p^e$ must be optimal for $\IP_p(A, \vv{u}, q)$''?
    ...
    \pedro[inline]{Sounds good. Will complete the changes soon.}
 }
...
\pedro[inline]{
   ...
   We can gather the results on collapsing at the end of Section~4 or, better, have a whole section on collapsing.
}
...
\emily[inline]{This sounds good to me.}
\pedro[inline]{Will work on it soon.}
}
...
lies in the boundary of the Newton polyhedron of $A$.  
\end{definition}
...
\begin{definition}
   ...
   \emily{There might be some confusion that this is a set to the $n^\text{th}$ power?  What about $\sierp_{p,n}$?}
...
\begin{definition}
Given a monomial pair $(A, \vv{u})$, the optimization problem $\fip_p(A,\vv{u})$ consists of maximizing $\vv{t}\mapsto \norm{\vv{t}}$, subject to the constraints $\vv{t} \geq \vv{0}$, $A\vv{t} < \vv{u}$, and $\vv{t}\in \sierp_p$. 
We define the feasible set for $\fip_p(A,\vv{u})$, $\feas \fip_p(A,\vv{u})$, to be the \emph{closure} of the set of all $\vv{t}\in \sierp_p$ satisfying $A\vv{t} < \vv{u}$.
% The value of the problem, $\val \fip_p(A,\vv{u})$, is defined as the supremum of $\norm{\vv{t}}$ among all $\vv{t} \in \feas \fip_p(A, \vv{u})$.  
\end{definition}

\begin{remark}
The points in $\vv{t}\in \sierp_p$ satisfying $A\vv{t} < \vv{u}$ are in the intersection of 
the feasible set of the linear program $\LP(A, \vv{u})$ with the Sierpi\'nski $p$-gasket.  
Since this intersection is a compact set, $\feas\fip_p(A,\vv{u})$ is also compact, and the value of $\Sigma_p(A, \vv{u})$ is finite.
\end{remark}
...
\emily[inline]{This is a good idea.}
