...
%\begin{remark} Lemma \ref{face: L}\eqref{rb_independence: e} implies that the recession basis of $\O$ is independent of the choice of the defining vector $\vv{a}$, while Lemma \ref{face: L}\eqref{rb_bounded: e} tells us that $\O$ is bounded if and only if $\rb(\O) = \emptyset$.    
%\end{remark}
...
%\subsection{A critical secondary linear program}  We continue to adopt the notation in Setup \ref{collapse: S}.  Below, we define and study a secondary integer program that will play a key role in our solution of $\IP(A, \vv{u}q)$ for $q \gg 0$.
%
%
...
%\begin{lemma}
%\label{key step: L}
%Adopt the context of Setup \ref{projection + IP: S}.  If $q$ is a positive integer, then every feasible point for the program  $\IP(\overline{A}, \ol{\vv{u}}q)$ in $\RR A(\O)$  is of the form \[ \vv{s}q - \tail{\vv{s}}_q + \vv{h} \] where $\vv{h}$ is feasible for the  program $\IP(\overline{A}, \overline{A}  \tail{\vv{s}}_q, I )$ in $\ZZ A(\O)$.
%\end{lemma}
%
%\begin{proof} 
%Lemma \ref{projection of a min expression: L} tells us that $\overline{A} \vv{s} = \ol{\vv{u}}$, and so $\vv{g} \in \ZZ^n$ satisfies the constraint $\overline{A} \vv{g} < \ol{\vv{u}}q = \overline{A} \vv{s} q$ if and only if $\vv{h} = \vv{g} - \vv{s}q + \tail{\vv{s}}_q$ satisfies the constraint $\overline{A} \vv{h} <  \overline{A} \tail{\vv{s}}_q$.  It then follows from this that if $\vv{g} \in \ZZ^n$ is feasible for $\IP(\overline{A}, \ol{\vv{u}}q)$, then $\vv{h} = \vv{g} - \vv{s}q + \tail{\vv{s}}_q$ is feasible for $\IP(\overline{A}, \overline{A}  \tail{\vv{s}}_q \, , I )$.
%\end{proof}
...
%
%\begin{proof}
%By construction, the $i$-th coordinate of $\vv{s}$ is nonzero if and only the $i$-th coordinate of $\tail{\vv{s}}_q$ is nonzero, while Lemma \ref{tail projection: L} tells us that $B \tail{\vv{s}}_q$ is positive in $\RR \rb(\O)^{\perp}$.  Consequently,  we may apply Proposition \ref{bounded value and image: P} relative to the matrix $B$ and point $\vv{t} = \tail{\vv{s}}_q$ to justify our assertions.
%\end{proof}
...
%\begin{remark} Above, we referred to Corollary \ref{uniform value and image: C} to deduce the independence of $\delta(A, \vv{u}, q)$ and $\Delta(A, \vv{u}, q)$ on the rational point $\vv{s} \in \mc_{\QQ}(A, \vv{u})$.  Though it seems likely that this can be established with a more direct argument, we have yet to identify one. 
%\end{remark}
...
%\begin{remark}
%Suppose $(A, \vv{u}, \vv{s}, q)$ is a list.  If $(B, \ol{\vv{u}})$ is the collapse of $(A ,\vv{u})$ along $\O = \mf(A, \vv{u})$, then Corollary \ref{collapse of mf and mc: C} implies that 
%\[ (B, \ol{\vv{u}}, \vv{s}, q) \] is also a monomial list.  Furthermore, it follows from Definition \ref{aux program: D} that 
%\[ \ip(A, \vv{u}, \vv{s}, q) = \ip(B, \ol{\vv{u}}, \vv{s}, q). \] 
%\end{remark}
%
%\begin{lemma}
...
%\end{proposition}
%
%\begin{proof}   Suppose $A$ is $d \times n$, and set $\ip =  \ip(A, \vv{u}, \vv{s}, q)$.  The positivity statement in Lemma \ref{tail projection: L} immediately implies that $\vv{0}$ in $\ZZ^n$ must be feasible for $\ip$.  In particular,  the value of $\ip$ is nonnegative.
%
...
%%
%and combining this with Lemma \ref{tail projection: L}, we see that there are also only finitely many possibilities for the point $B \tail{\vv{s}}_q$.
% \end{proof}
%
%The finiteness result Proposition \ref{bounded value and image: P} facillitates the following definition.
%
%\begin{definition}
%\label{T: D}
%
...
%
%\begin{remark}
%We stress that there is typically no canonical choice of the sets defined in Definition \ref{T: D} above.  Nevertheless, as it our arguments will depend on the existence of some choice of $T$, we fix one once and for all.
%\end{remark}
%
%\begin{remark}
%\label{finiteness of optimal reps: R}
%The finiteness result Theorem \ref{finitely many equivalence classes: T} implies that \[ \cup  \ T( A, \vv{s}, \vv{u}, q ) \] where the union is over all $A$-lists, is a finite for every monomial matrix $A$, 
%\end{remark}
%
%
%\newpage
...
%
%
%\begin{definition} With notation as in Lemma \ref{face: L}, the \emph{recession basis} of $\O$ is the set $\rb(\O)$ of all standard basis vectors $\vv{e}_i$ in $\RR^d$ with $\iprod{\vv{a}}{\vv{e}_i} = 0$.
%\end{definition}
%
%\begin{remark} Lemma \ref{face: L}\eqref{rb_independence: e} implies that the recession basis of $\O$ is independent of the choice of the defining vector $\vv{a}$, while Lemma \ref{face: L}\eqref{rb_bounded: e} tells us that $\O$ is bounded if and only if $\rb(\O) = \emptyset$.    
%\end{remark}
...
%\end{proof}
%
%Technicalities that arise in future sections whenever $\mf(A, \vv{u})$ is unbounded force us to consider a certain distinguished subset of optimal points, in which we require a strengthening of condition \eqref{mc decomposition: e} in Corollary \ref{opt set description: C}.
%
%
%\begin{definition}
%\label{mc: D} 
...
%\end{theorem}
%
%The main technical result we'll need to establish Theorem \ref{uniformity: T} is Lemma \ref{vertex: L} below.  Though variations of this lemma are well-known, we include the straightforward proof for lack of an appropriate reference.
%
%
%\begin{lemma}  
%\label{vertex: L}
...
%In this subsection, we adopt the notation established in Definition \ref{collapse: D} above.  Furthermore,  $\ol{X}$ denotes the collapse of a subset $X$ of $\RR^d$ along $\O$, and $B$ denotes the collapse of $A$ along $\O$.  
%\end{setup}
%
%\begin{remark}
%\label{collapse of a defining vector: R}
...
%\begin{corollary}
%\label{collapse of mf and mc: C}
%In the context of Setup \ref{collapse_ss: SU}, the following hold.
%\begin{enumerate}
%\item $\mf(B, \ol{\vv{u}}) = \ol{\O}$ and $\ft{A}{\vv{u}} = \ft{B}{\ol{\vv{u}}}$.
%\item Each optimal point for $\LP(A, \vv{u})$ is also optimal for $\LP(B, \ol{\vv{u}})$.  
%\item Each minimal coordinate of $(A, \vv{u})$ is a minimal coordinate of $(B, \ol{\vv{u}})$.
...
%
%Next, note that Proposition \ref{FT descriptions: P} and the above tells us that \[ \val \LP(A, \vv{u}) = \ft{A}{\vv{u}} = \ft{B}{\ol{\vv{u}}} = \val \LP(B, \ol{\vv{u}}). \] 
%As discussed in Remark \ref{collapsed programs: R}, the constraints of $\LP(B, \ol{\vv{u}})$ are a subset of those of $\LP(A, \vv{u})$, and so it follows that any optimal point for $\LP(A, \vv{u})$ must be optimal for $\LP(B, \ol{\vv{u}})$.  The boundedness of $\ol{\O}$, Proposition \ref{opt versus mc: P}, and the preceding observation then allows us to conclude that
%\[ \mc(A, \vv{u}) \subseteq \opt \LP(A, \vv{u}) \subseteq \opt \LP(B, \ol{\vv{u}}) = \mc(B, \ol{\vv{u}}).\] 
%\end{proof}
