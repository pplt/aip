...
\begin{setup}  Let $\ideala$ and $A$ be as above, and let
\[ \ideald := \diag(\vv{u}) := \langle x_1^{u_1}, \ldots, x_d^{u_d} \rangle \subseteq \kk[x_1, \cdots, x_d]\] be the diagonal ideal  associated to a point $\vv{u} \in \NN^d$ with positive coordinates.
\end{setup}

For most of what remains of this article, we are concerned with describing the numerical invariants $\nu(\ideala, \ideald, p^e)$ and $\ft{\ideala}{\ideald}$ when $p^e$ is large, and the numerical invariants $\mu(\ideala, \idealb, p^e)$ and $\crit(\ideala, \idealb)$ when $p$ is large and $e$ is arbitrary.  Though it is not immediately obvious, we explain in \Cref{monomial-reduction: A} how understanding these numbers allows one to completely describe $\nu(\ideala, \ideald, p^e)$ and $\ft{\ideala}{\ideald}$

tells us that we may also assume that $\idealb$ is monomial as well.  In fact, \hl{add the result} allows us to further assume that $\idealb$ is a monomial ideal generated by positive powers of each of the ambient variables.

Given this reduction, we will work in the following context.


\daniel[inline]{This section is in progress...}
...
\pedro[inline]{If and when we include an analogue to \Cref{cor: every crit is crit wrt diagonal ideal} for $F$-thresholds, we should mention that here, to justify the fact that we're only looking at $F$-thresholds with respect to diagonal ideals.}
...
\begin{definition}
   A \emph{monomial pair} $(A, \vv{u})$ consists of a monomial matrix $A$ and a positive point $\vv{u}$ in the target lattice of $A$.
\end{definition}

\emily{Removed integer programming in preliminaries, and added this.}
...
\emily{Paragraph also added to eliminate preliminaries on integer programming.}
