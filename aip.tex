...
\begin{definition}[Fundamental sequences]
	...
	Consider a monomial matrix $\matA$, and a universal denominator $d$ for $\matA$, and each of its finitely many descendants.  The \emph{fundamental sequences} associated to a unit $r \in \ZZ/d\ZZ$ and pair $(\matA, \vv{u})$ are the sequences $\{ \lambda_{r^s}(\matA, \vv{u})\}_{s=1}^{\infty}$  and $\{ \eta_{r^s}(\matA, \vv{u}) \}_{s=1}^{\infty}$ described in \Cref{main theorem wrt diagonal: T}, i.e., 
	%
\begin{align*}
...
\end{proposition}
\daniel[inline]{Update this statement and proof}
