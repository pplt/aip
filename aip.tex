...
\title[Arithmetic programming and Frobenius powers]{Arithmetic integer programming
 and Frobenius powers of monomial ideals
}
...
\begin{enumerate}
 ...
 \pedro[inline]{I'm trying $\graph$ for the notation, for ``sprouting graph''}
...
\begin{proof}  \Cref{tail-basics: R} tells us that if $\vv{t} := \vv{s} - q^{-1} \cdot {\tail{\vv{s}}_q}$, then $\vv{t}q = \vv{s}q -\tail{\vv{s}}_q$ has nonnegative integer coordinates.  In fact, \Cref{tail-basics: R} also tells us that we may take $\vv{t}$ in \Cref{less  than u: L} to deduce that $A (\vv{s} q - \tail{\vv{s}}_q ) =  A\vv{t}q <  \vv{u}q$.  
\end{proof}
...
\label{comparison: R}
Adopt the context of \Cref{canonical-feasible: T}, and fix an arbitrary point $\vv{k} \in \feas \IP(A, \vv{u}, q)$.  Our goal here is to describe some natural constraints on the difference 
\[ \vv{h} =  \vv{k} - \vv{s}q + \tail{\vv{s}}_q \]   
between $\vv{k}$, and the feasible point $\vv{s}q-\tail{\vv{s}}_q$ described in \Cref{canonical-feasible: T}.


Towards this, let $\ol{\mathcal{X}}$ denote the collapse of $\mathcal{X}$ along the face $\O = \mf(A, \vv{u})$.  Our choice $\vv{k}$ guarantees that $A \vv{k} < \vv{u}q = A \vv{s}q + \vv{w}q$, where $\vv{w} \in \RR \rb(\O)$ is as in \Cref{mc: D}, and inequality is preserved by collapses, it follows that
%
\[ \ol{A ( \vv{s}q-\tail{\vv{s}}_q + \vv{h})} = \ol{A \vv{k}}  < \ol{\vv{u}}q = \ol{A \vv{s}q + \vv{w}q} = \ol{A \vv{s}q} \] 
%
from which it follows that $\ol{A \vv{h}} < \ol{A \tail{\vv{s}}_q}$. Furthermore, our choice of $\vv{k}$ also guarantees that $\vv{k} \geq 0$, and so $\vv{h} \geq - \vv{s}q + \tail{\vv{s}}_q$.  In particular, the $i$-th coordinate of $\vv{h}$ is nonnegative whenever the $i$-th coordinate of $\vv{s}$ is zero.
\end{remark}
...
\item The constraints are that the $i$-th coordinate of $\vv{h}$ is nonnegative whenever the $i$-th coordinate of $\vv{s}$ is zero, and that \[ \ol{A \vv{h}}  < \ol{A \tail{\vv{s}}_q}\]
\daniel{We should remind the reader that the collapsed inequalities are a subset of the ones determined by $A$} where $\ol{\mathcal{X}}$ is the collapse of $\mathcal{X}$ along the face $\O = \mf(A, \vv{u})$.\end{enumerate}
\end{definition}


\begin{definition}
The \emph{image} of $\ip(A, \vv{u}, \vv{s}, q)$ is the set \[ \im \ip(A, \vv{u}, \vv{s}, q)  =  \ol{A (\opt \ip(A, \vv{u}, \vv{s}, q))}\] 
where $\ol{\mathcal{X}}$ is the collapse of $\mathcal{X}$ along the face $\O = \mf(A, \vv{u})$.
\end{definition}
...
Let $\ol{\mathcal{X}}$ denote the collapse of $\mathcal{X}$ along $\O = \mf(A, \vv{u})$, and write  \[ \vv{u} = A \vv{s} + \vv{w} \] for some $\vv{w}$ that is positive in $\RR \rb(\O)$, as in \Cref{mc: D}.  As $\vv{u}$ has integer coordinates, it follows that $\delta$ is also a denominator for $\vv{w}$.  
...
After projecting to $\RR \rb(\O)^{\perp}$, the bound \eqref{equivalent ineq: e} becomes $\ol{A \vv{h}} < \ol{A \tail{\vv{s}}}_q$, which holds by the feasibility of $\vv{h}$ for $\ip$.  If $\O$ is unbounded, then the projection of the right-hand side of \eqref{equivalent ineq: e} to $\RR \rb(\O)$ is at least $\vv{w}q$.  However, as $\delta$ is a denominator for $\vv{w}$, every coordinate of $\vv{w}q$ is at least $q/\delta$,  and our choice of $q \gg 0$ then guarantees that \eqref{equivalent ineq: e} holds after projecting to $\RR \rb(\O)$.  We conclude that \eqref{equivalent ineq: e} holds throughout $\RR^d = \RR \rb(\O) \oplus \rb(\O)^{\perp}$.
...
\begin{theorem}[Existence of finite sets of representatives]  Given a monomial matrix $A$, there exists a finite subset $\fsr(A)$ of the domain lattice of $A$ with the following property:  For every list $(A, \vv{u}, \vv{s}, q)$, and for every point $\vv{v} \in \im \ip(A, \vv{u}, \vv{s}, q)$, there exists a point $\vv{h} \in \fsr(A)$ with $\vv{h} \in \opt \Theta(A, \vv{s}, \vv{u}, q)$ and $\overline{ A \vv{h}} = \vv{v}$, where $\ol{\mathcal{X}}$ denotes the collapse of $\mathcal{X}$ along $\O = \mf(A, \vv{u})$.
\end{theorem}
...
\emily[inline]{It seems like we prove that $\mu(A, \vv{u}, q) = \mu(\overline{A}, \overline{\vv{u}}, q)$.  Let's make sure to state that later.
}
...
\begin{definition}
 Define
\begin{enumerate}
\item $\graph^1(A) = \{ p\text{-sprout of } (A, \vv{u}) : \vv{u} \in \NN_+^d \}$. \daniel{I think $\graph^1(A)$ should consist of all small $A$-pairs.  }
\item $\graph^{e+1}(A) = \{ p\text{-sprout of } (B, \vv{v}) : (B, \vv{v}) \in \graph^e  \}$ for $e \geq 1$. 
\end{enumerate}
\end{definition}
...
Once $A$ is fixed,
\begin{itemize}
 \item $\bigcup_{e=1}^\infty \graph^e(A)$ is finite.
 \item There exist $D$ such that for all $e \geq 1$ and $(B, \vv{v}) \in \graph^e(A)$, there exists $\vv{s} \in \mc(B, \vv{v})$ with denominator $D$. 
...
\begin{theorem}[Iterated lifting]
 There exists $\beta = \beta(A)$ with the following property:  If $p>\beta$ and $(A_1, \vv{u}_1) \to (A_2, \vv{u}_2) \to \cdots \to (A_e, \vv{u}_e)$ is a path in $\graph(A)$, then for all $1 \leq i \leq e$, there exists $\vv{k}_i \in \opt \Pi(A_i, \vv{u}_i,p)$  for which 
 \[
...
\daniel[inline]{Don't we just need this with $\mu$'s instead?  If so, should we combine this with the result that follows?}


\begin{proposition}
 There exists $\beta = \beta(A)$ with the following property:  If $p > \beta$ and $e \geq 1$ and
 \[
  (A_1, \vv{u}_1) \to (A_2, \vv{u}_2) \to \cdots \to (A_e, \vv{u}_e)
 \]
 is a sequence of $p$-sprouts such that $(A, \vv{u}_e)$ is \emph{medium small} and  $(A_i, \vv{u}_i)$ very small for $1 \leq i \leq e-1$, then  
...
\begin{definition}
Suppose that $(A, \vv{u})$ is very small.  For $e \geq 0$, we define the set $\widehat{\graph}^e(A)$ inductively as follows:
\begin{enumerate}
\item $\widehat{\graph}^1(A, \vv{u}, p)$ consists of the single monomial pair $(A, \vv{u})$.
\item Suppose that $\widehat{\graph}^e(A, \vv{u}, p)$ has been defined for some integer $e \geq 1$, and let $S$ be the set of all $p$-sprouts of all monomial pairs in $\widehat{\graph}^e(A, \vv{u}, p)$.
...
\begin{proposition}
Given a monomial matrix $A$, there exists an integer $D$ such that $\widehat{\graph}_p(A, \vv{u}) = \widehat{\graph}_q(A, \vv{u})$ for every pair $(A, \vv{u})$ whenever $p \equiv q \bmod D$.
\end{proposition}
...
\emily[inline]{It's important for us to go back and fix/finish the proof. }


\begin{theorem}\label{thm: higher mu formula for paths}
\daniel{Can we fix a $\beta= \beta(A)$ and take $p \geq \beta$ as in other results?}
If $(A, \vv{u})$ is a very small monomial pair and $p \gg 0$, then the following holds\textup:  If 
$(A_1, \vv{u}_1) \to \cdots \to (A_e, \vv{u}_e)$ is a path in $\widehat{\graph}(A, \vv{u}, p)$, and $(B, \vv{v})$ is any vertex in this graph whose level is the same as that of the initial point $(A_1, \vv{u}_1)$, then $\mu(B, \vv{v}, p^e)$ equals \daniel{The proof will show $\leq$.  To get equality, just make sure to prove iterated lifting lemma before this.}
%
...
\begin{proof}  
\todo{Maybe write this algebraically, when possible?}
...
\begin{corollary}\label{cor: constant mus}
   If $(A,\vv{u})$ is a small monomial pair and  $\beta = \beta(A)$ is as in \Cref{thm: higher mu formula for paths}, then $\crit$ and $\mu$ are constant on each level of $\widehat{\graph}(A,\vv{u},p)$ for every $p\ge \beta$. 
\end{corollary}
\todo[inline]{Restate more precisely}
% \begin{corollary}
%    If $(A,\vv{u})$ is a small monomial pair, then there exists $\beta = \beta(A)$ such that for all $p \ge \beta$ the following holds\textup:
%    If $(A_1, \vv{u}_1) \to \cdots \to (A_e, \vv{u}_e)$ is a path in $\widehat{\graph}(A,\vv{u},p)$, and $(B, \vv{v})$ is any vertex of $\widehat{\graph}(A, \vv{u},p)$ whose level is the same as that of $(A_1, \vv{u}_1)$, then
%    \begin{equation*}
%      \mu(B,\vv{v},qp^e)
%          \le \mu(A_1,\vv{u}_1,p)qp^{e-1} + \cdots + \mu(A_{e-1},\vv{u}_{e-1},p)qp + \mu(A_e,\vv{u}_e,qp)
%    \end{equation*}
%    for every $q$.
% \end{corollary}
% \begin{proof}
%    This follows by induction on $e$, arguing exactly as in \Cref{thm: higher mu formula for paths}, and noticing that \Cref{cor: constant mus} gives us the case $e=1$.
% \end{proof}
% 
% \details[inline]{
% Choose $\beta$ so that the conclusions of \Cref{arithmetic uniform value and image: T,trivial max value: T,cor: upper bound for higher mus} (and maybe more\ldots) hold for each one of the finitely many pairs in $\widehat{\graph}(A, \vv{u},p)$ whenever $p > \beta$.
%     In this case, the assumption that $(A_1, \vv{u}_1)$ and $(B, \vv{v})$ lie in the same level implies that $\mu(B,\vv{v},qp) = \mu(A_1, \vv{u}_1, qp)$, proving the result for $e=1$.
% 
%     Suppose that $e \geq 2$, and the result holds for paths of length $e-2$.
%     \Cref{cor: upper bound for higher mus} tells us that
%     \begin{align*}
%       \mu(B,\vv{v},qp^e) &\le \mu(B,\vv{v},p) qp^{e-1} + \max_{\tiny (C,\vv{z})\leftarrow (B,\vv{v})} \ \mu(C,\vv{z},qp^{e-1}) \\
%       &= \mu(A_1,\vv{u}_1,p) qp^{e-1} + \max_{\tiny (C,\vv{z})\leftarrow (B,\vv{v})} \ \mu(C,\vv{z},qp^{e-1}).
%     \end{align*}
%     Therefore, to complete our inductive step, it suffices to show that
%     \[\mu(C,\vv{z},qp^{e-1}) \le \mu(A_2,\vv{u}_2,qp^{e-1}) \]
%     for each $(C,\vv{z})$ sprouting from $(B,\vv{v})$, and the induction hypothesis does the rest. 
%     Towards this, first note that if $(C, \vv{z})$ does not lie in $\widehat{\graph}(A,\vv{u},p)$, then \Cref{cor: mu comparison} implies that $\mu(C, \vv{z},qp^{e-1}) < \mu(A_2,\vv{u}_2, qp^{e-1})$.
%     In light of this, \Cref{cor: mu comparison} then tells us that \ldots
% % and our induction hypothesis applied to the path 
% % \[ (A_2, \vv{u}_2) \to \cdots \to (A_e, \vv{u}_e) \] and the initial point $(A_2, \vv{u}_2)$ itself then tells us that the value of the program $\IP_p(A_2, \vv{u}_2 \cdot p^e)$ is at most the quantity in \eqref{target bound: e}.  
% 
% % On the other hand, if $(C, \vv{z})$ is a vertex of $\graph_p(A, \vv{u})$ of level equal to that of $(A_2, \vv{u}_2)$ for some $\vv{z} \in \Delta_p(B, \vv{v})$, then our induction hypothesis applied to the path 
