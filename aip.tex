...
\Input{ \\
   ...
   $\bullet$ a representative $p$ for a congruence class modulo a universal denominator $\denom$
}
\medskip
...
$M_1 \leftarrow \ft{A}{\vv{u}}\pp - \udeficit(A,\vv{u},p)$\;
\medskip
\While{\texttt{\em true}}{
   $e\leftarrow e+1$\;
   $S_{p^e} \leftarrow \big\{(A,\vv{v}): \vv{v} \in \bigcup_{(A,\vv{v}^*) \in \S^*_{p^{e-1}}} \ushort(A,\vv{v}^*, p)\big\}$\;
   \If{$\exists k < e: \S_{p^{k}} = \S_{p^e}$}{
      \KwRet{$\dfrac{\sum_{1\le i<k} M_i\tt^i}{1-\pp\tt}+\dfrac{\sum_{k\le i<e} M_i\tt^i}{(1-\pp\tt)(1-\tt^{e-k})}$}
   }
   $\epsilon \leftarrow \max \{\ft{A}{\vv{v}}: (A,\vv{v}) \in \S_{p^e}\}$\;
   \If{$\epsilon >1$}{
      \KwRet{$\dfrac{\sum_{1\le i<e}M_i\tt^i}{1-\pp\tt} + \dfrac{(\pp - 1)\tt^e}{(1-\pp \tt)(1-\tt)}$}
   }
   $\text{\texttt{aux}} \leftarrow \{(A,\vv{v})\in \S_{p^e}: \ft{A}{\vv{v}} = \epsilon\}$\;
   $\delta \leftarrow \min\{\udeficit(A,\vv{v},p): (A,\vv{v})\in \text{\texttt{aux}}\}$\;
   $\S^*_{p^e} \leftarrow \{(A,\vv{v})\in \text{\texttt{aux}}: \udeficit(A,\vv{v},p) = \delta\}$\;
