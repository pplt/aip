...
\begin{proof}
If we fix a universal denominator \emily{Use command ``{\tt denom}''?} $\ell$ for $A$,   then \Cref{finitely many coord sums: C} implies that there exists an integer $\beta=\beta(A,\ell)$ such that $p/\ell > \norm{\tail{\vv{s}}_p}$ for every $A$-list $(A,\vv{u},\vv{s}, p)$ with $p > \beta$.  In what follows, we will assume that $p > \beta$.
...
Observe that this set that depends on the class of $p$ modulo $\ell$\emily{Some {\tt ell} should be {\tt denom} below}, but not on $p$ itself.  Furthermore, \Cref{small pairs from shortfalls: E} tells us that every pair in $\S_{p^2}$ is small.
