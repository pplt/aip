...
\newcommand{\denom}{d} %We should stick to the same notation for the denominator of a special point.  Sometimes D is used, but I would prefer a lowercase letter.  Lowercase "d" makes sense, but we use that for the dimension of the ambient polynomial ring.
...
TO DO LIST:
\begin{enumerate}
\item[$\Box$] Decide on ``special'' or ``universal'' denominator and uniformize.
\item[$\Box$]  Look at wording in proof of \Cref{less than u: L}. 
\item[$\Box$] Fix Emily's mercurial
...
\item[$\Box$] Replace any remaining instances of $d$ for dimension with {\tt \text{numvars}}, and fix any inconsistencies. 
   \begin{itemize}
      ...
      \emily[inline]{Like $\mathfrak{m}$, I am OK using this notation; I don't think it is confusing since vectors all have the same notation.}
...
\begin{itemize}
   \item Example with $\opt \Theta$ infinite.
   \pedro[inline]{Done}
 \item Example with $\ldots$
\end{itemize}
...
\details[inline]{
 ...
 \emily[inline]{The current doc (6/5/20) has \Cref{refined-discreteness: L} not explicitly cited, and the lemma right after \Cref{cor: constant mus} is not currently cited, but a note before it gives a consequence we should add.   All other lemmas are used.}
...
%      \item[$\Box$] Does 7.10 come from 7.3, if we replace 1 with $q$ and $\IP$ with $\IP_p$? If so, we should write the proof this way, and replace the comment preceding 7.11 with a remark (that makes clear the reduction to the small case)
   ...
   \emily[inline]{This only seems to happen with $\Pi$ and $\Pi_p$ when the arithmetic constraint is automatically satisfied.  I think that the meaning will be clear to the reader, and I don't think we need to make it any more explicit by distinguishing between ``equal'' and ``equivalent.''}  
...
\emily[inline]{Add what we can say about critical exponents.}
\emily[inline]{Explain that our method replaces an arbitrary $\idealb$ with a diagonal ideal.}


\todo[inline]{Add conventions for $p,q,\kk,\mathfrak{m}$, etc}
...
\pedro{Explain also what we mean by ``$\vv{u}$ is less than $\vv{v}$ in $\mathcal{S}$''.}
...
\subsection{Relations with arithmetic and fractal programs}
 ...
 We now turn our attention to the integers $\mu(\ideala, \ideald, p^e)$ and rational numbers $\crit(\ideala,\ideald)$, \emily{We will reduce the general case to this one$\ldots$}.  First, recall that for each $q$ a power of $p$, the integer $\mu(\ideala, \ideald, q)$ is defined as 
...
% \begin{proposition}
%    \label{face: P}
%    If $\defpt \in \RR^\numvars$ defines a face $\O$ of the Newton polyhedron $\N$, then the $i$-th coordinate of $\defpt$ is zero if and only if $\vv{v} + \lambda \canvec_i \in \O$  for every $\vv{v} \in \O$ and $\lambda > 0$.
%    In particular, the supporting indices of $\defpt$ depend only on $\O$, and $\O$ is bounded if and only if $\defpt$ is positive.
% \end{proposition}
...
%    Conversely, if $\defpt$ is positive, then $\O$ is bounded, as each point $\vv{v}\in \O$ is bounded below by $\vv{0}$ and above by $(\alpha/c_1,\ldots,\alpha/c_\numvars)$. 
% % Similar logic will show that if $\rb(\O) \coloneqq  \{ \canvec_i \in \RR^\numvars : \iprod{\defpt}{\canvec_i} = 0\}$, then
% % \begin{equation}
% % \label{face: e}
% % \O =  \conv( \col(A) \cap \O ) + \cone(\rb(\O))
...
\emily[inline]{Define vertices here?}
...
\emily[inline]{Would having a picture for $m=2$ be overkill?}
...
\begin{example}\label{ex: ft}
   ...
   % shown in blue.
   % \daniel{Is this really blue?  The 3(B) figure looks blue; this looks purple}
...
\begin{definition}[Special denominator]  Suppose that $A$ is a monomial matrix. A \emph{special denominator} for $A$ is a positive integer $\denom = \denom(A)$ such that for every monomial pair $(A, \vv{u})$, there exists a point $\vv{s} \in \sp(A, \vv{u})$ so that $\denom \cdot \vv{s}$ has integer coordinates.
\end{definition}
...
\begin{proof}
   ...
   Let $\denom_{\circ}$ be an integer satisfying the property described in \Cref{uniform denominators for vertices:  T} relative to $A$, and fix a monomial pair $(A, \vv{u})$.
...
% \pedro[inline]{
%    If we figure we don't really need it, then omitting is OK with me. 
% }
% \daniel[inline]{We need it!}
...
\begin{proof}
   Set $\O = \mf(A, \vv{u})$.
   The fact that $\vv{s} \in \sp(A, \vv{u})$ implies that
   \[ \vv{u} = A \vv{s} + \vv{w}\]
   for some point $\vv{w}$ in $\rs(\O)$ that is positive in that subspace.
   ...
   \pedro{
      Maybe remind the reader of what we mean by that (here, or before the lemma).
   }
...
\begin{remark}
   ...
   \pedro{
      ...
      It's only used once (in \Cref{comparing deltas: R}), so maybe it should simply be incorporated there.
   }
   Suppose $(A, \vv{u}, \vv{s}, q)$ is a monomial list and $(\collapse{A}, \collapse{\vv{u}})$ is the collapse of $(A ,\vv{u})$ along $\mf(A, \vv{u})$.
...
\begin{example}
   ...
   In fact, the points of the form $(k,k,1-2k)$, $(k+1,k,-2k)$, and $(k+2,k,-1-2k)$ are optimal for $\ip$ for all $k \in \NN$.
...
\begin{proposition}
   ...
   In particular,
   \[\val \IP(A, \vv{u}, q) \le \ft{A}{\vv{u}}q - \norm{\tail{\vv{s}}_q} + \val \ip (A, \vv{u}, \vv{s}, q).\qed\]
\end{proposition}

The following may be regarded as a partial converse to \Cref{comparison: P}.
...
\begin{proof}
   ...
   To proceed, write $\vv{u} = A \vv{s} + \vv{w}$ for some $\vv{w}$ in $\rs(\O)$ that is positive in that subspace, as in \Cref{mc: D}.
   ...
   where we have used that $\vv{s} \in \sp_{\QQ}(A, \vv{u})$ and $\vv{h} \in \opt \ip$.
   Equality then follows from \Cref{comparison: P}.
\end{proof}
...
We now explore some finiteness properties, and our results are of two types:
\Cref{finite image: P} concerns the integer program $\ip$ associated with some fixed monomial list, while \Cref{finitely many secondary programs: T,finitely many coord sums: C,fsr-exist: T} concern the nature of these programs as the list varies.

Our first result will show that, even though the optimal set of an integer program $\ip$ may be infinite, as shown in \Cref{ex: ft.3}, its shortfall is always finite.
...
We can now prove our first finiteness result.

\begin{proposition}
   \label{finite image: P}
   If $(A, \vv{u}, \vv{s}, q)$ is a monomial list, then the shortfall of $\ip(A, \vv{u}, \vv{s}, q)$ is finite.
\end{proposition}
...
Potentially move it up before \Cref{bounded value: P}.}
...
\begin{theorem}[Existence of finite sets of representatives]
   \label{fsr-exist: T}
   Given a monomial matrix $A$, there exists a finite subset $\fsr = \fsr(A)$ of the domain lattice of $A$ with the following property\textup:
...
If $(A, \vv{u})$ is a monomial pair with $\vv{s} \in \sp_{\QQ}(A, \vv{u})$ is a point with denominator $D$, and $q>\beta D$, then
%
...
\begin{proof}
Let $\beta$ be as in \Cref{uniform value: P}.  Fix a monomial pair $(A, \vv{u})$ with $\O = \mf(A, \vv{u})$, a point $\vv{s} \in \sp_{\QQ}(A, \vv{u})$ with denominator $D$ \emily{Are we saving $\denom$ for special denominators?}, and an integer $q > \beta D$.  Let $\collapse{A}$ and $\collapse{X}$ be as above.
...
\begin{proof}
This follows immediately from \Cref{finitely many secondary programs: T} and \Cref{finitely many coord sums: C}.
\end{proof}
...
The remainder of this paper is dedicated to the study of the arithmetic integer programs $\IP_p(A, \vv{u}, p^e)$ described in \Cref{aip: D}, which perhaps unsurprisingly, turn out to be far more complex in nature than their non-arithmetic counterparts $\IP(A, \vv{u}, p^e)$. \daniel{Should we state the main result of this section?  \Cref{m-primary AIP: T}? }   Throughout this section, $e$ will denote a natural number, and $q$ will denote a power of $p$.  

\emily[inline]{We should write a line here to motivate \Cref{general AIP: T}.}
...
where the maximum\footnote{Recall that, as $A$ is monomial, the feasible set of $\IP_p(A, \vv{v}, q)$ is finite, and hence, so is the shortfall of this program.  In particular, this maximum value is well-defined.} is over all points $\vv{v}$ in the shortfall of $\IP_p(A, \vv{u}, q)$.
...
\[  \short \IP_p(A, \vv{u}, qp^e) = \cup \short \IP_p(A, \vv{v}^{\ast}, p^e) \] where the union is over all $\vv{v}^{\ast} \in \short \IP_p(A, \vv{u}, q)$ that achieve $M$.
\end{theorem}

\begin{proof}  It is clear that multiplying a feasible point for $\IP_p(A, \vv{u}, q)$ by $p^e$ yields a feasible point for $\IP_p(A, \vv{u}, qp^e)$, from which it follows that $L\geq 0$.


Next, consider $\vv{g} \in \opt \IP_p(A, \vv{u}, qp^e)$.  Dividing by $p^e$, we may write \[ \vv{g} = \vv{h} p^e + \vv{k} \]
with $\vv{h}$ and $\vv{k}$ nonnegative lattice points, and every coordinate of $\vv{k}$ is less than $p^e$.  
...
This optimality, along with the definition of shortfalls, tells us that $\vv{v} \coloneqq \vv{u}q-A\vv{h}$ is a point in the shortfall of $\IP_p(A, \vv{u},q)$.  We may then express the linear constraint on $\vv{g}$ imposed by $\IP_p(A, \vv{u}, qp^e)$ as  
\[ \vv{u}qp^e > A \vv{g} = A \vv{h} p^e + A \vv{k} = (\vv{u}q-\vv{v})p^e + A \vv{k} \] 
which is equivalent to the constraint $A \vv{k} < \vv{v}p^e$.   As we have already seen above that $\binom{\norm{\vv{k}}}{\vv{k}}$ is nonzero modulo $p$, it follows that $\vv{k} \in \feas \IP_p(A, \vv{v}, p^e)$.   

Putting everything together, we see that
\[ L = \norm{\vv{g}}-\norm{\vv{h}}p^e = \norm{\vv{k}} \leq \min \{ p^e-1, \val \IP_p(A, \vv{v}, p^e) \} \leq \min \{ p^e-1, M \}.\]

For the remainder of this proof, suppose that $M \leq p^e-1$.    Consider a point  $\vv{v}^{\ast} \in \short \IP_p(A, \vv{u}, q)$ that achieves $M$, and write $\vv{v}^{\ast} = \vv{u}q - A \vv{h}^{\ast}$ for some $\vv{h}^{\ast} \in \opt \IP(A, \vv{u}, q)$.  If $\vv{k}^{\ast} \in \opt \IP_p(A, \vv{v}^{\ast}, p^e)$, then $\norm{\vv{k}^{\ast}} = M \leq p^e-1$, and so every entry of $\vv{k}^{\ast}$ is at most $p^e$.  Thus, if $\vv{g}^{\ast} \coloneqq \vv{h}^{\ast} p^e + \vv{k}^{\ast}$, then \Cref{dickson: R} tells us that $\binom{\norm{\vv{g}^{\ast}}}{\vv{g}^{\ast}} \equiv  \binom{\norm{\vv{h}^{\ast}}}{\vv{h}^{\ast}} \binom{\norm{\vv{k}^{\ast}}}{\vv{k}^{\ast}} \not \equiv 0 \bmod p$.  A direct computation will also show that $\vv{g}^{\ast}$ satisfies the linear constraint of  $\IP_p(A, \vv{u}, qp^e)$, which shows that $\vv{g}^{\ast}$ must be feasible for this program.  It follows that
%
\[ L \geq \norm{\vv{g}}^{\ast} - \norm{\vv{h}}^{\ast}p^e = \norm{\vv{k}^{\ast}} = M \] 
%
...
\[  \vv{v}^{\ast} p^e-A\vv{k}^{\ast} = (\vv{u}q-A\vv{h}^{\ast})p^e-A(\vv{g}^{\ast}-\vv{h}^{\ast}) = \vv{u}qp^e-A \vv{g}^{\ast} \] 
demonstrates that $\vv{v}^{\ast} p^e-A\vv{k}^{\ast}$, a point in the shortfall of $\IP_p(A, \vv{v}^{\ast}, p^e)$, is also a point in the shortfall of $\IP_p(A, \vv{u}qp^e)$.  Moreover, as $\vv{v}^{\ast}$ was arbitrary with respect to achieving $M$, this shows that 
%
\begin{equation}
\label{general comparison of shortfalls: e}
 \cup_{\vv{v}^{\ast}} \short \IP_p(A, \vv{v}^{\ast}, p^e) \subseteq \short \IP_p(A, \vv{u}, qp^e).
 \end{equation}

To establish the opposite containment, let $\vv{g}$ be the optimal point for $\IP_p(A, \vv{u}, qp^e)$ considered earlier in this argument.  As the reader may verify, now that we know that $L=M$, the optimality of $\vv{g}$ implies that $\vv{v} \in \short \IP_p(A, \vv{u}, q)$ must also achieve $M$, and an argument similar to the one above will show that $\vv{u}qp^e - A \vv{g}$, which represents an arbitrary point in the shortfall of $\IP_p(A, \vv{u}, qp^e)$, equals $\vv{v}p^e-A\vv{k}$, a point in the shortfall of $\IP_p(A, \vv{v}, p^e)$.  Thus, the containment of sets in \eqref{general comparison of shortfalls: e} must be an equality.
\end{proof}

We record a consequence of the bound $L \leq p^e-1$ in \Cref{general AIP: T} below.

\begin{corollary}
   \label{cor: mu comparison}  Suppose that $(A,\vv{u})$ and $(B,\vv{v})$ are monomial pairs, not necessarily of the same dimension.  
...
\begin{proof}
Applying \Cref{general AIP: T} to a monomial pair $(E,\vv{w})$ tells us that
%
\begin{equation}
 \val \IP_p(E, \vv{w},q)p^e \leq \val \IP_p(E, \vv{w}, qp^e) < (\val \IP_p(E, \vv{w}, q)+1)p^e.
 \end{equation}

Combining these bounds, with $(E, \vv{w})$ replaced by $(A, \vv{u})$ and $(B,\vv{v})$, with our assumption that $\val \IP_p(A, \vv{u}, q)$ is at least $\IP_p(B, \vv{v}, q) + 1$, we see that
   \begin{align*}
...
\begin{example}[Small pairs from shortfalls]  
\label{small pairs from shortfalls: E}  If $\vv{v}$ is any point in the shortfall of some integer program $\IP = \IP(A, \vv{u}, q)$, then the pair $(A, \vv{v})$ is small.  Indeed, by definition of shortalls, we may write $\vv{v} = \vv{u}q-A\vv{h}$ for some $\vv{h} \in \opt \IP$.  Subtracting $\vv{a}_i$, the $i$-column of $A$, from both sides of this identity yields 
...
Similarly\daniel{This is done in \Cref{independence: L}, but I am not sure if that will survive our edits.  Also, I am using the terminology \emph{universal shortfall} here, but we can ditch it}, if $\vv{v}$ is any point in the universal shortfall $\Delta(A, \vv{u}, q)$, then the pair $(\collapse{A}, \vv{v})$ is also small, where the overbar denotes collapse along $\O = \mf(A, \vv{u})$.
\end{example}
...
\begin{proof}
For a change of pace, we give an algebraic proof.  If $(A,\vv{u})$ is associated to the ideal pair $(\ideala, \ideald)$, then as noted in \Cref{algebraic small and very small: R}, the smallness of $(A, \vv{u})$ is equivalent to the containment $\ideala \subseteq \ideald$.  However, this containment implies that $\ideala^{[p^e]} \subseteq \ideald^{[p^e]}$, which shows that $\val \IP_p(A, \vv{u}, p^e) = \mu(\ideala, \ideald, p^e) \leq p^e-1$.
\end{proof}

\begin{remark}[Reduction to the small case]  We now describe how to reduce the computation of the values of the integer programs $\IP_p(A, \vv{u}, p^e)$ for all $e \geq 1$ to the small case, at least when $p$ is large relative to the pair $(A, \vv{u})$.  

For what follows, fix a pair $(A, \vv{u})$, and suppose that $p$ is larger than the value of the standard integer program $\IP(A, \vv{u},1)$.  For all such primes, this bound on the value implies that $\IP_p(A, \vv{u}, 1) = \IP(A, \vv{u}, 1).$

\Cref{small pairs from shortfalls: E} tells us that $(A, \vv{v})$ is small for every $\vv{v} \in \short \IP(A,\vv{u},1)$, and for such a point, \Cref{trivial small value bound: P}  then tells us that $\val \IP_p(A, \vv{v}, p^e)$ is at most $p^e-1$.  Setting $q=1$ in the statement of \Cref{general AIP: T}, this observation tells us that quanitity $M$ defined therein is at most $p^e-1$. 

Invoking \Cref{general AIP: T} once more, we see that if $p$ is greater than the value of the standard integer program $\IP(A, \vv{u}, 1)$, which is independent of $p$, then
\[ \val \IP_p(A, \vv{u},p^e) = \val \IP(A, \vv{u},1)p^e + \max \{ \val \IP_p(A, \vv{v}, p^e) \} \] 
where the maximum is over all pairs $(A, \vv{v})$ with $\vv{v} \in \short \IP(A, \vv{u},1)$.  As noted earlier, each such pair is small, which completes our reduction.
\end{remark}

The following allows us to reduce even further, to the very small case.

 \begin{theorem}
 \label{small not very small value: T}  Given a monomial matrix $A$, there exists an integer $\beta=\beta(A)$ with the following property\textup:  
...
\begin{proof}
If we fix a universal denominator $\ell$ for $A$,   then \Cref{finitely many coord sums: C} implies that there exists an integer $\beta=\beta(A,\ell)$ such that $p/\ell > \norm{\tail{\vv{s}}_p}$ for every $A$-list $(A,\vv{u},\vv{s}, p)$ with $p > \beta$.  In what follows, we will assume that $p > \beta$.

Next, suppose that $(A,\vv{u})$ is small, but not very small, and let $\vv{s}$ be a special point for this pair with denominator $\ell$.   \Cref{canonical-feasible: T} then states that 
\[ \vv{k} \coloneqq \vv{s}p-\tail{\vv{s}}_p \in \feas \IP(A,\vv{u},p). \] 

The assumption that $(A,\vv{u})$ is not very small implies that $\vv{s} = \ft{A}{\vv{u}} > 1$, and our choice of $\ell$ then implies that $\norm{\vv{s}} \geq 1 + 1/\ell$, so that \[\norm{\vv{k}} \geq p + p/\ell - \norm{\tail{\vv{s}}_p} > p\] where the strict inequality follows from our choice of $p \gg 0$.  

This bound implies that there exists a point $\vv{h} \leq \vv{k}$ with $\norm{\vv{h}} = p-1$.  Without loss of generality, assume $h_1 < k_1$ , and define $\vv{g} = \vv{h}q + \vv{e}_1(q-1)$.  By construction, $\vv{g} \leq \vv{k}q, \norm{\vv{g}} = qp-1$, and $\binom{\vv{g}}{\vv{g}} \neq 0 \bmod p$.  Furthermore, the fact that $\vv{k} \in \feas \IP(A,\vv{u},p)$ implies that $A\vv{g} \leq A{\vv{k}}q < \vv{u}qp$, and so $\vv{g} \in \feas \IP_p(A,\vv{u},qp)$.  It follows that $\val \IP_p(A,\vv{u},qp) \geq \norm{\vv{g}} = qp-1$, and \Cref{trivial small value bound: P} implies that this bound must be an equality.
\end{proof} 
...
% \[ 0 \leq \vv{t} - \frac{\tail{\vv{t}}_p}{p} + \frac{\norm{\tail{\vv{t}}_p}}{p} \cdot \canvec_i  \leq \vv{s} \]
...
% %
%  \[ A \left(  \vv{t}p - \tail{\vv{t}}_p + \norm{\tail{\vv{t}}_p} \cdot \canvec_i  \right) < \vv{u}p. \]
% %
...
%   \[ \vv{k}_p  =   \vv{t}p - \tail{\vv{t}}_p+ (\norm{\vv{t}}_p - 1) \cdot \canvec_i   \] has nonnegative integer coordinates and satisfies $A \vv{k}_p < \vv{u}p - A \canvec_i$.  A direct calculation will also show that $\norm{\vv{k}_p} = p-1$.
%  
%  Given this, it is straightforward to verify that the point
%  \[ \vv{k}_p \cdot p^{e-1} + (p^{e-1} - 1) \cdot \canvec_i \]
%  has norm $p^e-1$ and is feasible for $\IP_p(A, \vv{u}, p^e)$ for all $e \geq 1$.  \Cref{trivial small value bound: P} then allows us to conclude that $\val \IP_p(A, \vv{u}, p^e)  = p^e-1$ for all $e \geq 1$.
% To conclude the proof, it suffices to recall that, as noted in \Cref{geometric small and very small: R},  there are only finitely many $(A, \vv{u})$ that are small, but not very small.
% \end{proof}

With these reductions in hand, we now describe how to compute the value of $\IP_p(A, \vv{u}, p^e)$ when $(A, \vv{u})$ is very small, and $e=1$,  by appealing to our earlier work on the standard integer programs $\IP(A, \vv{u}, p^e)$. 
...
If $(A, \vv{u})$ is very small, and $p > \beta$, then the arithmetic integer program $\IP_p(A, \vv{u}, p)$ agrees with the standard integer program $\IP(A, \vv{u}, p)$.  In particular, if $p > \beta$, then 
%
\[\deficit \IP_p(A,\vv{u},p)=\deficit \IP(A,\vv{u},p) = \udeficit(A,\vv{u},p) \] and \daniel{Weird observation:  If I replace the command ``ol" with ``collapse" below, the $p$ becomes bold too.  Check later to see if this introduced other typesetting errors in the paper.}
%
\[ \ol{\short \IP_p(A,\vv{u},p)}= \ol{\short \IP(A,\vv{u},p)}= \ushort(A,\vv{u},p) \]
%
where the overbar denotes collapse along $\O = \mf(A, \vv{u})$.
\end{theorem}
...
\Cref{arithmetic uniform value and image: T} is fundamental, and together with \Cref{general AIP: T}, forms the basis for a process to compute value of $\IP_p(A, \vv{u}, p^e)$ when $p$ is large.  Below, we describe this process in an important special case.  Though somewhat technical, these arguments will motivate some constructions in the general case, which is more complicated, and treated in the next section.

\subsection{The $\idealm$-primary case}
\label{m-primary: SS}
  Our goal here is to describe an algorithm for computing the value of the arithmetic integer program $\IP_p(A,\vv{u}, p^e)$ in the case that $A$ is associated to an $\idealm$-primary monomial ideal,  $(A,\vv{u})$ is very small, $e$ is an arbitrary positive integer, and $p$ is large relative to the ambient matrix $A$.  As a by-product, this algorithm  establishes the following result.

\begin{theorem}
\label{m-primary AIP: T}
 If $A$ is a monomial matrix associated to an $\idealm$-primary monomial ideal, then there exist $\beta=\beta(A)$ with the following property:  If $p > \beta$ and $(A,\vv{u})$ is very small, then the value of $\IP_p(A,\vv{u},p^e)$ is a polynomial in $p$ of degree $e$ with leading coefficient $\ft{A}{\vv{u}}$.  Furthermore,  the non-leading coefficients in this polynomial are nonpositive and rational, and depend on $(A,\vv{u})$ and the class of $p$ modulo any universal denominator for $A$.
\end{theorem}

Our assumption that $A$ comes from an $\idealm$-primary ideal implies that the minimal face $\mf(A, \vv{u})$ is bounded for every pair $(A, \vv{u})$, and so the collapse along every such face is the identity.  In this context, \Cref{arithmetic uniform value and image: T} becomes a significantly stronger result, as it computes \emph{full} shortfalls, as opposed to properly collapsed shortfall, whenever it applies.  We stress that the failure of \Cref{arithmetic uniform value and image: T} to compute full shortfalls in general  accounts for much of the increased technicality present in the next section.

\begin{remark}[On $p$ being sufficiently large]
\label{p large m-primary: R}
First off, throughout this discussion, we will always assume that $p$ is large enough, relative to $A$, so that the conclusions of \Cref{small not very small value: T} and \Cref{arithmetic uniform value and image: T} are valid.   
...
\[ \val \IP_p(A,\vv{u},p) = \ft{A}{\vv{u}}p-\udeficit(A,\vv{u},p) \] a linear polynomial in $p$.  By definition, the very small pair $(A,\vv{u})$ satisfies $\ft{A}{\vv{u}} \leq 1$, and \Cref{discreteness: P} then implies that there are only finitely many possible values for $\ft{A}{\vv{u}}$ as the very small pair $(A,\vv{u})$ varies.  

Similarly, \Cref{finitely many deltas for a fixed A: P} tells us that there are only finitely many possible values for $\udeficit(A,\vv{u},p)$ as we vary over all pairs $(A,\vv{u})$, while \Cref{pair periodicity: R} tells us that this universal deficit, with $(A,\vv{u})$ fixed, depends on the class of $p$ modulo any universal denominator for $A$, but not on $p$ itself.

It follows from these observations that we may choose $p \gg 0$, with the lower bound depending only on the ambient matrix $A$, so that to compare any two numbers of the form $\val \IP_p(A,\vv{u},p)$ with $(A,\vv{u})$ very small, it suffices to first compare their leading coefficients, and if necessary, their constant term.  Furthermore, the outcome of any such comparison will depend on the class of $p$ module any universal denominator for $A$, but not on $p$ itself.
\end{remark}

\daniel[inline]{Subscripts are now indexed by powers of $p$, and the indexing was modified so that it agrees with our choice of notation for the graphs later}

We are now prepared to describe our algorithm.   In what follows, let $p \gg 0$ be as in \Cref{p large m-primary: R}, and let $\denom$ be any universal denominator for $A$.

We begin our process by initially defining $\S_p = \S_p(A,\vv{u})$ and $\S^{\ast}_p(A,\vv{u}) = \S^{\ast}_p$ to be the singleton set $\S_p=\S_p^{\ast} = \{ (A,\vv{u}) \}$,  and noting that 
\[ \val \IP_p(A,\vv{u},p) = \ft{A}{\vv{u}}p-\udeficit(A,\vv{u}).\]
%$\mathcal{S}_1$ to be the first in the sequence of shortfalls associated to the programs $\IP_p(A,\vv{u},p^e)$.  In other words, set \daniel{Replace $\mathcal{S}_e \mapsto \S_{p^e}$? \\ Consist of pairs?}

We turn our attention to \emph{at least} computing $\val \IP_p(A,\vv{u},p^2)$, and set
%
\begin{align*}
 \S_{p^2} \coloneqq \S_{p^2}(A,\vv{u}) & = \{ (A,\vv{v}) : \vv{v} \in \short \IP_{p}(A,\vv{u},p) \} \\
 & = \{ (A,\vv{v}) : \vv{v} \in \ushort(A,\vv{u},p)\}.
\end{align*}  

Observe that this set that depends on the class of $p$ modulo $\ell$, but not on $p$ itself.  Furthermore, \Cref{small pairs from shortfalls: E} tells us that every pair in $\S_{p^2}$ is small.

This smallness, along with \Cref{trivial small value bound: P}, tells us that  \[ M_e \coloneqq \max \{ \val \IP_p(A, \vv{v}, p^e) :  (A,\vv{v}) \in \S_{p^2} \}\] is at most $p^e-1$.  Setting $q=p$ in \Cref{general AIP: T} then tells us that
%
\begin{equation}
...
\short \IP_p(A,\vv{u},p^{e+1}) = \cup \short \IP_p(A,\vv{v}, p^e)
\end{equation}
%
where the union is over all $(A,\vv{v}) \in \mathcal\S_{p^2}$ that achieve $M_e$.

As $\S_{p^2}$ depends on $p$ modulo the universal denominator $\denom$, the issue of whether there exists a pair in $\S_{p^2}$ that is not very small also depends on $p$ modulo $\ell$.  When this occurs, \Cref{small not very small value: T} tells us that $M_e = p^e-1$, and \eqref{m-primary value: e} then allows us to conclude that 
%
\[ \val \IP_p(A,\vv{u},p^{e+1})  = \ft{A}{\vv{u}} p^{e+1}-(\udeficit(A,\vv{u},p)-1)p^e-1.  \]
%
As this formula is valid for all $e \geq 1$, we our process terminates in this case.
...
Let $\S_{p^2}^{\ast} = \S_{p^2}^{\ast}(A,\vv{u})$ be the subset of $\S_{p^2}$ consisting of all $(A,\vv{v})$ that first maximize $\ft{A}{\vv{v}}$, and only then minimize $\udeficit(A,\vv{v},p)$.  By definition, $\S_{p^2}^{\ast}$ depends on $p$ modulo $\ell$, and our choice of $p \gg 0$ guarantees that $\S_{p^2}^{\ast}$ corresponds to the points in $\S_{p^2}$ that achieve $M_1$.  Therefore, we may write \[ M_1 = \epsilon_{p^2} p - \delta_{p^2}\]  where $\epsilon_{p^2},\delta_{p^2}$ are positive rational numbers that depend on $p$ modulo $\ell$.
...
%
\[ \short \IP_p(A,\vv{u},p^2) = \cup_{\vv{v} \in \S_{p^2}^{\ast}} \short \IP_p(A,\vv{v},p) = \cup_{\vv{v} \in \S_{p^2}^{\ast}} \ushort(A,\vv{v},p). \]  
%

Turning our attention to at least computing $\val \IP_p(A,\vv{u},p^3)$, we define
\begin{align*}
\S_{p^3} \coloneqq \S_{p^3}(A,\vv{u}) & = \{ (A,\vv{w}):  \vv{w} \in \short_p(A,\vv{u},p^2) \} \\
 & = \{ (A,\vv{w}): \vv{w} \in \cup_{\vv{v} \in \S_{p^2}^{\ast}} \ushort(A,\vv{v},p) \} 
 \end{align*}
%
Observe that $\S_{p^3}$ depends on the class of $p$ modulo $\ell$, but not on $p$ itself.  Furthermore, \Cref{small pairs from shortfalls: E} tells us that every pair in $\S_{p^3}$ is small.


Next, we set $M'_e  \coloneqq \max \{ \val \IP_p(A, \vv{w}, p^e) :  (A,\vv{w}) \in \S_{p^3}\}$ for every $e \geq 1$.  Arguing as above, we may demonstrate that%
\begin{equation}
...
However, if each pair in $\S_{p^3}$ is very small, then \Cref{arithmetic uniform value and image: T} instead implies that $M'_1 = \max \{ \ft{A}{\vv{w}}p-\udeficit(A,\vv{w},p) : (A,\vv{w}) \in \S_{p^3}\}$.  As above, the subset $\S^{\ast}_{p^3}$ of $\S_{p^3}$ of points that achieve $M'_1$ depends on $p$ modulo $\ell$, and can be explicitly described in a concrete way.  Thus, there exist positive rational numbers $\epsilon_{p^3}, \delta_{p^3}$, depending on $p \bmod \ell$, such that $M'_1 = \epsilon_{p^3} p -\delta_{p^3}$, and setting $e=1$ in \eqref{m-primary value 2: e} then demonstrates that $\val \IP_p(A,\vv{u},p^3)$ equals
%
...
Next, we set %
\begin{align*}
\S_{p^4} \coloneqq \S_{p^4}(A,\vv{u}) & = \{ (A,\vv{z}):  \vv{z} \in \short_p(A,\vv{u},p^3) \} \\
 & = \{ (A,\vv{z}): \vv{z} \in \cup_{\vv{w} \in \S_{p^3}^{\ast}} \ushort(A,\vv{w},p) \} 
 \end{align*}
%

Like $\S_p,\S_{p^2}$ and $\S_{p^3}$, the set $\S_{p^4}$ consists of small pairs, and depends on $p$ modulo $\ell$, but not on $p$ itself. If $\S_{p^4}$ contains a pair that is not very small, then repeating this process, we can compute the value of $\IP_p(A,\vv{u},p^{e+3})$ for all $e \geq 1$.  Otherwise, we can only compute the value and shortfalls of the next program $\IP_p(A,\vv{u},p^{4})$, and then analogously repeat our arguments.

We may continue iterating these steps.  Note that, when it is necessary to define it, the set $\S_{p^e}$ will consist of small pairs $(A,\vv{v})$, where $\vv{v}$ is in some universal shortfall of an $A$-pair.  It then follows from \Cref{finitely many deltas for a fixed A: P} that all defined $\S_{p^e}$ are subsets of some finite set determined by $A$.  Thus, there is an upper bound for the number of possible distinct  $\S_{p^e}$, and as soon as such a set repeats, the process must cycle from that point onward.
...
\begin{remark}
\label{p-sprout: R}
As noted in \Cref{collapse of monomial is monomial: R}, the collapse of a monomial matrix along a face of its Newton polyhedron is monomial.  It follows from this that the $p$-sprout of a monomial pair is also a monomial pair.  Furthermore,   \Cref{independence: L}\daniel{Correct pointer?} tells us that there are only finitely many $p$-sprouts of a fixed monomial pair, and that each such sprouted pair is small.
 \end{remark}

\begin{remark}[Sprouts as collapsed shortfall pairs]
\label{sprouts as collapsed shortfall pairs: R}
 Throughout this remark, suppose that $p \gg 0$ relative to $A$ so that the conclusion of \Cref{arithmetic uniform value and image: T} holds.  Let us call an $A$-pair $(A,\vv{w})$ a $p$-shortfall pair associated to $(A,\vv{u})$ if $\vv{w} \in \short \IP_p(A,\vv{u},p) = \ushort(A,\vv{u},p)$.  Recall that $p$-shortfall pairs played an important role in the process for computing $\val \IP_p(A,\vv{u},p^e)$ described in \Cref{m-primary: SS} when $A$ is associated to an $\idealm$-primary ideal.
...
\begin{definition}[The directed sprouting graph]  Let $A$ be a monomial matrix.  We define $\graph_p(A)$ to be the set consisting of all very small $A$-pairs\daniel{Define $A$-pairs}, and once $\graph_q(A)$ has been defined for some $q$ a power of $p$,  we then define $\graph_{qp}(A)$ to be the set consisting of all $p$-sprouts of all pairs in $\graph_q(A)$.  
...
\label{elementary1: R}
Consider a monomial matrix $A$.  In this remark, we gather some basic facts\daniel{Please verify these} concerning the graph $\sprout_p(A)$.
\begin{enumerate} 
\item The set $\graph_p(A)$ of very small $A$-pairs is independent of $p$, and may infinite, depending on the geometry of the Newton polyhedron of $A$.  
...
\item \label{elementary1 periodic in e} Applying \Cref{finitely many deltas for a fixed A: P} to $A$, and each of its descendent monomial matrices, we see that $\cup_{e>1} \graph_{p^e}(A)$ is a finite set, and their recursive nature then implies that the sets $\graph_{p^e}(A)$ are eventually periodic in $e$.  

\item \label{elementary1 periodic in p} Furthermore, \Cref{uniform periodicity: R} tells us that if $\denom$ is a universal denominator for $A$, then the sprouting graph  $\sprout_p(A)$ (meaning, all of its vertices \emph{and} arrows) depends on the class of $p$ modulo $\denom$.
...
\]
Observe that \eqref{p-big-1} and \eqref{p-big-2} imply that for every $1 \leq i \leq e$, the quantity $p/\ell$ is greater than every coordinate of $\vv{1}-\vv{h}_i$, and every coordinate of $A_i \vv{h}_i$.  It then follows from \Cref{uniform value: P} (or rather, its proof)\daniel{This follows  from the \emph{statement} of \Cref{uniform value: P}.} that
\begin{equation}
...
\begin{definition}[Subgraphs associated to very small pairs]  Suppose that $(A,\vv{u})$ is very small, and define $\S_p(A,\vv{u})$ to be the set consisting solely of $(A,\vv{u})$.
...
%We define $\sprout_p(A,\vv{u})$ to be the induced subgraph of $\sprout_p(A)$ on the set of vertices  \[ \bigsqcup_{e=0}^{\infty} \S_{p^e}(A,\vv{u}) \]
%and define $\sprout_p^{\ast}(A,\vv{u})$ to be the induced subgraph of $\sprout_p(A)$ on the set of vertices
%\[ \bigsqcup_{e=0}^{\infty} \S^{\ast}_{p^e}(A,\vv{u}) \]
\end{definition}

\begin{remark}[Elementary observations]
\label{elementary2: R}
Suppose $(A,\vv{u})$ is very small.  In this remark, we gather some basic facts\daniel{Please verify these} concerning the graph $\sprout^{\ast}_p(A,\vv{u})$.
\begin{enumerate}


\item  \label{elementary2 lowest level}  The only vertex of $\sprout_p^{\ast}(A,\vv{u})$ of level $p$ is $(A,\vv{u})$.
\item \label{elementary2 small pairs}  It follows from \Cref{elementary1: R}\eqref{elementary1 small pairs} that $\S_{q}(A,\vv{u})$, when non-empty, consists of small pairs whose monomial matrix is among the finitely many descendants of $A$.  Furthermore, $\S_{qp}(A,\vv{u}) = \emptyset$ if and only if $\S_q(A,\vv{u}) = \emptyset$, or $\S_q(A,\vv{u})$ contains a pairs a pair that is not very small, which itself holds if and only if $\S_{q}^{\ast}(A,\vv{u})= \emptyset$.   Furthermore, $\S_q^{\ast}(A,\vv{u})$ is either empty, or contains only very small pairs.

\item \label{elementary2 periodic in e} Arguing as in \Cref{elementary1: R}\eqref{elementary1 periodic in e}, we see that both $\cup_{e=1}^{\infty} \S_{p^e}(A,\vv{u})$ and  $\cup_{e=1}^{\infty} \S^{\ast}_{p^e}(A,\vv{u})$ are finite, and that each of the sets $\S_{p^e}(A,\vv{u})$ and $\S^{\ast}_{p^e}(A,\vv{u})$ are eventually periodic in $e$.

\item \label{elementary2 periodic in p} Similarly, arguing as in \Cref{elementary1: R}\eqref{elementary1 periodic in p}, we see that the graph $\sprout_p^{\ast}(A,\vv{u})$ depends only on $p$ modulo any universal denominator $\ell$ for $A$.
\end{enumerate}
\end{remark}
...
\label{epsilon delta: D}
 ...
 \end{align*}
 where $(B,\vv{v})$ is any pair in $\S^{\ast}_{p^e}(A,\vv{u}) \neq \emptyset$.  Observe that, by definition of $\S^{\ast}_{p^e}(A,\vv{u})$, these numbers are independent of the choice of $(B,\vv{v}) \in \S^{\ast}_{p^e}(A,\vv{u})$.
...
\Cref{arithmetic uniform value and image: T} hold for $A$, as well as for each of its finitely many descendants.  In particular, for such primes, $\val \IP_p(B,\vv{v}) = \ft{B}{\vv{v}}p-\udeficit(B,\vv{v},p)$  for each vertex $(B,\vv{v})$ of $\sprout_p^{\ast}(A,\vv{u})$, and the existence of $\beta(A)$ as above then follows from arguments similar to those made in \Cref{p large m-primary: R}.
\end{proof}
...
\begin{remark}
As noted in \Cref{elementary2: R}\eqref{elementary2 lowest level}, if $(A,\vv{u})$ is very small, then $\S^{\ast}_p(A,\vv{u}) = \{ (A,\vv{u} )\}$, which shows that $\Q_p(A,\vv{u})$ is at least $p$.  Furthermore, \Cref{elementary2: R}\eqref{elementary2 periodic in e}  implies the issue of whether $\Q_p(A,\vv{u}) = \infty$ can be determined by checking whether $\S_{qp}(A,\vv{u}) \neq \emptyset$ for a finite number of $q$.  Finally, \Cref{elementary2: R}\eqref{elementary2 periodic in p} implies that $\log_p \Q_p(A,\vv{u})$ depends only on $(A,\vv{u})$, and on the class of $p$ modulo any universal denominator for $A$, but not $p$ itself. 
\end{remark}
...
\daniel[inline]{Needs a transition}
...
\begin{proof}
   Fix a monomial matrix $A$, and choose $p$ large enough so that the conclusions of \Cref{arithmetic uniform value and image: T} hold for $A$, and for each of its finitely many descendants, and also so that \Cref{p large graph: P} holds for $A$.  
   
   \Cref{p large graph: P}, and our choice of $p \gg 0$, imply that $\val \IP_p(B,\vv{v},p)$ equals $\val \IP_p(A_1, \vv{u}_1, p)$, by virtue  of both being vertices of the same level in the graph $\sprout_p^{\ast}(A, \vv{u})$.  This establishes the result when $e = 1$.
   
    Next, consider a path $(A_1,\vv{u}_1) \to \cdots \to (A_e, \vv{u}_e)$ with $e \geq 2$, and suppose that the result holds for all shorter paths.  Invoking \Cref{general AIP: T} for the pair $(B,\vv{v})$, keeping in mind that  $\val \IP_p(B,\vv{v},p) =  \IP_p(A_1,\vv{u}_1,p)$, then shows that
    \[
    ...
    Let an overbar denote collapse along $\O=\mf(B,\vv{v})$.  If the pair $(B,\vv{w})$ is as above, then the linear constraints of $\IP_p(\ol{B}, \ol{\vv{w}}, p^{e-1})$ are a subset of the linear constraints of $\IP_p(B, \vv{w}, p^{e-1})$, while their arithmetic constraints are identical.  Therefore, the value of $\IP_p(B,\vv{w},p^{e-1})$ is at most the value of $\IP_p(\ol{B}, \ol{\vv{w}}, p^{e-1})$.  On the other hand, as noted in \Cref{sprouts as collapsed shortfall pairs: R}, the collapsed pairs $(\ol{B}, \ol{\vv{w}})$ of this form are precisely the $p$-sprouts of $(B,\vv{v})$.   Thus, 
    \[
...
Therefore, to complete our inductive step, it suffices to show that
    ...
    If $(C,\vv{z}) \notin \S^{\ast}_{qp}(A,\vv{u})$, then \Cref{p large graph: P} implies that $\val \IP_p(C,\vv{z}, p)$ is less than $\val \IP_p(A_2, \vv{u}_2, p)$.  In light of this, \Cref{cor: mu comparison} implies that $\mu(C, \vv{z},p^{e-1}) < \mu(A_2,\vv{u}_2, p^{e-1})$, and the induction hypothesis applied to $(A_2, \vv{u}_2) \to \cdots \to (A_e, \vv{u}_e)$ and $(A_2,\vv{u}_2)$ itself gives \eqref{ineq}.
...
\begin{proof}\daniel{Describe the condition $p \gg 0$}
...
However, as noted in \Cref{elementary2: R}\eqref{elementary2 small pairs}, each of the vertices $(A_s, \vv{u}_s)$ is very small, and  \Cref{arithmetic uniform value and image: T} then implies that $\IP(A_s,\vv{u}_s,p) = \IP_p(A_s, \vv{u}_s, p)$.  It follows from this, and \Cref{dickson: R}, that the point $\vv{k}$ satisfies the arithmetic constraint of the arithmetic program $\IP_p(A,\vv{u},p^e)$, so that 
%
\begin{align*} 
...
\end{align*}
\Cref{lem: upper bound for higher mu} then implies that we must have equality throughout.  Noticing that $\epsilon_p(A,\vv{u}) = \ft{A}{\vv{u}}$, this is simply a rearrangement of our claim.
...
Given this second path, \Cref{ILL: T} tells us that there exists a finite list of points $\{ \vv{h}_s \}_{s=1}^{l+1}$ with each $\vv{h}_s \in \opt \IP(B_s, \vv{v}_s, p)$, and such that  
%
\[ \vv{h} \coloneqq \sum_{s=1}^{l+1} \vv{h}_s p^{l+1-s} \in \feas \IP(A,\vv{u}, \Q p) \] 
%

 
Arguing as in the proof of \Cref{small not very small value: T}, our choice of $p \gg 0$ guarantees that the value of the standard linear program $\IP(B_{l+1}, \vv{v}_{l+1},p)$ is greater than~$p$.  Therefore,  $\norm{\vv{h}_{l+1}}$ is at least $p$, and so there exists a point $\vv{g} \leq \vv{h}_{l+1}$ with $\norm{\vv{g}} = p-1$.  Without loss of generality, suppose that the bound $\vv{g} \leq \vv{h}_{l+1}$ is strict in the first entry, so that $\vv{0} \leq \vv{g} + \vv{e}_1 \leq \vv{h}_{l+1}$.
...
& = \Big ( \sum_{s=1}^l (\epsilon_{p^s}(A,\vv{u})p-\delta_{p^s}(A,\vv{u})) p^{l-s} \Big ) \cdot p^e + p^e-1 \\ 
 & = \val \IP_p(A,\vv{u}, \Q) p^e + p^e-1
\end{align*}
and \Cref{general AIP: T} further implies that equality must hold throughout.

To conclude our proof, it remains to demonstrate that the non-leading terms of $\val \IP_p(A,\vv{u},p^e)$ are nonpositive.  However, this will follow\daniel{Elaborate!} from \Cref{non-leading coefficients: L} below.
\end{proof}


\daniel[inline]{Explain how this gives a new proof of the rationality of critical exponents?}
...
\label{non-leading coefficients: L}
 If  $p>0$ is prime and $(B, \vv{v})$ is a $p$-sprout of  $(A, \vv{u})$, then \[ \delta(A, \vv{u}, p) \geq \ft{B}{\vv{v}}\]
...
The following theorem, a generalization of \cite[Lemma 6.1]{budur+mustata+saito.roots_bs_polys_monomial}, is the main result of \Cref{monomial-reduction: A}.  This appendix concludes by applying the results herein to prove our main theorem, \Cref{general-mu-theorem: T}, by reducing it to the setting in which $\idealb$ is a monomial ideal of the form, for $\vv{u} > \vv{0}$, $\ideald = \ideal{x_1^{u_1}, \ldots, x_m^{u_m}}$; i.e., reduce it to \Cref{diagonal-mu-theorem: T}.
...
\daniel[inline]{By the way, it looks like we might only use this when $I$ is principal.}
Finally, for $I = \ideal{f_1, \ldots, f_n}$ arbitrary, consider the exact sequences  
\begin{align*}
 &0 \longrightarrow (J:_A I) \longrightarrow A \longrightarrow A/(J:_A I) \longrightarrow 0 \\
 &0 \longrightarrow (J:_A I) \longrightarrow A \longrightarrow \bigoplus_n A/J  
\end{align*}
where the final map of the second sequence sends $a \in A$ to $(\overline{a f_1}, \ldots, \overline{a f_n})$. 
An argument directly analogous to that above applied to these sequences yields the desired result, after noticing that $( \bigoplus \limits_n A/J ) A_p \cong \bigoplus \limits_{n} A_p/J A_p  $
\end{proof}
...
$\idealc^{[p^e]}$ by \Cref{frobenius-powers+mon-commute: L}.
 ...
 \emily[inline]{Can we instead assume $\ideala \subseteq \sqrt{\idealb}$ below? }
...
%As $\ideala_p$ is monomial,$\ideala_p^N$ ($\ideala_p^{[N]}$, respectively) is contained in $(\idealb_p)^{[p^e]}$ if any only if it is contained in $\mon((\idealb_p)^{[p^e]}) = (\mon(\idealb_p))^{[p^e]} =  \idealc_p^{[p^e]}$, where the first equality is due to \Cref{frobenius-powers+mon-commute: L}, proving the desired statement.
\end{proof}
...
Finally, we are equipped to prove \Cref{general-mu-theorem: T} from \Cref{diagonal-mu-theorem: T} by reducing to the case that $\idealb$ is a diagonal ideal. 
...
\[ \mu(\ideala_p, \idealb_p, p^e)= \mu(\ideala_p, \ideald_p, p^e) \]  where $\widetilde{\ideala}$ and $\ideald = \ideal{x_1^{u_1}, \ldots, x_t^{u_t}}$, for some positive $\vv{u} \in \NN^t$,  are ideals of $\QQ[x_1, \ldots, x_t]$.
...
\emily[inline]{The notation $\diag(-)$ used below is outdated.}
