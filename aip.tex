...
\pedro[inline]{
   A suggestion: reserve $p$ and $q$ for prime and power of prime.
   This will require us to change several uses of these guys with something else (I suggest $r$ and $s$) when they represent just arbitrary positive integers (that is, when they are used in the context of integer programs).
   It will be a bit of a pain, and maybe not practical, but I'm willing to give it a try.
}
...
It is easy to deduce from the fact that the matrix $A$ is a monomial matrix (\ie has nonnegative, nonzero rows and columns) that the feasible set of $\IP(A,\vv{u},q)$ is finite, and hence, this integer program has a well-defined value.  In fact, the discussion preceding \Cref{IP: D} demonstrates that 
%
\begin{equation}
...
\begin{remark}[Reduction to prime characteristic]
   ...
   As noted in \Cref{maximal ideals: R}, the finite generation of $A$ over $\ZZ$ implies that $R_A(\idealm)$ is a polynomial ring over the finite field $A/\idealm$, and for this reason, we may regard $I_A(\idealm)$ as a prime characteristic model of~$I$.
...
%\begin{corollary}
%\label{noncontainment mod p: C}  Let $\kk$ be a field of characteristic zero, and consider an ideal $I$ of $R=\kk[x_1, \ldots, x_\numvars]$.  If $A$ is a finitely generated $\ZZ$-subalgebra of $\kk$ such that $I_A$ is defined, then there exists a dense open set $\mathcal{U} \subseteq \mspec(A)$ such that $(A/\idealm) \otimes_A I_A \cong I_A(\idealm)$ for every $\idealm \in \mathcal{U}$.  Furthermore, $I$ is a proper ideal of $R$ if and only if $I_A(\idealm)$ is a proper of $R_A(\idealm)$ for every $\idealm \in \mathcal{U}$.
%\end{corollary}
%
%
...
%
%Next, let $\mathcal{U}$ denote the set of all points $\idealm \in \mspec(A)$ such that the characteristic of $A/\idealm$ does not divide $\ell$.  Note that $\mathcal{U}$ is an open dense (\ie open and non-empty) subset of $\mspec(A)$.  If $\idealm \in \mathcal{U}$, then $A/\idealm$ is an $A[\ell^{-1}]$-algebra, and setting $B=A/\idealm$ in \Cref{comparing expansions:  L} tells us that
%\[ (A/\idealm) \otimes_A  I_A \cong I_A \cdot (A/\idealm)[x] = I_A R_A(\idealm) = I_A(\idealm). \] 
%
%For the last statement, if $I=R$, then we may rewrite this in terms of $I_A$ as $I_A \kk[x] = \kk[x]$, which allows us to invoke \Cref{comparing expansions:  L} with $B_{\circ}=\kk$ to see that the expansion of $I_A$ to $R_A(\idealm) = (A/\idealm)[x]$ equals $R_A(\idealm)$ for every $\idealm \in \mathcal{U}$.  Similarly, if $I_A(\idealm) = R_A(\idealm)$ for some $\idealm \in \mathcal{U}$, then we may once again invoke \Cref{comparing expansions:  L}, this time with $B_{\circ} = A/\idealm$,  to see that $I=R$.
%\end{proof}
...
Next, let $\mathcal{U}$ denote the set of all points $\idealm \in \mspec(A)$ such that the characteristic of $A/\idealm$ does not divide $\ell$.  Note that $\mathcal{U}$ is an open dense (\ie open and nonempty) subset of $\mspec(A)$.  If $\idealm \in \mathcal{U}$, then $A/\idealm$ is an $A[\ell^{-1}]$-algebra, and setting $B=A/\idealm$ in \eqref{step in radical containment: e} tells us that
%
\[ J_A (\idealm) = I_A(\idealm)^t + J_A(\idealm)  \] 
%
which implies that $I_A(\idealm)^t \subseteq J_A(\idealm)$, and hence $I_A(\idealm) \subseteq \sqrt{J_A(\idealm)}$.
\end{proof}
...
\begin{proof}
Suppose that $0 \neq \ell \in A$ satisfies the conclusion of \Cref{colon mod p: L} relative to the ideal $J=I_A$ and polynomial $f \in R_A$.  We may then take $B=\kk$ in \Cref{colon mod p: L} to see that $(I_A:_{R_A} f)R = (I :_{R} f)$, which demonstrates that $(I_A :_{R_A} f)$ is an $A$-model for $(I :_R f)$.  Next, let $\mathcal{U}$ denote the set of all points $\idealm \in \mspec(A)$ such that the characteristic of $A/\idealm$ does not divide $\ell$.  If $\idealm \in \mathcal{U}$, then we may instead take $B=A/\idealm$ in \Cref{colon mod p: L} to see that 
...
\[ (\mon(I))_A(\idealm) \coloneqq \mon(I_A) R_A(\idealm) = \mon(I_A(\idealm)) \] for every point $\idealm \in \mathcal{U}$. \qed \daniel{Remove QED if we include a proof.}
\end{theorem}
...
\begin{proof}  Let $\mathcal{U} \subseteq \mspec(A)$ be a dense open set such that the conclusions of \Cref{containment in radical mod p: C} holds for $I$ and $J$, and such that the conclusions of \Cref{expansion of monomial mod most p is monomial: C,mon-operation-modulo-p: T} hold for $I$.  Fix a point $\idealm \in \mathcal{U}$.
