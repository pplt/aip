...
For the reader's convenience, we gathered in \Cref{convexity appendix} some of the terminology, basic facts, and notations associated with polyhedral geometry that will be used here and throughout the paper.
...
The previous proposition allows us to show the existence of a ``universal'' denominator for the vertices of $\opt\LP(A,\vv{u})$, dependent only on the monomial matrix $A$, and not on $\vv{u}$.
...
\begin{proof}
   ...
   By construction, each row of $\collapse{A}$ is a row of $A$, and so the constraints of $\LP(\collapse{A}, \collapse{\vv{u}})$ are a subset of those of $\LP(A, \vv{u})$.
...
Furthermore,
%
\[ \lambda q - \tail{\lambda}_q = \frac{aq-\lpr{aq}{b}}{b} \] is an integer, and in fact, is the \emph{greatest integer less than $\lambda q$}, i.e., $\lceil \lambda q \rceil-1$. 
...
\label{convexity appendix}

\pedro[inline]{
   This appendix ended up rather anemic.
   Maybe we should consider just moving these things back to the body of the paper.
}
...
If an inequality $\iprod{\defpt}{\vv{v}}\ge \alpha$ is satisfied by all points $\vv{v}$ in a polyhedron $\mathcal{P}$, then the subset of $\mathcal{P}$ where equality is attained is called a \emph{face} of $\mathcal{P}$.
Faces of dimension zero are the \emph{vertices} of $\mathcal{P}$, and can be characterized as follows.

\begin{proposition}
   \label{prop: characterization of vertices}
   Let $\mathcal{P}$ be the polyhedron defined by a system of inequalities $A \vv{x} \le \vv{b}$, where $A\in \RR^{m\times n}$, and let $\vv{v} \in \mathcal{P}$.
   For a subset $I$ of $\{1,\ldots,m\}$, let $A_I$ and $\vv{b}_I$ be the matrix and vector obtained by selecting the $i$-th rows of $A$ and $\vv{b}$, for each $i\in I$.
   Then $\vv{v}$ is a vertex of $\mathcal{P}$ if and only if there exists $I \subseteq \{1,\ldots,m\}$ such that $A_I$ is a nonsingular square matrix and $\vv{v} = A_I^{-1}\vv{b}_I$.
\qed
\end{proposition}
A proof of the above proposition can be found, for instance, in \cite[Theorem~3.34]{conforti+etal.integer_programming}.
...
Finally, when restricted to polyhedra, the relative interior simply consists of points not contained in any proper face.

We conclude this appendix with the following simple result.

\pedro[inline]{
   This seems a bit too specialized to be here.
   It's only used once, and maybe it should just be placed immediately before its use.
}
