...
\[ \witt_{e+1} = \begin{cases} \{ \vv{z} \} & \exists \text{ medium small point $\vv{z}$ in $\witt_e$}\\ \bigcup \limits_{\vv{z} \in \witt_e} \Delta(A, \vv{z},p) & \text{otherwise} \end{cases} \]
\end{enumerate}

\end{definition}


Let $\mu_e = \max \{ \mu_\ideala^{\vv{z}}(p) \ : \ \vv{z} \in \witt_e \}$.
Since each $\witt_e$ is a subset of the finite set $\Delta(A)$, there are only finitely many such sets.  Hence there exist $e, s \geq 1$ for which $\witt_{e+s} = \witt_e$.  
Then 
\[
