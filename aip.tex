...
\subsection{The degree of a monomial pair}
...
\begin{definition}[Degree of a monomial pair]
\label{FT: D}
   The \emph{degree} of a monomial pair $(\matA, \vv{u})$, denoted $\degree(\matA,\vv{u})$, is the number described in \Cref{FT descriptions: P}.
\end{definition}
...
\begin{example}[A running example]
   \label{ex: ft}
   We examine the different characterizations of $\degree(\matA,\vv{u})$ given in \Cref{FT descriptions: P}, for
...
Recall that a pair of ideals $(\ideala, \idealb)$ of a polynomial ring over a field are called \hl{\emph{compatible}} if $\idealb$ contains $\sqrt{\ideala}$, that is, there exists a positive integer $t$ such that $\ideala^t \subseteq \idealb$. \todo{Defined outside of appendix?}
...
\begin{remark}[On $p$ being sufficiently large]
   ...
   Moreover, as in this $\idealm$-primary case there are only finitely many very small pairs, $\degree(\matA,\vv{u})$ also takes on only finitely many values.\footnote{For later use, note that this conclusion remains valid without the $\idealm$-primary assumption: when restricted to very small pairs, $\degree(\matA,\vv{u})$ is bounded above by $1$, and thus takes on only finitely many values by \Cref{discreteness: P}.}
...
\begin{theorem}[Main Theorem for AIPs]
   \label{main theorem wrt diagonal: T}
   Consider a monomial matrix $\matA$, and let $d$ be a universal denominator for $\matA$, and also for each of its finitely many descendants.  Then there exist a positive integer $\beta = \beta(\matA)$, and a function that associates to every unit $r$ in $\ZZ/d\ZZ$ and monomial pair $(\matA, \vv{u})$, a pair of eventually periodic sequences $\{ \lambda_{r^s}(\matA, \vv{u})\}_{s=1}^{\infty}$  and $\{ \eta_{r^s}(\matA, \vv{u}) \}_{s=1}^{\infty}$ of rational numbers% determined by the graph $\sprout^{\ast}_r(\matA, \vv{u})$, 
...
The \emph{fundamental sequences} associated to pair $(\matA, \vv{u})$ and a positive integer $r$ are the sequences $\{ \lambda_{r^s}(\matA, \vv{u})\}_{s=1}^{\infty}$  and $\{ \eta_{r^s}(\matA, \vv{u}) \}_{s=1}^{\infty}$ referred to in the statement of \Cref{main theorem wrt diagonal: T}, and explicitly defined in its proof.  In other words,
	%
\begin{align*}
...
We now our focus on the fundamental sequences described in \Cref{fundamental-sequences: D}.  More precisely, our goals are to demonstrate that these sequences behave as asserted in \Cref{main theorem wrt diagonal: T}, and also to determine their values in some important special cases.    We start with a preliminary result.
\newcommand{\rr}{{r}}


\begin{lemma}
   \label{fundamental-sequences: L}
   If $(\matB, \vv{v})$ is a $\rr$-sprout of $(\matA, \vv{u})$, then $\degree(\matB,\vv{v}) \leq \udeficit(\matA, \vv{u}, \rr) $.  Furthermore, if $\ideala$ is an $\idealm$-primary monomial ideal generated by monomials that are all of the same degree, with respect to some positive $\ZZ$-grading, and $\matA$ is the monomial matrix associated to these generators, then $\degree(\matB,\vv{v}) = \udeficit(\matA, \vv{u}, \rr)$. 
\end{lemma}
...
\begin{proof}
   ...
   \Cref{collapse of Newton polyhedron: P} tells us that $\collapse{\O}$ is a face of $\collapse{\N}$, the Newton polyhedron of $\matB=\collapse{\matA}$, defined in $\collapse{\N}$ by the collapsed point $\collapse{\vv{c}}$.  It then follows from \Cref{FT descriptions: P}\eqref{min} that $ \degree(\matB,\vv{v}) \leq \iprod{\collapse{\vv{c}}}{\vv{v}}$. 
...
\end{equation}
%
where the first equality follows from our description of $\vv{v}$, the second equality from \eqref{norm of tail: e}, and the inequality from \eqref{bound in inner product: e}.  However, by definition, the right-most term above equals  $\udeficit(\matA, \vv{u}, \rr)$.  In summary, we have just demonstrated that \[ \deg(\matB, \vv{v}) \leq \iprod{\collapse{\vv{c}}}{\vv{v}} \leq \udeficit(\matA, \vv{u}, \rr).\]  

To address the second assertion, we will show that, whenever $\matA$ is the monomial matrix associated to such a generating set,  each of these inequalities is an equality.

To start, note that, in this context, the Newton polyhedron $\N$ is particularly simple.  Indeed, it has a unique standard face $\O$, which is compact, being defined by the positive point $\vv{c}$ that is a rescaling of the weight vector associated to the ambient positive $\ZZ$-grading.   In particular, the collapse along $\O$ is the identity, and so $\matB = \matA$, and $\collapse{\vv{c}} = \vv{c}$.  Furthermore, as $\O$ is the unique standard face of $\N = \collapse{\N}$, it follows from \Cref{FT descriptions: P}\eqref{new ip} that $\deg(\matB, \vv{v}) = \deg(\matA, \vv{v}) = \iprod{\vv{c}}{\vv{v}}$.  On the other hand, as every column of $\matB = \matA$ lies on $\O$, a direct calculation shows that $\iprod{\overline{\vv{c}}}{\matB \vv{h}} = \norm{\vv{h}}$, and substituting this into \eqref{lemma-fundamental-sequences-nonnegative: e} implies that $\iprod{\vv{c}}{\vv{v}} = \udeficit(\matA, \vv{u}, \rr)$.  
\end{proof}

\begin{proposition}
\label{finishing-with-the-fundamental-sequences: P}
Given a pair $(\matA, \vv{u})$ and positive integer $r$, the fundamental sequences $\{ \lambda_{r^s}(\matA, \vv{u})\}_{s=1}^{\infty}$  and $\{ \eta_{r^s}(\matA, \vv{u}) \}_{s=1}^{\infty}$ satisfy the conditions described in \Cref{main theorem wrt diagonal: T}.  That is, each is an eventually periodic sequence of rational numbers, with each term $\lambda_{r^s}(\matA, \vv{u})$  nonnegative, and each term $\eta_{r^s}(\matA, \vv{u})$  positive.
\daniel[inline]{Add that these sequences only depend on $r$ modulo a universal denominator for $\matA$, and all of its descendants?}
\end{proposition}

\begin{proof}
   Fix $(\matA,\vv{u})$ and $r$ as above, and set $\mathcal{E}=\mathcal{E}_r(\matA,\vv{u})$ and $\Q = \Q_r(\matA,\vv{u})$.  
   
   By definition, every $\delta_{r^s}(\matA, \vv{u})$ and $\delta_{r^s}(\matA, \vv{u})$ is rational, and thus, so is each term $\eta_{r^s}(\matA, \vv{u})$ and $\lambda_{r^s}(\matA, \vv{u})$.  When $\mathcal{E} = \infty$,  each such term is eventually constant, and when $\mathcal{E} \neq \infty$,  the eventually periodicity of the graph $\sprout_r^{\ast}(\matA, \vv{u})$ described by \Cref{elementary2: R}\eqref{elementary2 periodic in e} implies that each such term is eventually periodic in $s$.  
...
We now focus on determining the values of the terms $\lambda_{r^s}(\matA, \vv{u})$ and $ \eta_{r^s}(\matA, \vv{u})$ in some important special cases.  To do so, we require another preliminary result.

\begin{lemma}[On $1$-sprouts of very small pairs] 
\label{1-sprouts: L}
The collapse of a very small pair $(\matA, \vv{u})$ along its minimal face $\O = \mf(\matA, \vv{u})$ is a $1$-sprout of $(\matA, \vv{u})$.  Furthermore, if $(\matB, \vv{v})$ is any $1$-sprout of $(\matA, \vv{u})$, then $\udeficit(\matB, \vv{v}, 1) = \deg(\matB, \vv{v}) \leq \deg(\matA, \vv{u})$.
\end{lemma}

\begin{proof}
Fix a rational special point $\vv{s}$  for $(\matA, \vv{u})$ with denominator $\denom$.  Then, as $(\matA, \vv{u})$ is very small, we have that $\norm{\vv{s}} = \degree(\matA ,\vv{u}) \leq 1$, and so that we can write $\vv{s} = \vv{t}/\denom$, where $\vv{t}$ has nonnegative integer coordinates bounded above by $\denom$.  Thus,  by definition, $\tail{s_i}_1$ equals $\lpr{t_i}{d}/d = t_i/d = s_i$ if $t_i \neq 0$, and equals $0$ otherwise.

In other words, $\tail{\vv{s}}_1 = \vv{s}$.  Thus, if we set $\ip = \ip(\matA, \vv{u}, \vv{s}, 1)$, then \[ 0 \leq \val \ip < \norm{\tail{\vv{s}}_1} = \norm{\vv{s}} = \degree(\matA, \vv{u}) \leq 1\]   where the strict inequality follows from \Cref{bounded value: P}.   We conclude that  $\val \ip = 0$.

 Hence,  $\vv{0}$ is optimal for $\Theta$, and so by definition, the point $\collapse{A} \tail{\vv{s}}_1 - \collapse{A} \vv{0}$ lies in $\short \Theta = \ushort(\matA, \vv{u}, 1)$, where the overbar denotes collapse along $\O$. However, as $\tail{\vv{s}}_1 = \vv{s}$, this point may be rewritten as 
$\collapse{\matA} \tail{\vv{s}}_1 - \collapse{\matA} \vv{0}  = \collapse{\matA} \vv{s} = \collapse{\matA \vv{s}} = \collapse{\vv{u}}$ where the last equality follows from the fact that $\vv{s}$ is special for $(\matA, \vv{u})$.   In any case, we see that $\vv{u} \in \ushort(\matA, \vv{u}, 1)$, and so by definition, $(\collapse{\matA}, \collapse{\vv{u}})$ is a $1$-sprout of $(\matA, \vv{u})$.

We also record another consequence of the fact that $\val \Theta = 0$, namely, that 
\begin{equation}
...
\begin{proposition}
	\label{important-special: P}
	   Consider a monomial matrix $\matA$, a universal denominator $d$ for $\matA$, and also for each of its finitely many descendants, and a positive integer $r$ with $r \equiv 1 \bmod d$.  If $(\matA, \vv{u})$ is very small, then collapse of this pair along its minimal face $\O = \mf(\matA, \vv{u})$ lies in $\S^{\ast}_{r^e}(\matA, \vv{u})$ for every integer $e>1$.  Consequently, \[ \lambda_{r^e}(\matA, \vv{u}) = 0 \text{ and } \eta_{r^e}(\matA, \vv{u}) = \deg(\matA, \vv{u})\] for every pair $(\matA, \vv{u})$ that is very small, and every positive integer $e$.
	\end{proposition}

\begin{proof}  Let an overbar denote collapse along $\O = \mf(\matA, \vv{u})$, and fix an integer $r$ as above.  Note that \Cref{pair periodicity: R}, along with the assumption that $r \equiv 1 \bmod d$, implies that $\udeficit(\matB, \vv{v}, r) = \udeficit(\matB, \vv{v}, 1)$ for every pair $(\matB, \vv{v})$ with $\matB$ is a descendant of $\matA$.  Thus, in this context, the notions of $r$-sprout and $1$-sprout coincide.  

By definition, $\S^{\ast}_{r}(\matA, \vv{u})$ consists of the single vertex $(\matA, \vv{u})$, and $\S_{r^2}(\matA, \vv{u})$ consists of every $1$-sprout $(\matB, \vv{v})$ of $(\matA, \vv{u})$.   Lemma \ref{1-sprouts: L} then implies that every such $(\matB, \vv{v})$ is very small, and so by definition, $\S^{\ast}_{r^2}(\matA, \vv{u})$ consists of all $1$-sprouts $(\matB, \vv{v})$ of $(\matA, \vv{u})$ that maximize $\deg(\matB, \vv{v})$, and only then minimize $\udeficit(\matB, \vv{v})$.  Invoking Lemma \ref{1-sprouts: L} again, we see that the $1$-sprout $(\collapse{\matA}, \collapse{\vv{u}})$ of $(\matA, \vv{u})$ satisfies these criteria.

Next, suppose that $e$ is greater than $1$, and that $(\collapse{\matA}, \collapse{\vv{u}})$ lies in $\S^{\ast}_{r^e}(\matA, \vv{u})$.   Lemma \ref{1-sprouts: L} tells us that $(\collapse{\matA}, \collapse{\vv{u}})$ is a $1$-sprout of itself, and hence, lies in $\S_{r^{e+1}}(\matA, \vv{u})$.  In fact, Lemma \ref{1-sprouts: L} also implies that $(\collapse{\matA}, \collapse{\vv{u}})$ lies in $\S^{\ast}_{r^{e+1}}(\matA, \vv{u})$.  We leave the verification of this, which is similar to the base case considered above, to the reader.

We have just seen that $(\matA, \vv{u}) \in \S_r^{\ast}(\matA, \vv{u})$, and that $(\collapse{\matA}, \collapse{\vv{u}})$ lies in $\S_{r^e}^{\ast}(\matA, \vv{u})$ for every $e>1$  Using these points, we compute that $\epsilon_{p^e}(\matA,\vv{u}) = \delta_{p^e}(\matA,\vv{u}) = \degree(\matA,\vv{u})$ for all positive $e$, and the asserted values of the fundamental sequences follow.
\end{proof}


The next result is an immediate corollary of \Cref{main theorem wrt diagonal: T} and \Cref{important-special: P}.

\begin{corollary}[An important special case]
	\label{important-special: C}
   Consider a monomial matrix $\matA$, and let $d$ be a universal denominator for $\matA$, and also for each of its finitely many descendants.  Then there exists $\beta = \beta(\matA)$ such that if $p>\beta$ and $p \equiv 1 \bmod \denom$, then
\[
 \val\IP_p(\matA,\vv{u},p^e) = (p^e-1) \degree(\matA,\vv{u})
\]
for every very small $\matA$-pair $(\matA, \vv{u})$, and all $e \geq 1$.
In particular, for such $p$,
\[ \val \fip_p (\matA,\vv{u}) = \degree(\matA,\vv{u}).\]
\end{corollary}

\daniel[inline]{Another special case might be considered below}
...
We start with a couple of technical lemmas concerning the lifting of collapsed monomial pairs.
