...
\begin{example}
   ...
   The figure shows $\S_{p^e}(A,\vv{u})$ for $1 \le e \le 4$, with $\sprout^*_p(A,\vv{u})$ highlighted in green.
   The framed nodes, highlighted in red, are pairs that are not very small, which cause $\sprout^*_p(A,\vv{u})$ to terminate, as $\S_{p^e}(A,\vv{u})$ is empty for $e\ge 5$, and $\S_{p^e}^*(A,\vv{u})$ is empty for $e\ge 4$.
...
Below we present the proof of all claims above except the nonpositivity of the non-leading coefficients in $\val \IP_p(A,\vv{u},p^e)$, which is postponed until \Cref{non-leading coefficients: P}.
...
& = \ft{\ideala}{\ideald} -  \!\! \sum_{1 \le s < \log_p\!\! \Q} \!\!\! \frac{\delta_{p^s}(A,\vv{u})-\epsilon_{p^{s+1}}(A,\vv{u})}{p^s} -  \frac{ \delta_{\Q}(A,\vv{u})-1}{\Q}
\end{align*}
