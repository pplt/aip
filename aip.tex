...
\begin{proof}
   We will show the existence of $0 \ne \ell \in A$ with the following properties:
...
\begin{lemma}
   ...
   Suppose that $A$ is a Noetherian domain.
   Given an ideal $J$ of the polynomial ring $A[x_1, \ldots, x_\numvars] = A[x]$, there exists $0 \neq \ell \in A$ with the following property:
   If $B$ is any $A[\ell^{-1}]$-algebra \hl{that happens to be a field}, then
   \[ \mon(J) B[x] = \mon(JB[x]).\]
\end{lemma}

\begin{proof}
   Choose $0 \neq \ell \in A$ such that the conclusion of \Cref{uniform monomial containment: L} holds.
   By \Cref{single multiplier: R}, there exists $0 \neq a \in A$ such that $x^{\vv{u}} \in \mon(J)$ if and only if $ax^{\vv{u}} \in J$.
