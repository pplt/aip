...
% for
% Now let $\lambda = \ft{A}{\vv{u}}$, and let an overbar denote collapse along $\O = \mf(A, \vv{u})$, so that $\ft{\overline{A}}{\overline{\vv{u}}} = \lambda$ by \Cref{collapse of mf and mc: P}.
% Moreover, as pointed out in the proof of \Cref{bounded value: P}, $\vv{0}$ is feasible for $\Theta$, so $\overline{\vv{u}} = \overline{A}\vv{s} = \overline{A}\tail{\vv{s}}_p - \overline{A} \, \vv{0} \in \ushort(A, \vv{u}, p)$.
% Hence $(\overline{A}, \overline{\vv{u}})$ is a $p$-sprout of $(A, \vv{u})$.
% Moreover, noting \Cref{comparing deltas: R}, we see that $\udeficit(\overline{A}, \overline{\vv{u}}, p) = \udeficit(A, \vv{u}, p) = \norm{\vv{s}} = \lambda$.
% 
% Given an arbitrary $p$-sprout $(\overline{A}, \vv{v})$ of $(A, \vv{u})$, we have that  
% $\ft{\overline{A}}{\vv{v}} \leq 
% \udeficit(A, \vv{u}, p) = \lambda$ by \Cref{non-leading coefficients: L}.
% %,  and in particular, $(\overline{A}, \overline{\vv{u}})$ is very small.   
% Moreover, if $\ft{\overline{A}}{\vv{v}}$ achieves the maximum value among all such $p$-sprouts, namely $\lambda = \ft{\overline{A}}{\overline{\vv{u}}}$, then by our claim \eqref{very small udef: e} above, we have that $\udeficit(\overline{A},\vv{v},p) = \lambda = \udeficit(\overline{A}, \overline{\vv{u}}, p)$. 
% Hence $(\overline{A}, \overline{\vv{u}}) \in \S_{p^2}^\ast(A, \vv{u})$. 
% 
% 
% Finally, \Cref{main theorem wrt diagonal: T} now confirms that $\val_p(A, \vv{u}, p^e) = p^e \lambda - \lambda = (p^e-1)\lambda$.
\end{proof}
