...
\begin{proof}
   ...
   \begin{equation}
      ...
      \vv{v}p^e-A\vv{k}
   \end{equation}
   thus shows that $\vv{u}qp^e-A \vv{g}$, an arbitrary point in $\short\IP_p(A, \vv{u},q p^e)$, lies in $\short \IP_p(A, \vv{v},p^e)$.
   This shows that
   \begin{equation}
...
\end{cases}
\]
On the other hand, these values are ``switched'' for $\IP(A, \vv{v}, p^e)$: $\big( \frac{5p^e-3}{4}, \frac{p^e-3}{4} \big)$ is an optimal point for odd $e$,  and $\big( \frac{5p^e-1}{4},\frac{p^e-1}{4} \big)$ is one for even $e$, 
so that 
the program's value is $\frac{3p^e-3}{2}$ when $e$ is odd, and equals $\frac{3p^e-1}{2}$ for even $e$.
\end{remark}
