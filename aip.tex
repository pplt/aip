...
\begin{enumerate}
	...
	\item[$\Box$] Address Pedro's comment after the proof of \ref{non-leading coefficients: L}. 
...
We now turn our focus to the fundamental sequences considered in \Cref{fundamental-sequences: D}.

\begin{lemma}
   \label{non-leading coefficients: L}
   If $(\matB, \vv{v})$ is a $p$-sprout of $(\matA, \vv{u})$, then
   \[
      \udeficit(\matA, \vv{u}, p) \geq \degree(\matB,\vv{v})
   \]
...
\begin{proof}
   ...
   \Cref{FT descriptions: P}\ref{lambda} then shows that $\frac1{\degree(\matB,\vv{v})}\cdot \vv{v} \in \collapse{\N}$, so by \Cref{alpha=1: convention} we have $\iprod{\collapse{\vv{c}}}{\vv{v}} \ge \degree(\matB,\vv{v})$.
   ...
   \[
      \degree(\matB,\vv{v}) \le \iprod{\collapse{\defpt}}{\vv{v}} \leq \norm{\tail{\vv{s}}_p} - \val \ip= \udeficit(\matA, \vv{u}, p).
   \]
