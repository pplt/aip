...
\label{lifted-programs: L}
 {Suppose that $\matB$ is a monomial matrix obtained by deleting some rows a monomial matrix $\matA$ with $m$ rows.    In fact, for the sake of concreteness, suppose that $\matB$ consists of the first $\ell$ rows of $\matA$, and is monomial.   Given a positive point $\vv{v} \in \mathbb{Z}^{\ell}$, there is a positive point $\vv{u} \in \ZZ^{\numvars}$ such that $u_i = v_i$ for all $1 \leq i \leq \ell$, and such that the values of $\IP_p(\matA, \vv{u}, p^e)$ and $\IP_p(\matB, \vv{v}, p^e)$ coincide for all $p$ and $e$.}
\end{lemma}
...
\begin{theorem}[Uniform denominator for vertices of $\opt \LP(\matA,\bullet)$]
...
\begin{proof}
   By \Cref{opt set: P}, $\opt \LP(\matA,\vv{u})$ is defined by the inequalities $\matA\vv{s} \le \vv{u}$ and $\vv{s}\ge \vv{0}$, where equality is required in a prescribed set of coordinates.
   Thus, if $\matM$ is the matrix obtained by stacking $\matA$, $-\matA$, the identity matrix $I_n$, and $-I_n$, and $\vv{b}$ is the column vector obtained by stacking $\vv{u}$, $-\vv{u}$, and $\vv{0}\in \RR^{2n}$, then $\opt \LP(\matA,\vv{u})$ is defined by a subsystem of $\matM \vv{x} \le \vv{b}$.
   Let $\denom$ be the least common multiple of the nonzero $n\times n$ minors of $\matM$.
   Then by \Cref{prop: characterization of vertices}, every vertex of $\opt \LP(\matA,\vv{u})$ is rational, with denominator $\denom$.
\end{proof}
...
\begin{example}[Running example: an auxiliary integer program]
   ...
   \begin{figure}
      ...
      \caption{The feasible set for the integer program $\ip(\matA,\vv{u},\vv{s},q)$ in \Cref{ex: ft.3}, with $\lpr{q}{17}=11$}
      \label{fig: Theta}
   \end{figure}
...
Now that we have a grasp on the value of the arithmetic integer program $\Pi_p(\matA, \vv{u}, p^e)$ when the matrix $\matA$ is associated to an $\idealm$-primary monomial ideal, we broaden our scope to address the case of an arbitrary monomial matrix.
...
If $\Theta = \Theta(\matB, \vv{v}, \vv{s}, p)$, we have that $0 \leq \val \Theta < \norm{\vv{s}} = \ft{\matB}{\vv{v}} \leq 1$  by \Cref{bounded value: P}, forcing the value of the integer program $\Theta$ to be $0$.
Hence $\udeficit(\matB, \vv{v}, p) = \deficit \Theta = \norm{\vv{s}} - \val\Theta = \ft{\matB}{\vv{v}}$, proving the claim.\\[2mm]
...
The proof of the claim shows that a special point $\vv{s}$ for $(\matB,\vv{v})$ with denominator $\denom$ satisfies $\tail{\vv{s}}_p = \vv{s}$, and $\vv{0} \in \opt \Theta(\matB,\vv{v},\vv{s},p)$.
...
By the special case considered above, there is a pair $(\matA_i, \vv{u}_i)$ such that, when $\LL=\kk$ is a field of characteristic $p>0$, then $\nu(\ideala, \idealc_i, p^e) = \val \IP(\matA, \vv{u}_i, p^e)$ and $\mu(\ideala, \idealc_i, p^e) = \val \IP_p(\matA, \vv{u}_i, p^e)$ for every positive integer $e$, where $\idealc_i = \mathcal{C}_i \LL[x]$.    It follows from this, \eqref{irreducible-decomposition: e}, and the fact that standard Frobenius powers commute with intersection in $\kk[x]$ | see, e.g., \cite[Lemma 2.3]{blickle+mustata+smith.discr_rat_FPTs} | \pedro{Maybe include reference?} 
...
\begin{remark}[Generic freeness]\label{generic-freeness}
   Consider a Noetherian domain $\ringA$, and $\ringS$ a finitely generated $\ringA$-algebra.
...
\begin{definition}[Reduction to prime characteristic]
   ...
   If $\ringA$ is a finitely generated $\ZZ$-subalgebra of $\kk$, then we set $\ringR_\ringA \coloneqq \ringA[x]$, and if $I$ is an ideal of $\ringR$, then $I_\ringA$ denotes any ideal of $\ringR_\ringA$ that extends to $I$ under the inclusion $\ringR_\ringA \hooklongrightarrow \ringR$.   We call such an ideal $I_\ringA$ an $\ringA$-model of $I$, and when such an ideal exists, we say that $\ringA$ \emph{supports} {the model $I_\ringA$ of $I$}.\footnote{Note that every ideal $I$ of $\ringR$ has an $\ringA$-model for a suitable $\ringA$: one may take $\ringA$ to be the $\ZZ$-subalgebra of $\kk$ generated by the coefficients appearing in a finite generating set for $I$.}

   If $I_\ringA$ exists and $\idealm \in \mspec(\ringA)$, then $I_\ringA(\idealm)$ is the extension of $I_\ringA$ to the ring $\ringR_{\ringA}(\idealm) \coloneqq (\ringA/\idealm)[x]$ under the quotient map $\ringR_\ringA \longrightarrow \ringR_\ringA(\idealm)$.
...
\begin{proof}
   ...
   Let $0 \leq \rho \leq \infty$ denote the rank of the free $\ringA[\ell^{-1}]$-module $\ringA[\ell^{-1}] \otimes_\ringA (J/I)$, and let $\ringB$ be an $\ringA[\ell^{-1}]$-module.
...
% 
%   Localizing the exact sequence $0 \to I \to J \to J/I \to 0$ at $\ell \in \ringA \setminus \{0\}$, we obtain the exact sequence of $\ringA[\ell^{-1}]$-modules
%   \[0 \to \ringA[\ell^{-1}] \otimes_\ringA I \to \ringA[\ell^{-1}] \otimes_\ringA J \to \ringA[\ell^{-1}] \otimes_\ringA (J/I) \to 0.\]
%
%   By generic freeness, we may assume that $\ringA[\ell^{-1}] \otimes_\ringA (J/I)$ is a free $\ringA[\ell^{-1}]$-module of rank $0 \leq \rho \leq \infty$, so the localized sequence above is split exact.
%   Split exactness and free-rank is preserved by base change, so tensoring with an $\ringA[\ell^{-1}]$-algebra $\ringB$ over $\ringA[\ell^{-1}]$ gives the split exact sequence of $\ringB$-modules 
%   \begin{equation}
%      \label{eq: exact seq}
%      0 \to \ringB \otimes_\ringA I \to \ringB \otimes_\ringA J \to \ringB \otimes_\ringA (J/I) \to 0
%   \end{equation}
%   that terminates on a free $\ringB$-module of rank $\rho$.   
%   
%   In particular, tensoring the containment $I \subseteq J$ with $\ringB$ over $\ringA$ yields an inclusion of $\ringB$-modules $\ringB\otimes_\ringA I \hookrightarrow \ringB\otimes_\ringA J$.  In fact, after replacing $\ell$ by a nonzero $\ringA$-multiple, we may also assume that $\ringA[\ell^{-1}] \otimes_\ringA (\ringA[x]/J)$ is free over $\ringA[\ell^{-1}]$, and an analogous argument shows that tensoring the containment $J \subseteq \ringA[x]$ with $\ringB$ over $\ringA$ yields an inclusion of $\ringB$-modules $\ringB \otimes_\ringA J \hookrightarrow \ringB \otimes_\ringA \ringA[x] \cong \ringB[x]$.  Thus, there is a resulting string of inclusions $\ringB \otimes_\ringA I \hookrightarrow \ringB \otimes_\ringA J \hookrightarrow \ringB[x]$, and by examining the corresponding images in $\ringB[x]$, we may identify this with the containments $I \ringB[x] \subseteq J \ringB[x] \subseteq \ringB[x]$,  and in light of this, \eqref{eq: exact seq} then identifies $\ringB \otimes_\ringA (J/I)$ with $J \ringB[x]/I \ringB[x]$.
% 
% Finally, for the last statement, simply note that $\rho$, the rank of the free $\ringB$-module $\ringB \otimes_\ringA (J/I) \cong J \ringB[x]/I \ringB[x]$, is independent of the choice of $\ringA[\ell^{-1}]$-algebra $\ringB$.   
\end{proof}
...
These isomorphisms illustrate that  \Cref{reduction: D} is consistent with other standard notions of reduction to prime characteristic considered in the literature.  For example, the {conditions $I_\ringA \ringR = I$ on $I_\ringA$ and $\kk \otimes_\ringA I_\ringA \cong I$} are equivalent, and though $I_\ringA \ringR_\ringA(\idealm)$  may differ from $(\ringA/\idealm) \otimes_\ringA I_\ringA$ for some $\idealm \in \mspec(\ringA)$, they are isomorphic for all $\idealm$ in some dense open subset of $\mspec(\ringA)$.  In particular, every result below concerning the expansions $I_\ringA(\idealm) =I_\ringA \ringR_\ringA(\idealm)$ for all $\idealm$ in such a subset of $\mspec(\ringA)$ is valid for $(\ringA/\idealm) \otimes_\ringA I_\ringA$ for all $\idealm$ in another such subset of $\mspec(\ringA)$.
\end{remark}
...
\item \label{reduction preserves being monomial} If $I$ is a monomial ideal generated by a collection of monomials $G$, then $I_\ringA(\idealm) = G \ringR_\ringA(\idealm)$ for every point $\idealm$ in some dense open subset of $\mspec(\ringA)$.
\item \label{reduction preserves compatibility} \hl{If $(I,J)$ is a (small) compatible pair, then so are almost all of its reductions.  More precisely, If $I^t \subseteq J$ for some natural number $t$, then $I_\ringA(\idealm)^t \subseteq J_\ringA(\idealm)$ for every point $\idealm$ in some dense open subset of $\mspec(\ringA)$.}
...
%
%For the last assertion, for every $\ringA$-algebra $\ringB$, consider the canonical map 
%\[ \theta_\ringB:  \ringB[x] {\longrightarrow} M_\ringB \coloneqq \ringB[x]/ I_\ringA \ringB[x] \oplus \ringB[x]/ J_\ringA \ringB[x] \] 
%%
%
%By generic freeness, we may fix  $\ell \in \ringA \setminus \{ 0 \}$ such that  $\ringA[\ell^{-1}] \otimes_\ringA (M_\ringA / \im (\theta_\ringA))$ is free over $\ringA[\ell^{-1}]$.  Thus, if $\ringB$ is any $\ringA[\ell^{-1}]$-algebra, then applying $N \mapsto \ringB \otimes_\ringA N$ to the short exact sequence
%$0 \to \im(\theta_\ringA) \to M_\ringA \to  M_\ringA / \im(\theta_\ringA) \to 0$ 
%yields a split exact sequence of $\ringB$-modules $0 \to \ringB \otimes_\ringA \im(\theta_\ringA) \to \ringB \otimes_\ringA  M_\ringA \to \ringB \otimes_\ringA (M_\ringA/\im(\theta_\ringA)) \to 0$ 
%and the reader may verify that the composition $\ringB \otimes_\ringA \im(\theta_\ringA) \hookrightarrow \ringB \otimes_\ringA M_\ringA \cong M_\ringB$ induces an isomorphism $\ringB \otimes_\ringA \im(\theta_\ringA) \cong \im(\theta_\ringB)$.
\end{proof}

\begin{example} The dense open set referenced in \Cref{reduction basics: C} \ref{reduction preserves being monomial} may not be all of $\mspec(\ringA)$, even when $\ringA=\ZZ$.  For an extreme example,  note that the ideal $I_{\ZZ} = \ideal{2x, 3x+y} \subseteq \ZZ[x,y]$ is a $\ZZ$-model of $I=\langle x, y \rangle \subseteq \QQ[x,y]$, and through $I_{\ZZ} \bmod p$ is what one expects when $p \neq 2$, the reduction $I_{\ZZ} \bmod 2  = \langle x+y \rangle \subseteq \FF_2[x,y]$ is especially far from being a monomial ideal, given that it contains no monomials.  
\end{example}
...
By  \Cref{comparing expansions: P}, there exists a nonzero element $\ell \in \ringA$ such that \[  \ringB \otimes_\ringA ((f \ringA[x] + J)/J)  \cong (fB[x] + J \ringB[x])/J \ringB[x]  \] for every  $\ringA[\ell^{-1}]$-algebra $\ringB$.  Thus, the exact sequence of $\ringA$-modules 
\[  
...
\begin{proof}   Let $\ringA, I_\ringA, J_\ringA$, and $L_\ringA$ be as above.  A standard application of \Cref{comparing expansions:  P} shows that $I_\ringA(\idealm)^t \not \subseteq J_\ringA(\idealm)$ for all $\idealm$ in some dense open subset $\mathcal{U}$ of $\mspec(\ringA)$, and \Cref{reduction basics: C}  \eqref{reduction preserves compatibility} tells us that, after possible shrinking $\mathcal{U}$, we may also assume that $I_\ringA(\idealm)^{t+1} \subseteq J_\ringA(\idealm)$ for all $\idealm \in \mathcal{U}$. In addition,  \Cref{reduction basics: C} \eqref{different models of same ideal}
 ...
 Replacing $\mathcal{U}$ with a dense open subset, we may even assume that the characteristic of $\ringA/\idealm$ is greater than $t+1$ for every $\idealm \in \mathcal{U}$, and the result then follows from \Cref{compatible to small compatible: L} with $p = \operatorname{char}(\ringA/\idealm)$, and $\ideala = I_\ringA(\idealm), \idealb = J_\ringA(\idealm)$, and $\idealc = L_\ringA(\idealm)$.
 \end{proof}
...
\begin{theorem}
   ...
   If $\idealm \in \mathcal{U}, p = \operatorname{char}(\ringA/\idealm)$, then 
   \[
...
We claim that each of these statements hold for all points $\idealm$ in some dense open subset of $\mspec(\ringA)$.  For \eqref{step-3-mu-reduction: e}, this simply follows from  \Cref{reduction basics: C} \eqref{reduction preserves being monomial}.  

For \eqref{step-2-mu-reduction: e},  \Cref{mon-operation-modulo-p: T} implies that $\mon(L_\ringA)$ is an $\ringA$-model for $\mon(L)$, and our choice of $\mathcal{C}$ guarantees that $\mathcal{C} \ringR_\ringA$ is another $\ringA$-model for $\mon(L)$.  In light of this redundancy, \Cref{reduction basics: C} \eqref{different models of same ideal} implies that $\mon(L_\ringA) \ringR_\ringA(\idealm) = \mathcal{C} \ringR_\ringA(\idealm)$ for all $\idealm$ in some dense open subset of $\mspec(\ringA)$.  On the other hand, \Cref{mon-operation-modulo-p: T} also tells us that $\mon(L_\ringA) \ringR_\ringA(\idealm) = \mon(L_\ringA(\idealm))$ for all $\idealm$ in some dense open subset of $\mspec(\ringA)$, and combining this with the preceding observation, we conclude that \eqref{step-2-mu-reduction: e} also holds for all $\idealm$ in some dense open subset of $\mspec(\ringA)$.  
...
\begin{theorem}
   ...
   \[
      \nu(I_\ringA(\idealm), J_\ringA(\idealm), p^e) = \max \val \IP (\matM, \vv{u}_i, p^e).  \qed
   \]
\end{theorem}
