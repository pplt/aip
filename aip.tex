...
\begin{remark}[An algorithm]
   \label{algorithm: R}
   Like in the $\idealm$-primary case, we have an algorithm that, at least in theory, returns the formulas given in \Cref{main theorem wrt diagonal: T} for \emph{all} $e \ge 1$.
   This algorithm is based on determining the entire structure of $\sprout^*_p(A,\vv{u})$, which is possible because either this graph is finite or levels eventually repeat cyclically, as pointed out in \Cref{elementary2: R}\eqref{elementary2 periodic in e}.
   ...
   Moreover, the pseudocode presented in Algorithm~\ref{alg} applies to the general case with minimal modifications:
