...
\While{\texttt{\em true}}{
   ...
   $\text{\texttt{max}} \leftarrow \max \{\ft{A}{\vv{v}}: (A,\vv{v}) \in \S_{p^e}\}$\;
   \If{$\text{\em \texttt{max}} >1$}{
      \KwRet{$\frac{\sum_{1\le i<e}M_i\tt^i}{1-\pp\tt} + \frac{(\pp - 1)\tt^e}{(1-\pp \tt)(1-\tt)}$}
   }
   $\S^*_{p^e} \leftarrow \{(A,\vv{v})\in \S_{p^e}: \ft{A}{\vv{v}} = \text{\texttt{max}}\}$\;
   $\text{\texttt{min}} \leftarrow \min\{\udeficit(A,\vv{v},p_0): (A,\vv{v})\in \S^*_{p^e}\}$\;
   $\S^*_{p^e} \leftarrow \{(A,\vv{v})\in \S^*_{p^e}: \udeficit(A,\vv{v},p_0) = \text{\texttt{min}}\}$\;
   $M_e \leftarrow p\, \text{\texttt{max}} - \text{\texttt{min}}$\;
}
\end{algorithm}
...
\begin{proof}
   ...
   Then for $p>\beta$, $\omega_1 \cdot p^{e-1} + \cdots + \omega_{e-1} \cdot p + \omega_e$ is at most $\lambda(p^{e-1} + p^{e-2} + \cdots + 1) = \lambda \cdot \frac{p^e-1}{p-1}$, which, in turn, is less than $\lambda (p^e-1) \cdot w/\lambda < w p^e$.
\end{proof}
...
\begin{remark}[Elementary observations]
   ...
   \begin{enumerate}
      ...
      Similarly, arguing as in \Cref{elementary1: R}\eqref{elementary1 periodic in p}, the graph $\sprout_p^{\ast}(A,\vv{u})$ depends only on $p$ modulo any universal denominator $\denom$ for $A$.
      \pedro{$A$ and descendants?}
   \end{enumerate}
\end{remark}
...
\begin{proposition}
   ...
   Suppose $p > \beta$.
   If  $(A,\vv{u})$ is very small, $e$ is a natural number, and $\S_{p^e}^{\ast}(A,\vv{u})$ is non-empty, then
   \[
      \max \{ \val \IP_p(B,\vv{v},p) : (B,\vv{v}) \in \S_{p^e}(A,\vv{u}) \} = \epsilon_{p^e}(A,\vv{u}) \cdot p-\delta_{p^e}(A,\vv{u})
   \]
...
\begin{proof}
   ...
   In particular, for such primes, $\val \IP_p(B,\vv{v}) = \ft{B}{\vv{v}}p-\udeficit(B,\vv{v},p)$  for each vertex $(B,\vv{v})$ of $\sprout_p^{\ast}(A,\vv{u})$, and the existence of $\beta(A)$ as above then follows from arguments similar to those made in \Cref{p large m-primary: R}.
\end{proof}
