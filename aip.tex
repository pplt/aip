...
We review in this subsection some of the terminology and notation concerning euclidean spaces and convex geometry used throughout the paper.
...
In the same vein, operations on numbers are extended to points in $\RR^n$ in a coordinatewise fashion; for instance, $\up{\vv{u}}=(\up{u_1},\cdots,\up{u_n})$.
...
Turning to concepts and constructions of convex geometry,
\pedro{If anyone wishes, we can make this more formal: definitions, remarks, theorems\ldots}
a (convex) \emph{polyhedron} in $\RR^n$ is a subset of $\RR^n$ obtained by intersecting finitely many closed halfspaces, that is, a set consisting of all points $\vv{x}\in \RR^n$ satisfying an inequality of the form $A\vv{x}\le \vv{u}$, where $A$ is a matrix with $n$ columns.
