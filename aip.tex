...
\end{equation}
In \Cref{ft as val LP: P}, we realize this limit as the value of a certain linear program.
...
\begin{proof}
   ...
   If $\vv{k} \in \ZZ^n$ is optimal for the integer program $\IP(A,\vv{u},q)$, then the scaled point $\vv{k}/q$ is feasible for the linear program $\LP = \LP(A, \vv{u})$, so $\val \LP \geq \val \IP(A, \vv{u}, q)/q$ for all $q$.
...
We say that such a point $\defpt$ \emph{defines} $\O$ in $\N$.  In this article, we are largely concerned with faces $\O$ of $\N$ that do not lie in any proper coordinate subspace of $\RR^\numvars$, which we call \emph{standard}.
