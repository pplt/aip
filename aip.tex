...
\begin{remark}[Degrees of small pairs]
   ...
   Even though there may exist infinitely many small $\matA$-pairs, as shown in \Cref{ex: small vs very small}, the degrees of small $\matA$-pairs are bounded above, by a bound dependent solely on $\matA$, and consequently there are only finitely many such degrees by \Cref{discreteness: P}.
...
\begin{proof} 
The first point is a translation of \Cref{small pairs from shortfalls: L}, and the remaining points follow from \Cref{sprout: D} and \Cref{fundamental-properties-deficits-shortfalls: P}.
\end{proof}

\begin{lemma}
   \label{fundamental-sequences: L}
   If $(\matB, \vv{v})$ is a $\q$-sprout of $(\matA, \vv{u})$, then $\degree(\matB,\vv{v}) \leq \udeficit(\matA, \vv{u}, \q) $.  
\end{lemma}
...
\begin{proof}
   ...
   By definition, $\matB=\collapse{\matA}$ and $\vv{v} \in \ushort(\matA,\vv{u},\q)$.
...
On the other hand, since $\vv{v} \in \ushort(\matA, \vv{u}, \q)$, we can write $ \vv{v} = \matB \tail{\vv{s}}_{\q} - \matB \vv{h}$ for some $\vv{s} \in \sp_{\QQ}(\matA, \vv{u})$ and $\vv{h}$  optimal for $\ip = \ip(\matA, \vv{u}, \vv{s}, \q)$.  Then
\[ 
...
If $\matA$ is \hl{convenient and planar}, then every $r$-sprout of a pair $(\matA, \vv{u})$ is of the form $(\matA, \vv{v})$, and satisfies  $\degree(\matA,\vv{v}) = \udeficit(\matA, \vv{u}, \q)$. 
\end{lemma}
...
Thus, if $(\matB, \vv{v})$ is an $r$-sprout of $(\matA, \vv{u})$, then $\matB = \matA$, and $\vv{v} \in \ushort(\matA, \vv{u}, \q)$.  To realize $\vv{v}$,  fix a special point $\vv{s}$ for $(\matA, \vv{u})$, and write $\vv{v} = \matA \tail{\vv{s}}_{\q} - \matA \vv{h}$ for some $\vv{h}$ that is optimal for $\ip = \ip(\matA, \vv{u}, \vv{s}, \q)$.  \Cref{FT descriptions: P}\ref{new ip} tells us that $\deg(\matA, \vv{v}) = \iprod{\vv{c}}{\vv{v}}$, and as the inner product of $\vv{c}$ with every column of $\matA$ is $1$, we see that
\[ \deg(\matA, \vv{v}) = \iprod{\vv{c}}{\vv{v}} = \iprod{\vv{c}}{\matA \tail{\vv{s}}_{\q}} - \iprod{\vv{c}}{\matA \vv{h}} = \norm{\tail{\vv{s}}_{\q}} - \norm{\vv{h}} = \norm{\tail{\vv{s}_{\q}}} - \val \ip.\]
However, by definition, the right-most term above equals $\udeficit(\matA, \vv{u}, \q)$. 
\end{proof}
...
\hl{The set $\graph_{\q}^1(\matA)$, which is independent of $q$, is finite if and only if $\matA$ is convenient.}\daniel{Will not be used, but provides context for the second part of this statement.  No proof yet.}  In contrast, $\graph_{\q}^e(\matA)$ is nonempty and finite for every $e$ greater than $1$, and the sequence of sets $\{ \graph_{\q}^e(\matA) \}_{e=1}^{\infty}$ is eventually periodic in $e$.

\item \label{elementary1 periodic in r}
If $d$ is a \daniel{I think that \emph{universal denominator} implies \emph{collapsed universal denominator}, which is a ud for each of the collapses.}\hl{collapsed universal denominator} for $\matA$, and $r= \lpr{\q}{d}$, then the graphs $\graph_{\q}(\matA)$ and $\graph_r(\matA)$ --- that is, their vertices {and} arrows--- coincide.
\end{enumerate}
\end{proposition}
...
For the second assertion, let $\mathrm{L}$ be the set consisting of the finitely many collapses of $\matA$, fix an integer $e$ greater than $1$, and a vertex $(\matC, \vv{z})$ in $\graph_{\q}^e(\matA)$.  The first assertion tells us that both $\matC$, and the first entry $\matB$ of any vertex of level $e-1$ that $(\matC, \vv{z})$ sprouts from, lie in $\mathrm{L}$.  Thus, by definition of $\q$-sprout, the point $\vv{z}$ must lie in the double union $\mathrm{M} = \cup \cup \ushort(\matB, \vv{v}, \q)$, where the outer union is over all $\matB \in \mathrm{L}$, and the inner union is over all $\matB$-triples.    As $\mathrm{L}$ is finite, \Cref{fundamental-properties-deficits-shortfalls: P}\ref{finitely-many-A-universal-shortfalls: i}  implies that $\mathrm{M}$ is a nonempty, finite set of lattice points.   Thus, there is a finite set $\mathrm{H}$ such \ $\graph_{\q}^e(\matA) \subseteq \mathrm{H}$ for each $e>1$.  By the pigeon-hole principle, there must then exist indices $t > s > 1$ such that $\graph_{\q}^s(\matA) = \graph_{\q}^t(\matA)$, and the recursive nature of the definition of the sets $\graph_{\q}^i(\matA)$ then implies that $\graph_{\q}^{s+i}(\matA) = \graph_{\q}^{t+i}(\matA)$ for all $i \geq 0$.

Finally, the third assertion follows from \Cref{sprout: P}, {which implies that, in this context, the notions of $\q$-sprout and $r$-sprout coincide.}
\end{proof}
...
\daniel[inline]{Update $r \to \q$, revisit proof}
...
\newpage
\subsection{Canonical subgraphs associated to very small pairs}
...
\begin{definition}[Canonical subgraphs]
   ...
   \S_{\q}^e(\matA, \vv{u})$ to be the subset of $\T_{\q}^e(\matA, \vv{u})$ consisting of all $(\matB,\vv{v})$ that first maximize $\degree(\matB,\vv{v})$, and only then minimize $\udeficit_\q(\matB,\vv{v})$.  We then define $\T_\q^{e+1}(\matA,\vv{u})$ to be the set of all $\q$-sprouts of all pairs in $\S_{\q}^e(\matA,\vv{u})$.
...
\begin{definition}[The fundamental sequences]
   ...
   \end{align*}
   where $(\matB,\vv{v})$ is any pair in $\S_{\q}^e(\matA,\vv{u})$ | this is well-defined, by definition of $\S_{\q}^e(\matA,\vv{u})$ | and if $\E_{\q}(\matA, \vv{u})$ is finite, and $e$ is greater than $\E_{\q}(\matA, \vv{u})$, then \[\delta_{\q}^e(\matA, \vv{u}) = \eta_{\q}^e(\matA, \vv{u}) = 1.\]
