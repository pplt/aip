...
\begin{proof}
   ...
   If $\vv{k}$ is feasible for $\ip = \ip(A, \vv{u}, \vv{s}, q)$, then its $i$-th coordinate can only be negative if the $i$-th coordinate of $\vv{s}$ is positive, implying that the $i$-th column of $A$ must lie in $\O$.
   \Cref{prop: inner product with columns of A} then tells us that
\begin{equation}
...
showing that $\val \ip < \norm{\tail{\vv{s}}_q}$.
Finally, the positivity of $\collapse{A}\tail{\vv{s}}_q$ described in \Cref{tail projection: L} implies that $\vv{0}$ is feasible for $\ip$, and thus $\val \ip \ge 0$.
\end{proof}
