...
\pedro[inline]{
   ...
   This would imply that $p\vv{a}_i < p \vv{u}$, hence $\vv{a}_i < \vv{u}$, so the pair isn't even small, a contradiction.\\[2mm]
...
\begin{proof}
   ...
   The finiteness of $\fsr$,  and \Cref{positive-polynomial: L} below,
\pedro{Maybe place the lemma before this theorem?}
...
By definition, $B$ is the collapse of $A$ along $\O$, and $\vv{v} \in \ushort(A,\vv{u},p)$.  With this notation, \Cref{collapse of Newton polyhedron: P} tells us that $\collapse{\O}$ is a face of $\collapse{\N}$, the Newton polyhedron of $B=\collapse{A}$, and that $\collapse{\O}$ is defined in $\collapse{\N}$ by the collapsed point $\collapse{\vv{c}}$.




\Cref{FT descriptions: P}\eqref{lambda} implies that there exists a point $\vv{n} \in \collapse{\N}$ such that $\vv{v} = \ft{B}{\vv{v}} \cdot \vv{n}$.  As noted earlier, the point $\collapse{\vv{c}}$ defines the face $\collapse{\O}$ in $\collapse{\N}$, and by \Cref{alpha=1: convention}, it follows that $\iprod{\collapse{\vv{c}}}{\vv{n}}$ is at least $1$.  Thus, 
%
\[ \ft{B}{\vv{v}} \leq  \ft{B}{\vv{v}} \cdot \iprod{\collapse{\vv{c}}}{\vv{n}} = \iprod{\collapse{\vv{c}}}{\vv{v}}.\]
%
and so to complete our proof, it suffices to show that $\iprod{\collapse{\defpt}}{\vv{v}} \leq \udeficit(A, \vv{u}, p)$.

However, the containment $\vv{v} \in \ushort(A, \vv{u}, p)$ implies that we may write $ \vv{v} = B \tail{\vv{s}}_p - B \vv{h}$ for some $\vv{s} \in \sp_{\QQ}(A, \vv{u})$ and $\vv{h}$  optimal for $\ip = \ip(A, \vv{u}, \vv{s}, q)$.  As $\collapse{\vv{c}}$ defines $\collapse{\O}$,  the inner product of $\collapse{\defpt}$ with each column of $B$ is at least one, with equality whenever that column lies on $\collapse{\O}$.  Arguing as in the proof of \Cref{bounded value: P}, one may show that $\iprod{\collapse{\defpt}}{B \tail{\vv{s}}_p} = \norm{\tail{\vv{s}}_p}$ and also that $\iprod{\collapse{\defpt}}{ B \vv{h}} \geq \norm{\vv{h}} = \val \ip$,
